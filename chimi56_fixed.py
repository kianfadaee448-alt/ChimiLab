...
def configure_opengl_compatibility():
    """نسخه ۵۰ بدون SurfaceFormat کار می‌کرد — همان رفتار."""
    return HAS_OPENGL
...
HAS_WEBENGINE = None  # lazy — import زودهنگام روی ویندوز context OpenGL را خراب می‌کند
QWebEngineView = None
QWebEnginePage = None
QWebEngineSettings = None

def _ensure_webengine():
    """بارگذاری تنبل WebEngine فقط وقتی تب lab3d لازم است."""
    global HAS_WEBENGINE, QWebEngineView, QWebEnginePage, QWebEngineSettings
    if HAS_WEBENGINE is True:
        return True
    if HAS_WEBENGINE is False:
        return False
    try:
        from PySide6.QtWebEngineWidgets import QWebEngineView as _V
        from PySide6.QtWebEngineCore import QWebEnginePage as _P, QWebEngineSettings as _S
        QWebEngineView = _V
        QWebEnginePage = _P
        QWebEngineSettings = _S
        HAS_WEBENGINE = True
        print("[APP] QtWebEngine بارگذاری شد (lazy)", flush=True)
        return True
    except Exception as e:
        print(f"[WARN] QtWebEngine در دسترس نیست: {e}", flush=True)
        HAS_WEBENGINE = False
...
def get_lab3d_html_path():
    """مسیر فایل وب سه‌بعدی (lab3d.html) — جستجوی گسترده کنار exe/cwd/script."""
    ...
    candidates = [
        ...
        os.path.join(script_dir, "lab3d.html"),
        ...
        os.path.join(script_dir, "artifacts", "lab3d.html"),
        os.path.join(script_dir, "attachments", "lab3d.html"),
...
if True:
    ...
    class GLLabWorld(QWidget):
        """Fallback سه‌بعدی اتاق آزمایشگاه بدون OpenGL؛ چرخش و زوم با ماوس."""
        def __init__(self, engine=None, parent=None):
            super().__init__(parent); self.engine=engine; self.on_station=None; self._last=None
            self.figure=Figure(figsize=(8,5),facecolor="#0d1520"); self.canvas=FigureCanvas(self.figure); self.ax=self.figure.add_subplot(111,projection="3d")
            lay=QVBoxLayout(self); lay.setContentsMargins(0,0,0,0); lay.addWidget(self.canvas,1)
            self.canvas.mpl_connect("button_press_event",lambda e:setattr(self,"_last",(e.x,e.y))); self.canvas.mpl_connect("button_release_event",lambda e:setattr(self,"_last",None)); self.canvas.mpl_connect("motion_notify_event",self._move); self.canvas.mpl_connect("scroll_event",self._wheel); self._draw()
        def set_engine(self,eng): self.engine=eng; self._draw()
        def _move(self,e):
            if self._last is None or e.x is None or e.y is None:return
            dx=e.x-self._last[0]; dy=e.y-self._last[1]; self.ax.view_init(elev=max(5,min(75,self.ax.elev-dy*.25)),azim=self.ax.azim+dx*.3); self._last=(e.x,e.y); self.canvas.draw_idle()
        def _wheel(self,e):
            f=.88 if getattr(e,"button","")=="up" else 1.12; x=self.ax.get_xlim(); y=self.ax.get_ylim(); z=self.ax.get_zlim()
            self.ax.set_xlim((x[0]*f,x[1]*f)); self.ax.set_ylim((y[0]*f,y[1]*f)); self.ax.set_zlim((z[0]*f,z[1]*f)); self.canvas.draw_idle()
        def _box(self,x,y,z,dx,dy,dz):
            import itertools, numpy as _np
            verts=list(itertools.product([x-dx/2,x+dx/2],[y-dy/2,y+dy/2],[z-dz/2,z+dz/2])); faces=[(0,1,3,2),(4,5,7,6),(0,1,5,4),(2,3,7,6),(0,2,6,4),(1,3,7,5)]
            for face in faces:
                q=[verts[i] for i in face]; xs,ys,zs=zip(*q); self.ax.plot(xs+(xs[0],),ys+(ys[0],),zs+(zs[0],),alpha=.55,linewidth=.9)
        def _draw(self):
            ax=self.ax; ax.clear(); ax.set_facecolor("#0d1520")
            self._box(0,0,0,12,8,.1); self._box(0,2.5,-4,12,.1,5); self._box(-6,2.5,0,.1,8,5); self._box(6,2.5,0,.1,8,5)
            stations=[(-3,0,-2,"🔥"),(0,0,-2,"💧"),(3,0,-2,"🧊"),(-3,0,2,"🧪"),(0,0,2,"⚗️"),(3,0,2,"🚿")]
            for x,y,z,label in stations: ax.scatter([x],[y+.3],[z],s=130); ax.text(x,y+.7,z,label,ha="center",va="center",fontsize=12)
            ax.set_xlim(-7,7); ax.set_ylim(-4.5,4.5); ax.set_zlim(0,6); ax.set_box_aspect((1.55,1,0.7)); ax.view_init(elev=22,azim=-55); ax.set_axis_off(); ax.set_proj_type("ortho"); self.canvas.draw_idle()
        def update(self): self._draw()
        def get_hint(self): return "نمای ۳بعدی آفلاین · درگ ماوس برای چرخش · چرخ‌ماوس برای زوم"
...
class ModernLabWindow(QMainWindow):
    ...
    def _create_bottom_toolbar(self, parent_layout=None):
        ...
        menu.addSeparator(); menu.addAction("💧 تیتراسیون", self._menu_toggle_titration); menu.addAction("🧬 مولکول ۳بعدی", self.show_selected_molecule_3d); menu.addAction("🧪 ۳بعدی مخلوط", self.show_mixture_molecule_3d); menu.addAction("🏢 اتاق ۳بعدی", lambda: self.tabs.setCurrentWidget(self.lab3d_tab) if hasattr(self,'lab3d_tab') else None)
    ...
    def _create_center_panel(self):
        ...
        # 3D مستقل از OpenGL: همیشه قابل استفاده در محیط دسکتاپ
        try:
    ...
    def toggle_beaker_view(self):
        ...
        if self._beaker_is_3d:
            ...
            # دوبار update بعد از نمایش در stack (رفع صفحه سیاه Qt)
    ...
    def _create_all_tabs(self):
        ...
        if self.is_admin:
            ...
            self.lab3d_tab = self.create_lab3d_tab()
            self.tabs.addTab(self.lab3d_tab, "🏢 آزمایشگاه ۳بعدی")
    ...
    def create_lab3d_tab(self):
        ...
        has_html = os.path.isfile(html_path) and os.path.getsize(html_path) > 500
        print(f"[lab3d] path={html_path} exists={has_html} size={os.path.getsize(html_path) if has_html else 0}", flush=True)
        if _ensure_webengine() and has_html:
            try:
        ...
        if self.lab3d_web is None:
            try:
                ...
                self.lab3d_hint.setText(
                    "نمای ساده OpenGL (فایل lab3d.html یا WebEngine پیدا نشد) — "
                    "lab3d.html را کنار chimi56.py بگذارید و PySide6-WebEngine نصب کنید"
                )
        ...
        help_l = QLabel(
            "فایل‌های لازم: db.db · lab_save.json · chimi56.py · lab3d.html  |  "
    ...
    def _lab3d_toggle_fullscreen(self):
        ...
        try:
            target = getattr(self, "_lab3d_container", None)
            if target is None:
                target = self.lab3d_web if getattr(self, "lab3d_web", None) else getattr(self, "lab3d_world", None)
            if target is None:
                return
            # اگر خود پنجره قبلاً fullscreen است، خروج
            win = self.window()
            if win and win.isFullScreen() and getattr(self, "_lab3d_fs_window", False):
                win.showNormal()
                self._lab3d_fs_window = False
                self.btn_lab3d_fs.setText("⛶ تمام‌صفحه")
                return
            # تلاش JS fullscreen داخل WebEngine
            if getattr(self, "lab3d_web", None) is not None:
                try:
                    self.lab3d_web.page().runJavaScript(
                        "(function(){try{"
                    ...
                    pass
            # Qt window fullscreen به‌عنوان پشتیبان
            if win:
                if not win.isFullScreen():
                    win.showFullScreen()
                    self._lab3d_fs_window = True
    ...
    def _lab3d_enter_scene(self):
        """ورود صریح به کنترل صحنه — فوکوس وب‌ویو + JS enter."""
        try:
            if getattr(self, "lab3d_web", None) is not None:
                try:
                ...
                js = (
                    "(function(){"
                    "try{"
                    "if(typeof window.chimiEnter==='function'){window.chimiEnter();return 'chimiEnter';}"
                    "if(window.chimiBridge&&typeof window.chimiBridge.enter==='function'){window.chimiBridge.enter();return 'bridge';}"
                    "var b=document.getElementById('btn-enter'); if(b){b.click();return 'click';}"
                    "return 'noop';"
                ...
                try:
                    ...
                    tlog(f"lab3d enter js: {e}", "WARN")
                self.lab3d_hint.setText("کنترل فعال — WASD حرکت · کلیک‌چپ+درگ نگاه · E تعامل · Esc خروج")
                return
            if getattr(self, "lab3d_world", None) is not None:
                try:
                    self.lab3d_world.setFocus()
                except Exception:
                    pass
                self.lab3d_hint.setText("OpenGL — WASD | ماوس | E | تمام‌صفحه")
        except Exception as e:
    ...
    def _on_graph_tab_changed(self, index):
        """تغییر تب: نمودار / lab3d (بارگذاری تنبل) / آزادسازی WebEngine برای OpenGL."""
        try:
            cur = self.tabs.currentWidget()
            if cur is getattr(self, "graph_tab_widget", None):
                self._refresh_graph()
                self.graph_canvas.draw_idle()
            lab = getattr(self, "lab3d_tab", None)
            if lab is not None and cur is lab:
                # ورود به آزمایشگاه ۳بعدی — WebEngine فقط اینجا
                self._ensure_lab3d_view()
            else:
                # خروج از lab3d — WebEngine را ببند تا بشر/بور سیاه نشوند
                if getattr(self, "lab3d_web", None) is not None:
                    self._release_lab3d_web_for_opengl()
                # بعد از آزادسازی، OpenGL را تازه کن
                glb = getattr(self, "gl_beaker", None)
                if glb is not None and getattr(self, "_beaker_is_3d", False):
                    try:
                        glb.makeCurrent()
                    except Exception:
                        pass
                    setattr(glb, "_needs_redraw", True)
                    glb.update()
                    QTimer.singleShot(80, glb.update)
        except Exception as e:
...
if __name__ == '__main__':
    ...
    try:
        from PySide6.QtCore import Qt as _QtGL
        ...
        print(f"[GL] AA_UseDesktopOpenGL: {e}", flush=True)
    try:
    ...
    print(f"[APP] {APP_NAME} {APP_VERSION} | OpenGL={HAS_OPENGL} | WebEngine=lazy | molmass={HAS_MOLMASS}", flush=True)
