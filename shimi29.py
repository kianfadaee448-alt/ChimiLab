...
PERIODIC_TABLE = {
    ...
    "O": "اکسیژن", "F": "فلوئور", "Ne": "نئون", "Na": "سدیم", "Mg": "منیزیم", "Al": "آلومینیوم", "Si": "سیلیسیم",
    "P": "فسفر", "S": "گوگرد", "Cl": "کلر", "Ar": "آرگون", "K": "پتاسیم", "Ca": "کلسیم", "Fe": "آهن", "Cu": "مس",
    "Zn": "روی", "Ag": "نقره", "Au": "طلا", "Hg": "جیوه", "Pb": "سرب", "Sn": "قلع", "Mn": "منگنز", "Cr": "کروم",
    "Ni": "نیکل", "Co": "کبالت", "Br": "برم", "I": "ید"
}
CHEMICAL_DB = {
...
def normalize_key(key):
    """استانداردسازی نام مواد برای جستجو"""
    key = re.sub(
...
def load_databases():
    """بارگذاری هوشمند دیتابیس از منابع مختلف"""
    global CHEMICAL_DB, CUSTOM_REACTIONS
    loaded = False

    # 1. تلاش برای ایمپورت از shimi27
    try:
        from shimi27 import CHEMICAL_DB as DB_27, CUSTOM_REACTIONS as RXN_27
        CHEMICAL_DB.update(DB_27)
        CUSTOM_REACTIONS.update(RXN_27)
        print("Database loaded from shimi27.py successfully.")
        loaded = True
    except ImportError:
        pass

    # 2. تلاش برای بارگذاری از JSON
    if os.path.exists("materials.json"):
        try:
            with open("materials.json", "r", encoding="utf-8") as f:
                new_materials = json.load(f)
                for key, val in new_materials.items():
                    CHEMICAL_DB[normalize_key(key)] = val
            loaded = True
    ...
    if os.path.exists("reactions.json"):
        try:
            with open("reactions.json", "r", encoding="utf-8") as f:
                ...
                CUSTOM_REACTIONS.update(new_reactions)
            loaded = True
        except Exception as e:
            print(f"Error loading reactions.json: {e}")
...
class ChemicalCalculator:
    ...
    def parse_formula(formula):
        ...
        # باز کردن پرانتزها: (NH4)2 -> NH4NH4
    ...
    def format_gross_formula(atom_counts, simplify=True, empirical=False):
        ...
        if empirical:
            ...
            if vals:
                ...
                for el in sorted_elements: atom_counts[el] = round(atom_counts[el], 2)
        ...
        for el in sorted_elements:
            ...
            if count > 0:
                ...
                formula_str += f"{el}{display_count if display_count != 1 else ''}"
...
class AnimatedBeaker(QWidget):
    ...
    def fluidHeight(self, value):
        self._fluid_height = value; self.update()
    ...
    def fluidColor(self, color):
        self._fluid_color = color; self.update()
    ...
    def flashOpacity(self, value):
        self._flash_opacity = value; self.update()
...
class LabEngine:
    ...
    def check_reactions(self):
        """بررسی واکنش‌ها با اولویت کشف‌های جدید"""
            ...
            if needed.issubset(present):
                ...
                if self.temp_c >= req_temp:
                    if name not in self.discovered:
                        # اولویت بالا: کشف جدید
                        self.discovered.add(name)
                        self.score += rxn["xp"]
                        if self.score >= self.level * 100: self.level += 1
                        self.save_data()
                        self.temp_c += 10.0  # گرمای واکنش
                        return (name, rxn["xp"], "new")
                    else:
                        # اولویت پایین: واکنش قدیمی (فقط اگر جدید پیدا نشد برگردانده می‌شود)
...
class ModernLabWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("آزمایشگاه شیمی پیشرفته (نسخه بهینه)")
        self.resize(1500, 900)
        self.setLayoutDirection(Qt.RightToLeft)

        # بارگذاری دیتابیس
        if not load_databases():
            QMessageBox.warning(self, "هشدار دیتابیس",
                                "دیتابیس مواد خالی است.\nلطفاً فایل shimi27.py را در کنار این فایل قرار دهید یا دیتابیس را پر کنید.")
    ...
    def setup_ui(self):
        ...
        tabs.addTab(self.create_builder_tab(), "🧩 اتم ساز")
    ...
    def get_state_color_text(self, formula):
        ...
        d = CHEMICAL_DB.get(norm)
        if not d: return formula
        return f"{d.get('name', formula)} <span style='color:{d.get('color', '#fff')};'>■</span>"
    ...
    def create_builder_tab(self):
        ...
        btn_add = QPushButton("افزودن");
        btn_add.clicked.connect(self.action_builder_add)
        btn_calc = QPushButton("محاسبه");
        btn_calc.clicked.connect(self.action_builder_calc)
        btn_clr = QPushButton("پاک");
        ...
        vl.addWidget(btn_calc);
        vl.addWidget(btn_clr);
        vl.addStretch()

        res_frm = QFrame();
        rl = QVBoxLayout(res_frm)
        self.lbl_b_res = QLabel("?");
        self.lbl_b_res.setStyleSheet("font-size: 24px; color: #fab387; border: 1px dashed white; padding: 10px;")
        self.lbl_b_desc = QLabel("...")
        rl.addWidget(self.lbl_b_res, 0, Qt.AlignCenter);
        rl.addWidget(self.lbl_b_desc)
        l.addWidget(frm);
        l.addWidget(res_frm)
        return w
    ...
    def create_datasheet_tab(self):
        ...
        t = QTableWidget();
        t.setColumnCount(3);
        t.setHorizontalHeaderLabels(["نام", "فرمول", "نوع"])
    ...
    def populate_chemicals(self):
        ...
        for k, v in sorted(CHEMICAL_DB.items(), key=lambda x: x[1]['name']):
            self.combo_chem.addItem(f"{v['name']}", k)
    ...
    def filter_chemicals(self, text):
        ...
        t = text.lower()
        for k, v in sorted(CHEMICAL_DB.items(), key=lambda x: x[1]['name']):
            if t in v['name'].lower() or t in k:
                self.combo_chem.addItem(f"{v['name']}", k)
    ...
    def handle_reaction_result(self, disc):
        ...
        if status == "new":
            ...
            self.update_discoveries_table()
            self.update_wiki_table()
            QMessageBox.information(self, "کشف!", f"واکنش: {name}\nامتیاز: {xp}")
            self.timer.start(50)
        elif status == "old":
            # لاگ تکراری ندهد
            pass
    ...
    def action_wash(self):
        ...
        self.update_contents_ui()
        self.txt_log.append("بشر شسته شد.")

    def action_hard_reset(self):
        if QMessageBox.question(self, "ریست", "مطمئنی؟", QMessageBox.Yes | QMessageBox.No) == QMessageBox.Yes:
    ...
    def action_builder_calc(self):
        ...
        form = ChemicalCalculator.format_gross_formula(atoms, False, False)
        self.lbl_b_res.setText(form)
        # جستجو در دیتابیس
        emp = ChemicalCalculator.format_gross_formula(atoms, True, True)
        plain = emp.translate(str.maketrans("₀₁₂₃₄₅₆₇₈₉", "0123456789"))
        found = []
        for v in CHEMICAL_DB.values():
            da = ChemicalCalculator.parse_formula(v['formula'])
            de = ChemicalCalculator.format_gross_formula(da, True, True)
            dp = de.translate(str.maketrans("₀₁₂₃₄₅₆₇₈₉", "0123456789"))
            if plain == dp: found.append(v['name'])
        self.lbl_b_desc.setText(f"مشابه: {', '.join(found)}" if found else "ناشناخته")
    ...
    def update_discoveries_table(self):
        ...
        for i, (n, d) in enumerate(CUSTOM_REACTIONS.items()):
            if n in self.engine.discovered:
                ...
                self.table_disc.setItem(i, 1, QTableWidgetItem(str(d['xp'])))
            else:
    ...
    def update_contents_ui(self):
        ...
        for k, v in self.engine.contents.items():
            if v > 0.0001:
                ...
                n = CHEMICAL_DB.get(k, {}).get('name', k)
                f = CHEMICAL_DB.get(k, {}).get('formula', '?')
        ...
        self.lbl_mix.setText(f"مجموع: {f}")

    def game_loop(self):
        self.engine.update_physics()
        if not hasattr(self, 'st'): self.st = time.time()
        t = time.time() - self.st
        self.lbl_ph_display.setText(f"pH: {self.engine.get_ph():.2f}")
        self.lbl_temp_display.setText(f"{self.engine.temp_c:.1f} °C")

        disc = self.engine.check_reactions()
        self.handle_reaction_result(disc)

        self.data_time.append(t);
        self.data_ph.append(self.engine.get_ph());
        self.data_temp.append(self.engine.temp_c)
