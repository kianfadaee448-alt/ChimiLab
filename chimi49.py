...
class LabEngine:
    ...
    def get_ph(self):
        if self.total_volume <= 0 or self.is_broken:
            return 7.0
        # هر چند فراخوانی یک‌بار از محتویات همگام شو
        if not hasattr(self, "_ph_rebuild_i"):
            self._ph_rebuild_i = 0
        self._ph_rebuild_i += 1
        if self._ph_rebuild_i % 5 == 1:
            try:
                self.rebuild_acid_base_moles()
            except Exception:
                pass
...
class ModernLabWindow(QMainWindow):
    ...
    def action_export_pdf(self):
        ...
        try:
            ...
            painter.drawText(QRectF(margin, y + 160, w - 2 * margin, 70), Qt.AlignHCenter, "شیمی‌لَب  •  Universe ChimiLab V49")
            ...
            painter.drawText(QRectF(margin, h - 70, w - 2 * margin, 40), Qt.AlignHCenter,
                             f"شیمی‌لَب (Universe ChimiLab V49)  |  صفحه {page_num[0]}  |  تولید خودکار گزارش")
