...
# import copy  # removed - no longer using deepcopy
...
try:
    ...
    from PySide6.QtGui import QFont, QColor, QPainter, QPen, QLinearGradient, QPainterPath, QBrush, QRadialGradient, \
        QPixmap, QPdfWriter, QPageSize, QImage, QAction, QIcon, QDrag
...
# QPainter alias used in export/screenshot code
QPainterGui = QPainter

# تلاش برای وارد کردن OpenGL (اگر نباشد برنامه کرش نمی‌کند)
HAS_OPENGL = False
try:
    ...
    print(f"[WARN] OpenGL در دسترس نیست — حالت ۲بعدی فعال می‌شود: {e}", flush=True)
    HAS_OPENGL = False


# ============================================================
# ثابت‌های سراسری و تنظیمات
# ============================================================
...
QMainWindow { background-color: #0c0914; }
QWidget { color: #e8e0f5; font-family: 'Tahoma', sans-serif; font-size: 13px; }
QGroupBox {
    border: 1px solid #3d2a55; border-radius: 14px; margin-top: 18px;
    background-color: qlineargradient(x1:0,y1:0,x2:0,y2:1, stop:0 #1a1228, stop:1 #120e1c);
    font-weight: bold;
}
QGroupBox::title {
    subcontrol-origin: margin; subcontrol-position: top center;
    padding: 5px 16px; background-color: #2a1a40; color: #c4b5fd;
    border-radius: 10px; border: 1px solid #5b3d7a;
}
QPushButton {
    background-color: qlineargradient(x1:0,y1:0,x2:0,y2:1, stop:0 #3b2a55, stop:1 #2a1e3d);
    border: 1px solid #5b3d7a; border-radius: 8px;
    padding: 5px 10px; color: #f0e8ff; font-weight: bold; font-size: 12px;
    min-height: 24px;
}
QPushButton:hover {
    background-color: qlineargradient(x1:0,y1:0,x2:0,y2:1, stop:0 #4c3670, stop:1 #3a2855);
    border: 1px solid #a78bfa;
}
QPushButton:pressed { background-color: #1a1028; border: 2px solid #e879f9; }
QPushButton:checked {
    background-color: qlineargradient(x1:0,y1:0,x2:0,y2:1, stop:0 #c4b5fd, stop:1 #8b5cf6);
    color: #0c0914; border: 2px solid #ddd6fe;
}
QLineEdit {
...
# ATOMIC_DB removed for performance (Bohr model eliminated)
...
# ============================================================
# دیتابیس و بارگذاری داده (SQLite)
# ============================================================
CUSTOM_REACTIONS = {}
CHEMILAB_DB = {}
...
def load_databases():
    ...
    try:
        ...
        import traceback
        traceback.print_exc()
        # داده‌های پیش‌فرض حداقلی تا برنامه قابل استفاده بماند
        if not CHEMILAB_DB:
            CHEMILAB_DB["h2o"] = {"name": "آب", "type": "Liquid", "pH": 7.0, "molarity": 55.5, "heat": 0.0,
                                  "color": "#aaddff", "formula": "H2O"}
            CHEMILAB_DB["hcl"] = {"name": "هیدروکلریک اسید", "type": "Strong Acid", "pH": 1.0, "molarity": 1.0, "heat": 0.0,
...
class AppLogger:
    """لاگ با بافر حافظه + نوشتن غیرمسدود روی دیسک (جلوگیری از لگ GUI)"""
    def __init__(self, filename="chimi_lab.log"):
        ...
        self._flush_interval = 2.0  # ثانیه
        try:
    ...
    def _flush_buffer(self, force=False):
        ...
        if not self._buffer:
            return
        if not force and (now - self._last_flush) < self._flush_interval and len(self._buffer) < self._max_buffer:
            return
        try:
            with open(self.path, "a", encoding="utf-8") as f:
                f.write("\n".join(self._buffer) + "\n")
    ...
    def log(self, level, msg, **extra):
        ...
        if len(self.memory) > self.max_mem:
            self.memory.pop(0)
        # لاگ‌های پرتکرار UI/LAYOUT/CLICK فقط در حافظه
...
# ============================================================
# DimensionController — کنترل پایدار ابعاد ویجت‌ها (ضد پرش layout)
# ============================================================
...
class ChemicalCalculator:
    ...
    # ایزوتوپ هیدروژن → در شمارش اتمی می‌تواند جدا یا با H باشد
    ISOTOPE_MAP = {"D": "H", "T": "H"}

    ATOMIC_MASS = {
        "H": 1.008, "D": 2.014, "T": 3.016, "He": 4.003, "Li": 6.94, "Be": 9.012,
        "B": 10.81, "C": 12.011, "N": 14.007, "O": 15.999, "F": 18.998, "Ne": 20.18,
        "Na": 22.99, "Mg": 24.305, "Al": 26.982, "Si": 28.085, "P": 30.974, "S": 32.06,
        "Cl": 35.45, "K": 39.098, "Ca": 40.078, "Fe": 55.845, "Cu": 63.546, "Zn": 65.38,
        "Br": 79.904, "Ag": 107.87, "I": 126.90, "Ba": 137.33, "Au": 196.97, "Hg": 200.59,
        "Pb": 207.2, "U": 238.03,
    }
    ...
    def parse_formula(formula, merge_isotopes=False):
        """
        پارسر امن فرمول شیمیایی — بدون recursion عمیق و مقاوم در برابر ورودی‌های مخرب.
        اولویت: molmass → پارسر خطی ساده.
        """
        s = ChemicalCalculator._normalize_input(formula)
        if not s:
            return Counter()
        # محدودیت طول برای جلوگیری از حملات ReDoS / recursion
        if len(s) > 200:
            return Counter()
        # تعداد پرانتزها را محدود کن
        if s.count('(') > 12 or s.count(')') > 12:
            return Counter()
        ...
        # پارسر خطی (غیر بازگشتی) با پشته و محدودیت عمق
        i, nlen = 0, len(s)
        MAX_DEPTH = 8
        ...
        try:
            while i < nlen:
                ...
                if ch == '(':
                    ...
                    i += 1
            result = stack[0] if stack else Counter()
            if result:
                return result
        except Exception:
            pass

        # fallback خیلی ساده با regex محدود
        flat = Counter()
        try:
            for el, num in re.findall(r'([A-Z][a-z]?)(\d{0,4})', s[:200]):
                if merge_isotopes:
                    el = ChemicalCalculator.ISOTOPE_MAP.get(el, el)
    ...
    def molar_mass(formula):
        ...
        for el, cnt in ChemicalCalculator.parse_formula(s, merge_isotopes=False).items():
            total += ChemicalCalculator.ATOMIC_MASS.get(el, 0.0) * cnt
        return total
    ...
    def reduce_slots(atom_amounts):
        """مقادیر خام ظرف‌ها → کوچک‌ترین نسبت صحیح"""
        from fractions import Fraction
        from math import gcd
        from functools import reduce

        filtered = {}
        for k, v in (atom_amounts or {}).items():
            try:
                fv = float(v)
            except (TypeError, ValueError):
                continue
        ...
        min_v = min(filtered.values())
        fracs = {el: Fraction(val / min_v).limit_denominator(24) for el, val in filtered.items()}
        denoms = [f.denominator for f in fracs.values()]

        def _lcm(a, b):
            return abs(a * b) // gcd(a, b) if a and b else (a or b)

        common = reduce(_lcm, denoms, 1)
        if common <= 0:
            return {}

        counts = {
            el: int(f.numerator * (common // f.denominator))
        ...
        for v in vals[1:]:
            g = gcd(g, v)
        if g > 1:
            counts = {el: n // g for el, n in counts.items()}
        return counts
    ...
    def fill_atom_jars(contents_moles, db, merge_isotopes=True):
        ...
        for key, moles in contents_moles.items():
            ...
            if not form or form in ("-", "?", "None"):
                # گاهی کلید خودش فرمول است
                form = str(key)
    ...
    def mixture_formula(contents_moles, db, merge_isotopes=True):
        ...
        # تک‌ماده: فرمول اصلی را نشان بده
        if len(active) == 1:
            ...
            if db and key in db:
                ...
                if form and form not in ("-", "?"):
                    # نرمال و زیروند
                    parsed = ChemicalCalculator.parse_formula(form, merge_isotopes=False)
                    if parsed:
                        return ChemicalCalculator.format_from_slots(parsed, subscript=True)
                    return ChemicalCalculator.to_subscript(
                        ChemicalCalculator._normalize_input(form) or form
                    )
            return ChemicalCalculator.to_subscript(str(key))

        # چندماده: پر کردن ظرف‌ها و تجربی
    ...
    def elemental_mass_percent(formula):
        ...
        masses = {}
        for el, cnt in atoms.items():
            m = ChemicalCalculator.ATOMIC_MASS.get(el, 0.0) * cnt
            masses[el] = m
            total += m
        if total <= 0:
            return {}
    ...
    def empirical_from_percentages(percent_dict):
        ...
        for el, pct in percent_dict.items():
            try:
                ...
                continue
            mass = ChemicalCalculator.ATOMIC_MASS.get(el, 0.0)
            if mass > 0 and p > 0:
                moles[el] = p / mass
        if not moles:
            return "-"
    ...
    @staticmethod
    def estimate_reaction_delta_h(reactants_keys, products_keys, db):
        """
        تقریب خام ΔH از فیلد heat مواد (kJ/mol تقریبی).
        مثبت = گرماگیر، منفی = گرمازا
        """
        def sum_heat(keys):
            total = 0.0
            for k in keys or []:
                ...
                total += safe_float(data.get("heat", 0.0), 0.0)
            return total
        # heat در دیتابیس اغلب گرمای تشکیل/انحلال است؛ ΔH ≈ Σproducts − Σreactants
        return sum_heat(products_keys) - sum_heat(reactants_keys)

    @staticmethod
    def find_limiting_reagent(reactants_needed, contents_moles, db, coeffs=None):
        """
        reactants_needed: list کلید مواد واکنش‌دهنده
        contents_moles: self.contents
        coeffs: dict کلید→ضریب استوکیومتری (پیش‌فرض ۱)
        خروجی: (کلید ماده محدودکننده, مول موجود, درصد تبدیل تقریبی, moles_extent)
        """
        ...
        for r in reactants_needed:
            ...
            for ck, cm in (contents_moles or {}).items():
                if normalize_key(ck) == nk or normalize_key(
                        (db.get(ck) or {}).get("formula", "")) == nk:
                    moles += safe_float(cm, 0.0)
            coef = max(1e-12, float(coeffs.get(r, coeffs.get(nk, 1)) or 1))
            extent = moles / coef if coef > 0 else 0.0
            ratios.append((r, moles, extent, coef))
        if not ratios:
            return None, 0.0, 0.0, 0.0
        # کمترین extent = محدودکننده
        lim_key, lim_moles, lim_extent, lim_coef = min(
            ratios, key=lambda x: x[2] if x[2] > 0 else 1e99)
        if lim_extent <= 0:
            return lim_key, 0.0, 0.0, 0.0
        # درصد تبدیل تقریبی بر اساس محدودکننده
        return lim_key, lim_moles, 100.0, lim_extent


# ============================================================
# منطق اصلی آزمایشگاه (LabEngine)
# ============================================================
class LabEngine:
    def __init__(self):
        self.score = 0
    ...
    def load_data(self):
        ...
        if os.path.exists(save_path):
            try:
                ...
                print(f"[SAVE][ERROR] بارگذاری lab_save.json ناموفق: {e}", flush=True)
    ...
    def save_data(self):
        ...
        try:
            ...
            print(f"[SAVE][ERROR] ذخیره lab_save.json ناموفق: {e}", flush=True)
    ...
    def from_dict(self, d):
        ...
        # نرمال‌سازی محتویات به float
        self.contents = {str(k): safe_float(v, 0.0) for k, v in self.contents.items()}
        layers = d.get("visual_layers", [])
        self.visual_layers = [dict(l) for l in layers] if isinstance(layers, list) else []
    ...
    def _make_snapshot(self):
        """ساخت snapshot سبک وضعیت ظرف (کپی سطحی + کپی محتویات)"""
        # contents و visual_layers را با list/dict comprehension کپی می‌کنیم (سبک‌تر از deepcopy کامل)
        layers = []
    ...
    def push_undo(self):
        """ذخیره وضعیت فعلی ظرف برای بازگشت — محدود به تعداد کم و کپی سبک"""
        # جلوگیری از pushهای پشت‌سرهم خیلی سریع (مثلاً در تیتراسیون)
        now = time.time()
        if hasattr(self, '_last_undo_push') and (now - self._last_undo_push) < 0.15:
            return
        self._last_undo_push = now
        snap = self._make_snapshot()
        self._undo_stack.append(snap)
        self._redo_stack.clear()
        if len(self._undo_stack) > 20:  # کاهش از 40 به 20
            self._undo_stack.pop(0)

    def undo(self):
        if not self._undo_stack:
            return False
        # وضعیت فعلی را برای redo نگه دار
        ...
        self.contents = dict(snap["contents"])
        self.visual_layers = [dict(l) for l in snap["visual_layers"]]
    ...
    def add_chemical(self, key, amount, custom_molarity=None):
        ...
        if self.temp_c < -20:
            ...
            self.safety_warnings_count += 1
        # ۷. مواد ناسازگار رایج (کلر + آمونیاک تقریبی)
        incompat = [
    ...
    def _parse_simple_coeffs(self, rxn):
        ...
        # برای reactants و products اگر ضریب نداشتند ۱
    ...
    def _apply_reaction_stoich(self, rxn, extent):
        ...
        for r in rxn.get("reactants", []):
            ...
            if not ck:
                continue
            coef = coeffs.get(normalize_key(r), 1)
            consume = extent * coef
            if ck in self.contents:
                self.contents[ck] = max(0.0, self.contents[ck] - consume)
                if self.contents[ck] <= 1e-12:
                    del self.contents[ck]
            # کاهش از لایه‌های بصری
            remaining = consume
            for layer in list(self.visual_layers):
                if layer.get('key') == ck and remaining > 0:
                    take = min(layer.get('moles', 0), remaining)
                    if take <= 0:
                        continue
                    ratio = 1.0 - (take / max(layer.get('moles', 1e-12), 1e-12))
                    layer['moles'] = max(0.0, layer['moles'] - take)
                    layer['amount'] *= max(0.0, ratio)
                    remaining -= take
                    if layer['moles'] <= 1e-12 or layer['amount'] <= 0.05:
                        self.visual_layers.remove(layer)
        # افزودن محصولات
        for p in rxn.get("products", []):
            pk = normalize_key(p)
            # پیدا کردن کلید دیتابیس
            db_key = None
            if pk in CHEMILAB_DB:
                ...
                for k, v in CHEMILAB_DB.items():
                    if normalize_key(v.get("formula", "")) == pk or normalize_key(k) == pk:
                        ...
                        break
            if not db_key:
                # محصول ناشناخته — به‌صورت کلید خام اضافه کن
                db_key = pk
            coef = coeffs.get(normalize_key(p), 1)
            ...
            if any(x in str(chem_type) for x in ["Solid", "Metal", "Salt", "Powder", "Precipitate", "جامد"]):
                amt = add_moles * 20.0  # تقریبی g
            elif "Gas" in str(chem_type) or "گاز" in str(chem_type):
                amt = add_moles * 50.0
            else:
                amt = add_moles * 30.0  # mL تقریبی
            ...
            self.visual_layers.append({
                ...
                'moles': add_moles,
                'formula': data.get('formula', p)
            })
            # به‌روزرسانی H/OH تقریبی
            ph_val = safe_float(data.get("pH", 7.0), 7.0)
            if ph_val < 7:
                self.moles_h += add_moles * (1 if ph_val < 2 else 0.1)
            elif ph_val > 7:
                self.moles_oh += add_moles * (1 if ph_val > 12 else 0.1)
        # بازمحاسبه حجم از لایه‌ها
        self.total_volume = sum(l.get('amount', 0) for l in self.visual_layers)
        if self.total_volume < 0:
            self.total_volume = 0.0
        # پاکسازی محتویات صفر
        ...
        for name, rxn in CUSTOM_REACTIONS.items():
            ...
            if len(needed) > 0 and needed.issubset(present):
                ...
                if self.temp_c >= req_temp:
                    ...
                    # محدود کردن extent برای جلوگیری از مصرف کامل در هر فریم (واکنش تدریجی)
                    # برای کشف جدید، بخش قابل توجهی اعمال می‌شود
                    ...
                    thermo = {
                        ...
                        "yield_approx": min(100.0, conv_pct * 0.92),  # بازده تقریبی آموزشی
                    }
                    ...
                    if name not in self.discovered:
                        self.push_undo()
                        # اعمال واقعی واکنش فقط هنگام کشف جدید (یک‌بار)
                        self._apply_reaction_stoich(rxn, applied_extent)
                        ...
                        if thermo["is_exothermic"]:
                            self.temp_c += max(8.0, min(40.0, abs(dH) * 0.8))
                        elif thermo["is_endothermic"]:
                            self.temp_c -= max(3.0, min(20.0, abs(dH) * 0.4))
                        else:
                            self.temp_c += 10.0
                        self.add_to_log(
                            f"واکنش جدید کشف شد: {name} | محدودکننده: {lim_key} | "
                            f"بازده تقریبی: {thermo['yield_approx']:.0f}% | تبدیل: {conv_pct:.0f}%")
                        return (name, rxn["xp"], "new", has_precipitate, has_gas, thermo)
                    else:
                        # واکنش از قبل کشف‌شده — فقط اطلاع، بدون deepcopy/اعمال مکرر (علت اصلی لگ)
                        found_old = (name, 0, "old", has_precipitate, has_gas, thermo)
        return found_old
...
# ============================================================
# مدل اتمی بور و تمام وابستگی‌های آن حذف شدند (رفع لگ و افت فریم)
# ============================================================
...
class ModernLabWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("آزمایشگاه شیمی پیشرفته - شیمی‌لَب (نسخه 46.1 — واکنش واقعی + Undo/Redo + ایمنی)")
        ...
        try:
            get_logger().info(f"شروع برنامه — کاربر: {self.engine.player_name}")
        except Exception:
            pass
    ...
    def setup_ui(self):
        ...
        self.btn_redo.setStyleSheet(
            "background-color: #cba6f7; color: #1e1e2e; font-weight: bold; padding: 8px 14px; "
            "border-radius: 8px; font-size: 14px;")
        ...
        self.lbl_top_status = QLabel("شیمی‌لَب V46.1")
        ...
        # حداقل عرض تا هیچ پنلی صفر نشود
        ...
        # اطمینان از ترتیب stretch در layout
    ...
    def eventFilter(self, obj, event):
        ...
        if obj == getattr(self, 'combo_chem', None):
            if event.type() == QEvent.MouseButtonPress and event.button() == Qt.LeftButton:
                self._drag_start_pos = event.pos()
            elif event.type() == QEvent.MouseMove and event.buttons() & Qt.LeftButton:
                if hasattr(self, '_drag_start_pos'):
                    if (event.pos() - self._drag_start_pos).manhattanLength() >= 12:
                        key = self.combo_chem.currentData()
                        if key:
                            drag = QDrag(self.combo_chem)
                            mime = QMimeData()
    ...
    def open_settings_panel(self):
        ...
        btn_ok.setStyleSheet("background-color: #a6e3a1; color: #1e1e2e; font-weight: bold; padding: 10px; border-radius: 8px;")
        btn_restore = QPushButton("بازگردانی پنل‌ها")
        btn_restore.setStyleSheet("background-color: #fab387; color: #1e1e2e; font-weight: bold; padding: 10px; border-radius: 8px;")
        btn_log = QPushButton("📋 باز کردن فایل لاگ")
        btn_log.setStyleSheet("background-color: #89b4fa; color: #1e1e2e; font-weight: bold; padding: 10px; border-radius: 8px;")
        def open_log():
            import subprocess
            p = get_logger().path
            try:
    ...
    def _create_left_panel(self):
        panel = QFrame()
        DimensionController.lock(panel, w=400, min_w=400, max_w=400)
        ...
        # نام + تم در یک ردیف (مثل قبل)
        h_top = QHBoxLayout()
        self.lbl_welcome = QLabel(f"👤 شیمیدان: {self.engine.player_name}")
        self.lbl_welcome.setStyleSheet("color: #a6e3a1; font-size: 15px; font-weight: bold;")
        self.lbl_welcome.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Preferred)
        btn_theme = QPushButton("🌓 تم")
        DimensionController.lock_btn(btn_theme, w=90, h=36)
        btn_theme.setStyleSheet(
            "background-color: #45475a; color: #f9e2af; font-weight: bold; "
            "border-radius: 8px; font-size: 14px;")
        btn_theme.clicked.connect(self.toggle_theme)
        h_top.addWidget(self.lbl_welcome, 1)
        h_top.addWidget(btn_theme, 0)
        v_player.addLayout(h_top)

        # ذخیره / بارگذاری — ارتفاع ثابت، بدون فشرده‌سازی عجیب
        h_save = QHBoxLayout()
        h_save.setSpacing(10)
        btn_save = QPushButton("💾 ذخیره")
        btn_load = QPushButton("📂 بارگذاری")
        for b in (btn_save, btn_load):
            DimensionController.lock(b, h=36, min_w=110)
            b.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)
            b.setStyleSheet("font-size: 12px; font-weight: bold; padding: 6px 10px; border-radius: 8px;")
        ...
        self.combo_chem.setToolTip("ماده را انتخاب کنید و روی ظرف بکشید (Drag & Drop) یا دکمه افزودن را بزنید")
        ...
        btn_add.setStyleSheet("background-color: #a6e3a1; color: #1e1e2e; font-weight: bold; border-radius: 7px;")
        btn_drag = QPushButton("🖐️ کشیدن")
        btn_drag.setFixedHeight(34)
        btn_drag.setToolTip("بزنید و روی ظرف رها کنید")
        btn_drag.setStyleSheet("background-color: #f9e2af; color: #1e1e2e; font-weight: bold; border-radius: 7px;")
        ...
        self.btn_titrate.setStyleSheet("background-color: #89b4fa; color: #1e1e2e; font-weight: bold; border-radius: 7px;")
        ...
        # ---------- دما و سرعت (عملیات ظرف در نوار پایین) ----------
        gb_tools = QGroupBox("دما و سرعت")
        v_tools = QVBoxLayout()
        v_tools.setSpacing(8)
        v_tools.setContentsMargins(8, 14, 8, 8)

        h_temp = QHBoxLayout()
        h_temp.setSpacing(10)
        btn_cool = QPushButton("🧊 −۵°")
        btn_heat = QPushButton("🔥 +۵°")
        DimensionController.lock_btn(btn_cool, w=150, h=36)
        DimensionController.lock_btn(btn_heat, w=150, h=36)
        btn_cool.clicked.connect(self.action_cool)
        btn_heat.clicked.connect(self.action_heat)
        btn_cool.setStyleSheet("background-color: #89b4fa; color: #1e1e2e; font-weight: bold; border-radius: 8px;")
        btn_heat.setStyleSheet("background-color: #f38ba8; color: #1e1e2e; font-weight: bold; border-radius: 8px;")
    ...
    def _create_bottom_toolbar(self, parent_layout=None):
        ...
        toolbar.setStyleSheet(
            "#bottomToolbar { background-color: #1a1a28; border-top: 2px solid #89b4fa; }")
        ...
        def mk(text, slot, bg, name=""):
            ...
            b.setSizePolicy(QSizePolicy.Preferred, QSizePolicy.Fixed)
            b.setStyleSheet(
                f"QPushButton {{ background-color: {bg}; color: #1e1e2e; font-weight: bold; "
        ...
        lay.addWidget(mk("⚗️ فیلتر جامدات", self.action_filter, "#f9e2af"))
        ...
        self.btn_stirrer.setStyleSheet(
            "background-color: #cba6f7; color: #1e1e2e; font-weight: bold; border-radius: 8px;")
        def _stir():
        ...
        lay.addWidget(mk("📸 عکس", self.action_screenshot, "#6c7086"))
        lay.addWidget(mk("🚿 تعویض ظرف", self.action_wash, "#89dceb"))
        lay.addStretch(1)
        if HAS_OPENGL:
            lay.addWidget(mk("۳بعدی", self.toggle_beaker_view, "#a6e3a1"))
    ...
    def _create_center_panel(self):
        ...
        # دکمه با اندازه ثابت — تغییر متن باعث پرش layout نمی‌شود
        ...
        self.btn_toggle_beaker_view.setStyleSheet(
            "background-color: #a6e3a1; color: #1e1e2e; font-weight: bold; border-radius: 8px; font-size: 13px;")
        ...
        # هر دو حالت در یک محفظه با حداقل ارتفاع ثابت
        ...
        info_frame.setStyleSheet(
            "background-color: #1e1e2e; border-radius: 10px; border: 1px solid #313244;")
        ...
        self.lbl_ph_display.setStyleSheet("font-size: 16px; color: #a6e3a1; font-weight: bold;")
        ...
        self.lbl_stirrer_3d.setStyleSheet("font-size: 13px; color: #cba6f7; font-weight: bold;")
    ...
    def toggle_beaker_view(self):
        ...
        if self._beaker_is_3d:
            ...
            self.gl_beaker._needs_redraw = True
            self.gl_beaker.update()
            # روشن کردن تایمر انیمیشن ۳بعدی فقط در حالت ۳بعدی
            ...
            # خاموش کردن تایمر وقتی ۲بعدی هستیم — صرفه‌جویی CPU
    ...
    def _create_all_tabs(self):
        ...
        # Bohr tab removed for performance
        self.tabs.addTab(self.create_calc_tools_tab(), "🧮 ابزار محاسباتی")
        return self.tabs

    def create_calc_tools_tab(self):
        """تب ابزارهای محاسباتی مستقل از دیتابیس و فیزیک ظرف"""
        w = QWidget()
        ...
        scroll.setStyleSheet(
            ...
            "QScrollBar:vertical { background: #100a18; width: 12px; border-radius: 6px; }"
            "QScrollBar::handle:vertical { background: #5b3d7a; border-radius: 6px; min-height: 28px; }"
            "QScrollBar::handle:vertical:hover { background: #a78bfa; }"
        )
        host = QWidget()
        host.setStyleSheet("background-color: #100a18;")
        layout = QVBoxLayout(host)
        layout.setSpacing(14)

        _gb_css = """
            QGroupBox {
                font-weight: bold; font-size: 14px; color: #ddd6fe;
                border: 1px solid #5b3d7a; border-radius: 12px; margin-top: 14px;
                background-color: #1a1228; padding-top: 8px;
            }
            QGroupBox::title {
                subcontrol-origin: margin; subcontrol-position: top right;
                padding: 4px 14px; background-color: #2a1a40; color: #c4b5fd;
                border-radius: 8px; border: 1px solid #7c5cbf;
            }
            QLabel { color: #e8e0f5; font-size: 13px; background: transparent; }
            QLineEdit, QDoubleSpinBox, QComboBox {
                background-color: #160f22; border: 1px solid #5b3d7a; border-radius: 8px;
                padding: 6px 10px; color: #f0e8ff; font-size: 13px; min-height: 26px;
            }
            QLineEdit:focus, QDoubleSpinBox:focus, QComboBox:focus {
                border: 1px solid #a78bfa; background-color: #1e1530;
            }
            QPushButton {
                background-color: qlineargradient(x1:0,y1:0,x2:1,y2:0, stop:0 #7c3aed, stop:1 #a78bfa);
                color: #0c0914; font-weight: bold; border-radius: 8px; padding: 8px 16px;
                border: none; font-size: 13px; min-height: 30px;
            }
            QPushButton:hover {
                background-color: qlineargradient(x1:0,y1:0,x2:1,y2:0, stop:0 #8b5cf6, stop:1 #c4b5fd);
            }
            QPushButton:pressed { background-color: #5b21b6; color: #fff; }
        """
        _result_css = "color: #f0abfc; font-weight: bold; font-size: 15px; background: #221833; " \
                      "border-radius: 8px; padding: 8px 12px; border: 1px solid #7c5cbf;"

        def make_group(title):
            gb = QGroupBox(title)
            gb.setStyleSheet(_gb_css)
            return gb

        # ۱. تبدیل دما
        gb_t = make_group("🌡️ تبدیل واحد دما (°C ↔ K ↔ °F)")
        ft = QFormLayout(gb_t)
        self.calc_temp_val = QDoubleSpinBox()
        self.calc_temp_val.setRange(-500, 5000)
        self.calc_temp_val.setDecimals(2)
        self.calc_temp_val.setValue(25)
        self.calc_temp_from = QComboBox()
        self.calc_temp_from.addItems(["C", "K", "F"])
        self.calc_temp_to = QComboBox()
        self.calc_temp_to.addItems(["C", "K", "F"])
        self.calc_temp_to.setCurrentText("K")
        self.lbl_temp_result = QLabel("—")
        self.lbl_temp_result.setStyleSheet(_result_css)
        btn_t = QPushButton("تبدیل")
        def do_temp():
            r = ChemicalCalculator.convert_temperature(
                self.calc_temp_val.value(),
                self.calc_temp_from.currentText(),
                self.calc_temp_to.currentText())
            self.lbl_temp_result.setText(f"{r:.4g} {self.calc_temp_to.currentText()}" if r is not None else "خطا")
        btn_t.clicked.connect(do_temp)
        ft.addRow("مقدار:", self.calc_temp_val)
        ft.addRow("از:", self.calc_temp_from)
        ft.addRow("به:", self.calc_temp_to)
        ft.addRow(btn_t)
        ft.addRow("نتیجه:", self.lbl_temp_result)
        layout.addWidget(gb_t)

        # ۲. تبدیل انرژی
        gb_e = make_group("⚡ تبدیل واحد انرژی (kJ ↔ kcal ↔ eV)")
        fe = QFormLayout(gb_e)
        self.calc_e_val = QDoubleSpinBox()
        self.calc_e_val.setRange(-1e12, 1e12)
        self.calc_e_val.setDecimals(6)
        self.calc_e_val.setValue(1)
        self.calc_e_from = QComboBox()
        self.calc_e_from.addItems(["kJ", "kcal", "eV"])
        self.calc_e_to = QComboBox()
        self.calc_e_to.addItems(["kJ", "kcal", "eV"])
        self.calc_e_to.setCurrentText("kcal")
        self.lbl_e_result = QLabel("—")
        self.lbl_e_result.setStyleSheet(_result_css)
        btn_e = QPushButton("تبدیل")
        def do_e():
            r = ChemicalCalculator.convert_energy(
                self.calc_e_val.value(),
                self.calc_e_from.currentText(),
                self.calc_e_to.currentText())
            self.lbl_e_result.setText(f"{r:.6g} {self.calc_e_to.currentText()}" if r is not None else "خطا")
        btn_e.clicked.connect(do_e)
        fe.addRow("مقدار:", self.calc_e_val)
        fe.addRow("از:", self.calc_e_from)
        fe.addRow("به:", self.calc_e_to)
        fe.addRow(btn_e)
        fe.addRow("نتیجه:", self.lbl_e_result)
        layout.addWidget(gb_e)

        # ۳. گاز ایده‌آل
        gb_g = make_group("🎈 گاز ایده‌آل  PV = nRT  → حجم (لیتر)")
        fg = QFormLayout(gb_g)
        self.calc_n = QDoubleSpinBox()
        self.calc_n.setRange(0, 1e6)
        self.calc_n.setDecimals(4)
        self.calc_n.setValue(1)
        self.calc_T = QDoubleSpinBox()
        self.calc_T.setRange(-273, 5000)
        self.calc_T.setValue(25)
        self.calc_T.setSuffix(" °C")
        self.calc_P = QDoubleSpinBox()
        self.calc_P.setRange(0.001, 1e6)
        self.calc_P.setDecimals(4)
        self.calc_P.setValue(1)
        self.calc_P.setSuffix(" atm")
        self.lbl_gas_result = QLabel("—")
        self.lbl_gas_result.setStyleSheet(_result_css)
        btn_g = QPushButton("محاسبه حجم")
        def do_gas():
            v = ChemicalCalculator.ideal_gas_volume(self.calc_n.value(), self.calc_T.value(), self.calc_P.value())
            self.lbl_gas_result.setText(f"V = {v:.4g} L" if v is not None else "خطا (P یا n نامعتبر)")
        btn_g.clicked.connect(do_gas)
        fg.addRow("n (مول):", self.calc_n)
        fg.addRow("T:", self.calc_T)
        fg.addRow("P:", self.calc_P)
        fg.addRow(btn_g)
        fg.addRow("نتیجه:", self.lbl_gas_result)
        layout.addWidget(gb_g)

        # ۴. درصد ترکیب عنصری
        gb_p = make_group("📊 درصد ترکیب عنصری از فرمول")
        fp = QFormLayout(gb_p)
        self.calc_formula = QLineEdit()
        self.calc_formula.setPlaceholderText("مثلاً H2SO4 یا C6H12O6")
        self.lbl_pct_result = QLabel("—")
        self.lbl_pct_result.setWordWrap(True)
        self.lbl_pct_result.setStyleSheet(_result_css)
        btn_p = QPushButton("محاسبه درصد")
        def do_pct():
            pcts = ChemicalCalculator.elemental_mass_percent(self.calc_formula.text())
            if not pcts:
                self.lbl_pct_result.setText("فرمول نامعتبر")
            else:
                parts = [f"{el}: {p:.2f}%" for el, p in sorted(pcts.items())]
                self.lbl_pct_result.setText("  |  ".join(parts))
        btn_p.clicked.connect(do_pct)
        fp.addRow("فرمول:", self.calc_formula)
        fp.addRow(btn_p)
        fp.addRow("نتیجه:", self.lbl_pct_result)
        layout.addWidget(gb_p)

        # ۵. فرمول تجربی از درصد
        gb_emp = make_group("🧪 فرمول تجربی از درصد جرمی")
        femp = QVBoxLayout(gb_emp)
        self.calc_emp_input = QLineEdit()
        self.calc_emp_input.setPlaceholderText("مثلاً C:40 H:6.7 O:53.3")
        self.lbl_emp_result = QLabel("—")
        self.lbl_emp_result.setStyleSheet(_result_css)
        btn_emp = QPushButton("محاسبه فرمول تجربی")
        def do_emp():
            text = self.calc_emp_input.text().strip()
            pct = {}
            for part in re.split(r'[,\s]+', text):
                if ':' in part:
                    ...
                    el = el.strip()
                    try:
                        pct[el] = float(val.strip())
                    except ValueError:
                        pass
            formula = ChemicalCalculator.empirical_from_percentages(pct)
            self.lbl_emp_result.setText(formula or "—")
        btn_emp.clicked.connect(do_emp)
        femp.addWidget(QLabel("درصدها (عنصر:درصد جدا شده با فاصله یا ویرگول):"))
        femp.addWidget(self.calc_emp_input)
        femp.addWidget(btn_emp)
        femp.addWidget(self.lbl_emp_result)
        layout.addWidget(gb_emp)

        # ۶. استوکیومتری ساده
        gb_st = make_group("⚖️ استوکیومتری ساده (جرم → جرم)")
        fst = QFormLayout(gb_st)
        self.calc_st_rxn = QLineEdit()
        self.calc_st_rxn.setPlaceholderText("مثلاً 2H2 + O2 → 2H2O")
        self.calc_st_known = QLineEdit()
        self.calc_st_known.setPlaceholderText("نماد ماده معلوم (مثلاً H2)")
        self.calc_st_mass = QDoubleSpinBox()
        self.calc_st_mass.setRange(0.001, 1e9)
        self.calc_st_mass.setDecimals(4)
        self.calc_st_mass.setValue(2.0)
        self.lbl_st_result = QLabel("—")
        self.lbl_st_result.setWordWrap(True)
        self.lbl_st_result.setStyleSheet(_result_css)
        btn_st = QPushButton("محاسبه جرم‌ها")
        def do_st():
            rxn = self.calc_st_rxn.text().strip()
            if '→' in rxn:
                left, right = rxn.split('→', 1)
            elif '->' in rxn:
                left, right = rxn.split('->', 1)
            else:
                self.lbl_st_result.setText("از → یا -> استفاده کنید")
                return
            def parse_side(side):
                coeffs = {}
                ...
                return coeffs
            r_c = parse_side(left)
            p_c = parse_side(right)
            known = self.calc_st_known.text().strip()
            res = ChemicalCalculator.simple_stoichiometry(r_c, p_c, known, self.calc_st_mass.value())
            if not res:
                self.lbl_st_result.setText("خطا در ورودی")
            else:
                parts = [f"{k}: {v:.4g} g" for k, v in res.items()]
                self.lbl_st_result.setText("  |  ".join(parts))
        btn_st.clicked.connect(do_st)
        fst.addRow("واکنش:", self.calc_st_rxn)
        fst.addRow("ماده معلوم:", self.calc_st_known)
        fst.addRow("جرم معلوم (g):", self.calc_st_mass)
        fst.addRow(btn_st)
        fst.addRow("نتیجه:", self.lbl_st_result)
        layout.addWidget(gb_st)

        # ۷. خواص کولیگاتیو
        gb_col = make_group("❄️ خواص کولیگاتیو (افزایش جوش / کاهش انجماد آب)")
        fcol = QFormLayout(gb_col)
        self.calc_molal = QDoubleSpinBox()
        self.calc_molal.setRange(0, 50)
        self.calc_molal.setDecimals(3)
        self.calc_molal.setValue(1.0)
        self.calc_i = QDoubleSpinBox()
        self.calc_i.setRange(1, 10)
        self.calc_i.setDecimals(2)
        self.calc_i.setValue(1.0)
        self.lbl_col_result = QLabel("—")
        self.lbl_col_result.setStyleSheet(_result_css)
        btn_col = QPushButton("محاسبه")
        def do_col():
            dtb, dtf = ChemicalCalculator.colligative_delta(
                self.calc_molal.value(), self.calc_i.value())
            if dtb is None:
                self.lbl_col_result.setText("خطا")
            else:
                self.lbl_col_result.setText(
                    f"ΔTb = +{dtb:.3f} °C  |  ΔTf = −{dtf:.3f} °C  "
                    f"(نقطه جوش ≈ {100+dtb:.2f}°C ، انجماد ≈ {0-dtf:.2f}°C)")
        btn_col.clicked.connect(do_col)
        fcol.addRow("مولالیته (m):", self.calc_molal)
        fcol.addRow("ضریب وانت‌هوف (i):", self.calc_i)
        fcol.addRow(btn_col)
        fcol.addRow("نتیجه:", self.lbl_col_result)
    ...
    def create_contents_tab(self):
        ...
        btn_mass.setStyleSheet("background-color: #89b4fa; color: #1e1e2e; font-weight: bold; padding: 6px 12px;")
    ...
    def create_graph_tab(self):
        ...
        self.line_ph, = self.ax1.plot([], [], color='#a6e3a1', linewidth=2, label='pH')
        self.line_temp, = self.ax2.plot([], [], color='#f38ba8', linewidth=2)
        # خط نقطه هم‌ارزی تقریبی (pH=7) — فقط هنگام تیتراسیون نمایش داده می‌شود
        self.eq_line = self.ax1.axhline(y=7.0, color='#f9e2af', linestyle='--', linewidth=1.5,
                                        alpha=0.0, label='نقطه هم‌ارزی ≈۷')
        try:
            self.ax1.legend(loc='upper right', fontsize=8, facecolor='#1e1e2e', edgecolor='#45475a', labelcolor='white')
        except Exception:
            pass
    ...
    def update_contents_ui(self):
        ...
        # فرمول: اگر تک‌ماده مولکولی، وگرنه تجربی
    ...
    def update_discoveries_table(self):
        ...
        self.table_disc.setStyleSheet(
            "QTableWidget { background-color: #11111b; gridline-color: #313244; color: #cdd6f4; "
            "border: 1px solid #313244; border-radius: 10px; font-size: 13px; }"
            "QHeaderView::section { background-color: #1e1e2e; color: #f9e2af; font-weight: bold; padding: 8px; }"
    ...
    def toggle_theme(self):
        tlog("toggle_theme", "CLICK", dark=not self.is_dark_mode)
        self.is_dark_mode = not self.is_dark_mode
        # ذخیره اندازه اسپلیتر تا بعد از تغییر استایل نپرد
        sizes = self.main_splitter.sizes() if hasattr(self, 'main_splitter') else None
    ...
    def resizeEvent(self, event):
        ...
        try:
            ...
            # --- نوار پایین را مجبور کن داخل پنجره بماند ---
            if tb is not None:
                tb.setFixedHeight(58)
                tb.setVisible(True)
                tb.show()
                # اگر layout جا نداده، هندسه را اصلاح نکن — فقط activate
                if self.centralWidget() and self.centralWidget().layout():
                    self.centralWidget().layout().activate()

            # --- اسپلیتر: هیچ پنلی صفر نباشد ---
            ...
            # لاگ resize حذف شد — پرتکرار و بی‌فایده برای عملکرد
        except Exception as e:
    ...
    def game_loop(self):
        try:
            ...
            # تشخیص نقطه هم‌ارزی با هیسترزیس
            ...
            # چک واکنش — نه هر فریم (کاهش CPU شدید)
            if fi % 5 == 0:
            ...
            # نمودار: فقط وقتی تب نمودار فعال است یا هر ~0.5s
            graph_tab_active = False
            try:
                if hasattr(self, 'tabs') and self.tabs.currentIndex() >= 0:
                    # تب نمودار معمولاً یکی از تب‌هاست؛ همیشه سبک بکشیم ولی کمتر
            ...
            if self.engine.speed_multiplier > 0 and fi % 4 == 0:
                ...
                # draw فقط وقتی واقعاً لازم است
                if graph_tab_active and fi % 8 == 0:
                    try:
                        self.canvas.draw_idle()
                    except Exception:
                        pass
...
if __name__ == '__main__':
    # در Qt6 این attributeها به‌صورت پیش‌فرض فعال و deprecated هستند — عمداً تنظیم نمی‌شوند
