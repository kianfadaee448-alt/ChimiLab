...
class LabEngine:
    def __init__(self):
        ...
        self.discovered = set()
        self.notes = ""  # اضافه شدن متغیر برای یادداشت‌ها
        self.visual_layers = []
    ...
    def load_data(self):
        ...
        if os.path.exists(save_path):
            try:
                # اضافه شدن انکودینگ utf-8 برای پشتیبانی کامل از فارسی
                with open(save_path, "r", encoding="utf-8") as f:
                    ...
                    self.notes = d.get("notes", "")  # لود کردن یادداشت
            except:
                pass
        ...
        try:
            with open(save_path, "w", encoding="utf-8") as f:
                json.dump({
                    ...
                    "notes": self.notes  # ذخیره یادداشت
    ...
    def add_chemical(self, key, amount):
        ...
        # ---- اصلاح علمی: محاسبه دقیق ترمودینامیک و دمای اختلاط ----
        old_vol = self.total_volume
        self.total_volume += amount

        if self.total_volume > 0:
            # ۱. محاسبه دمای میانگین (فرض بر این است که ماده جدید دمای محیط یعنی 25 درجه دارد)
            self.temp_c = ((old_vol * self.temp_c) + (amount * 25.0)) / self.total_volume

            # ۲. محاسبه تغییر دما ناشی از گرمای انحلال/واکنش (آنتالپی)
            # مقدار heat در دیتابیس آنتالپی است. مقادیر منفی یعنی گرمازا (دما بالا می‌رود)
            # فرمول Q = mcΔT
            # ظرفیت گرمایی ویژه آب تقریبا 4.18 J/g°C است.
            q_joules = added_moles * (-heat) * 1000
            mass_approx = self.total_volume  # فرض بر اینکه چگالی محلول نزدیک به آب 1g/mL است
            if mass_approx > 0:
            ...
            if len(needed) > 0 and needed.issubset(present):
                ...
                if self.temp_c >= req_temp:
                    if name not in self.discovered:
                        ...
                        self.save_data()
                        # اصلاح علمی: واکنش ها گرمازا هستند، اما برای اینکه بازی متعادل بماند
                        # یک شوک گرمایی 15 درجه‌ای معقول ایجاد می‌کنیم
                        self.temp_c += 15.0
...
class ModernLabWindow(QMainWindow):
    ...
    def create_notes_tab(self):
        ...
        lbl_hint = QLabel("📝 دفترچه یادداشت آزمایشگاه:")
        lbl_hint.setStyleSheet("font-size: 16px; font-weight: bold; color: #cdd6f4;")
        l.addWidget(lbl_hint)

        self.txt_notes = QTextEdit()
        # استایل ویژه برای خوانایی بسیار بالا (بک گراند سفید، نوشته مشکی و فونت درشت)
        ...
        self.txt_notes.setPlaceholderText(
            "یادداشت‌ها، فرمول‌ها و مشاهدات علمی خود را با خیالی آسوده اینجا بنویسید... (این متن به صورت خودکار ذخیره می‌شود)")
