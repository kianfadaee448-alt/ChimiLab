...
# برای جلوگیری از کرش‌های احتمالی مت پلات لیب در PyQt
...
try:
    ...
    print(f"Error: {e}")
    print("Please install required libraries: pip install PyQt5 matplotlib molmass")
    sys.exit(0)

# ----------------- تنظیمات مسیر و دیتابیس -----------------
CUSTOM_REACTIONS = {}
CHEMILAB_DB = {}


def get_save_path():
    """ مسیر ذخیره فایل جیسون دقیقاً در کنار فایل اجرایی اصلی (جلوگیری از حذف شدن در پوشه موقت) """
...
def get_db_path():
    """ یافتن یا استخراج دیتابیس در کنار فایل اجرایی """
    if getattr(sys, 'frozen', False):
        base_dir = os.path.dirname(sys.executable)
    else:
        base_dir = os.path.abspath(".")
    ...
    # اگر دیتابیس در کنار فایل اجرایی نبود، آن را از داخل فایل موقت به بیرون کپی کن
    if not os.path.exists(local_db) and hasattr(sys, '_MEIPASS'):
        ...
        if os.path.exists(bundled_db):
            try:
                ...
                print(f"Could not extract db: {e}")
                return bundled_db  # در صورت عدم دسترسی، از همان مسیر موقت بخوان
...
def load_databases():
    """ لود کردن ایمن دیتابیس با اصلاح کلیدها و لیست‌ها """
    db_path = get_db_path()
    if not os.path.exists(db_path):
        print(f"❌ فایل دیتابیس یافت نشد: {db_path}")
        return  # ادامه می‌دهیم تا حداقل محیط باز شود، اما خالی خواهد بود

    try:
        connection = sqlite3.connect(db_path)
        cursor = connection.cursor()

        # تابع کمکی برای استخراج درست لیست‌ها حتی اگر کوتیشن اشتباه داشته باشند
        ...
        for i in cursor.fetchall():
            # نرمال سازی دقیق کلیدها هنگام ورود به مموری
            fa_name = normalize_key(str(i[2]))

            # جلوگیری صد در صدی از کرش‌های TypeError (با تبدیل حتمی به Float)
            try:
                ph_val = float(i[4])
            except:
                ph_val = 7.0
...
class LabEngine:
    ...
    def add_chemical(self, key, amount):
        ...
        # گرفتن مقادیر با اطمینان از تبدیل درست
    ...
    def check_reactions(self):
        # موادی که به مقدار کافی در ظرف وجود دارند (مقادیر بسیار ناچیز نادیده گرفته می‌شوند)
        present = {normalize_key(k) for k, v in self.contents.items() if v > 1e-5}

        found_old = None
        for name, rxn in CUSTOM_REACTIONS.items():
            needed = {normalize_key(r) for r in rxn["reactants"]}

            if len(needed) > 0 and needed == present:
...
class ModernLabWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        # پنل چپ (تب‌ها)
        tabs = QTabWidget()
        tabs.addTab(self.create_about_tab(), "ℹ️ درباره برنامه")
...
طراح و برنامه‌نویس: کیانوش فدائی (دانش‌آموز کلاس نهم، مدرسه شهید اسدزاده)"""
    ...
    def get_state_color_text(self, formula):
        # حذف کاراکترهای مزاحم که ممکن است هنگام پارس کردن از دیتابیس جا مانده باشند
        ...
        if norm in CHEMILAB_DB:
            ...
            # جستجوی هوشمند در صورت تطابق نداشتن کلید (بررسی فرمول و نام)
    ...
    def action_add(self):
        try:
            ...
            self.txt_log.append(f"❌ خطای غیرمنتظره در اضافه کردن: {e}")
            print(f"Error in action_add: {e}")
    ...
    def game_loop(self):
        try:
            ...
            # جلوگیری از افت فریم و کرش کردن نمودار با کشیدن آن در زمان استراحت UI (نه در هر لوپ)
