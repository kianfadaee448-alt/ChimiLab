...
class ChemicalCalculator:
    ...
    def parse_formula(formula, merge_isotopes=False):
        ...
        if not s:
            return Counter()
        # ورودی‌های دارای HTML/SQL/کاراکترهای تصادفی را نپذیر.
        # بار یونی را قبل از پارس حذف کن؛ هم SO4^2- و SO4²⁻ و هم SO42- را پوشش می‌دهد.
        # نکته: فرمول‌های گروهی مثل (NH4)2SO4 مجازند، بنابراین شروع با '(' معتبر است.
        # توان‌های یونیکد را به رقم معمولی تبدیل کن: SO4²⁻ → SO42-
        s = s.translate(str.maketrans({"⁰":"0","¹":"1","²":"2","³":"3","⁴":"4","⁵":"5","⁶":"6","⁷":"7","⁸":"8","⁹":"9","⁺":"+","⁻":"-"}))
        # حذف بار یونی انتهایی با توجه به ابهام نگارشی فرمول‌ها.
        # مثال‌های مهم: SO42- → SO4 ، SO4^2- → SO4 ، SO4²⁻ → SO4 ،
        # NH4+ → NH4 و Fe3+ → Fe.
        def _strip_ionic_charge(text):
            ...
            if re.search(r"[+-]$", text):
                ...
                # چند رقم پایانی مثل SO42-/PO43-: رقم آخر بار است.
                if len(digits) >= 2:
                    return base + digits[:-1]
                # تک‌رقمی: اگر قبل از آن یک نماد عنصرِ تنهاست، بار است؛
                # در فرمول‌های چندعنصری مثل NH4+ رقم، ضریب اتم است.
                if re.fullmatch(r"[A-Z][a-z]?", base):
                    return base
                return core
            return text

        s = _strip_ionic_charge(s)
        if not s or not re.fullmatch(r"[A-Za-z0-9().]+", s) or not (s[0].isupper() or s[0] == "("):
            return Counter()
        try:
        ...
        try:
            result = parse_group()
            # پارسر باید کل رشته را مصرف کرده باشد؛ فرمول ناقص/بدساختار قبول نشود.
            if i != nlen:
                return Counter()
            if result:
        ...
        # اگر parser گروهی موفق نشد، فرمول را حدسی و ناقص parse نکن.
        # این کار از جرم مولی جعلی برای فرمول‌های بدساختار جلوگیری می‌کند.
        return Counter()
    ...
    def molar_mass(formula):
        ...
        if not s:
            return 0.0
        # همان نرمال‌سازی بار یونیِ parser برای molmass هم اعمال شود.
        s = s.translate(str.maketrans({"⁰":"0","¹":"1","²":"2","³":"3","⁴":"4","⁵":"5","⁶":"6","⁷":"7","⁸":"8","⁹":"9","⁺":"+","⁻":"-"}))
        if re.search(r"\^\d*[+-]$", s):
            s = re.sub(r"\^\d*[+-]$", "", s)
        elif re.search(r"[+-]$", s):
            ...
            if m:
                ...
                if len(digits) >= 2:
                    s = base + digits[:-1]
                elif re.fullmatch(r"[A-Z][a-z]?", base):
                    s = base
                else:
                    s = core
            else:
                s = core
        cache_key = s
        cached = ChemicalCalculator._molar_mass_cache.get(cache_key)
        if cached is not None:
            return cached
        if HAS_MOLMASS and Formula is not None:
            try:
                value = float(Formula(s).mass)
                ChemicalCalculator._molar_mass_cache[cache_key] = value
                return value
            except Exception:
                pass
        total = 0.0
        ...
        ChemicalCalculator._molar_mass_cache[cache_key] = total
        return total
    ...
    def reduce_slots(atom_amounts):
        """
        تبدیل مقدار مولی عناصر به ساده‌ترین نسبت صحیح با تقریب کسری پایدار.

        الگوریتم قبلی فقط چند ضریب ثابت را امتحان می‌کرد و با گرد کردن همزمان
        نسبت‌ها می‌توانست نسبت‌های غلط بسازد (مثلاً برای 1 : 1.5). این نسخه
        هر نسبت را به کسر محدود تبدیل می‌کند، مخرج مشترک می‌گیرد و در پایان
        بزرگ‌ترین مقسوم‌علیه مشترک را حذف می‌کند.
        """
        if not atom_amounts:
            return {}

        cleaned = []
        for element, value in atom_amounts.items():
            try:
                fv = float(value)
            except (TypeError, ValueError):
                continue
            if np.isfinite(fv) and fv > 1e-12:
                cleaned.append((str(element), fv))
        if not cleaned:
            return {}

        base = min(v for _, v in cleaned)
        if base <= 0:
            return {}

        # مخرج 24 برای داده‌های آزمایشگاهی اعشاری کافی است و جلوی
        # تولید فرمول‌های غیرواقعی با ضرایب صدها را می‌گیرد.
        fractions = []
        for _, value in cleaned:
            ratio = value / base
            if abs(ratio - round(ratio)) < 0.015:
                frac = Fraction(int(round(ratio)), 1)
            else:
        ...
        for frac in fractions:
            scale = _lcm(scale, frac.denominator)
            if scale > 240:
                break

        if 0 < scale <= 240:
            nums = [int(frac.numerator * (scale // frac.denominator)) for frac in fractions]
        else:
            # fallback برای نسبت‌های دشوار: نزدیک‌ترین ضرایب کوچک را پیدا کن.
            ratios = np.asarray([v / base for _, v in cleaned], dtype=float)
            best = None
            for mult in range(1, 61):
                cand = np.maximum(1, np.rint(ratios * mult)).astype(int)
                rel_err = np.abs(cand / mult - ratios) / np.maximum(ratios, 1e-12)
                score = float(np.sum(rel_err))
                if best is None or score < best[0]:
                    best = (score, cand.tolist())
            nums = best[1] if best else [1] * len(cleaned)

        common = 0
        for n in nums:
            common = gcd(common, int(n))
        if common > 1:
            nums = [int(n // common) for n in nums]

        return {cleaned[i][0]: max(1, int(nums[i])) for i in range(len(cleaned))}
...
class LabEngine:
    def __init__(self):
        ...
        self._ph_cache_key = None
        self._ph_cache_value = 7.0
    ...
    def get_ph(self):
        if self.total_volume <= 0 or self.is_broken:
            self._ph_cache_key = (0.0, bool(self.is_broken), ())
            self._ph_cache_value = 7.0
            return 7.0
        try:
            cache_items = tuple((str(k), round(float(v), 9)) for k, v in self.contents.items() if float(v) > 1e-12)
        except Exception:
            cache_items = ()
        cache_key = (round(float(self.total_volume), 9), bool(self.is_broken), cache_items)
        if cache_key == getattr(self, "_ph_cache_key", None):
            return float(getattr(self, "_ph_cache_value", 7.0))
        try:
            self.rebuild_acid_base_moles()
        except Exception:
            pass
        ...
        try:
            ...
            result = float(np.clip(ph, 0.0, 14.0))
            self._ph_cache_key = cache_key
            self._ph_cache_value = result
            return result
        except Exception:
            self._ph_cache_key = cache_key
            self._ph_cache_value = 7.0
...
class MolecularStructureDB:
    ...
    def from_engine_contents(cls, contents, db):
        """سازگاری قدیمی: اولین مولکول شناخته‌شده از محتویات ظرف."""
        items = cls.from_engine_contents_all(contents, db)
        if items:
            return items[0][0], items[0][1]
        return None, None

    @classmethod
    def from_engine_contents_all(cls, contents, db):
        """تمام مواد فعال ظرف را به صورت فهرست ساختارهای ۳بعدی برمی‌گرداند."""
        result = []
        if not contents:
            return result
        for key, moles in contents.items():
            ...
            if amount <= 1e-12:
                continue
            data = db.get(key, {}) if db else {}
            if not data and db:
                nk = normalize_key(key)
                for _dk, _dv in db.items():
                    if normalize_key(_dk) == nk:
                        data = _dv
                        break
            form = str(data.get("formula") or key).strip()
            name = str(data.get("name") or form)
            structure = cls.get(form)
            if not structure:
                try:
                    structure = cls.synthetic(form)
                except Exception:
                    structure = None
            if structure:
                result.append((form, structure, name, amount))
        return result

    @classmethod
    def combine_structures(cls, items):
        """چند مولکول مستقل را در یک صحنه با فاصله مناسب کنار هم می‌چیند."""
        if not items:
            return None
        combined_atoms = []
        combined_bonds = []
        labels = []
        offsets = [(-3.0, 0.0, 0.0), (3.0, 0.0, 0.0),
                   (0.0, 2.8, 0.0), (0.0, -2.8, 0.0),
                   (-3.0, 2.8, 0.0), (3.0, 2.8, 0.0)]
        for idx, item in enumerate(items):
            form, structure, name, amount = item
            ox, oy, oz = offsets[idx % len(offsets)]
            if idx >= len(offsets):
                ring = idx // len(offsets)
                ox *= 1.0 + ring * 0.35
                oy *= 1.0 + ring * 0.35
            raw_atoms = structure.get("atoms") or []
            base_index = len(combined_atoms)
            pts = []
            for atom in raw_atoms:
                if isinstance(atom, dict):
                    el = str(atom.get("el") or atom.get("element") or atom.get("symbol") or "C")
                    x, y, z = float(atom.get("x", 0) or 0), float(atom.get("y", 0) or 0), float(atom.get("z", 0) or 0)
                else:
                    el, x, y, z = str(atom[0]), float(atom[1]), float(atom[2]), float(atom[3])
                pts.append((el, x + ox, y + oy, z + oz))
            combined_atoms.extend(pts)
            for bond in structure.get("bonds") or []:
                try:
                    if isinstance(bond, dict):
                        bi, bj, order = int(bond.get("i", 0)), int(bond.get("j", 1)), bond.get("order", 1)
                    else:
                        bi, bj = int(bond[0]), int(bond[1])
                        order = bond[2] if len(bond) > 2 else 1
                    combined_bonds.append((base_index + bi, base_index + bj, order))
                except Exception:
                    continue
            labels.append(f"{name} ({form}) × {amount:.3g} mol")
        return {
            "name": "مخلوط چندماده‌ای",
...
class BohrModelWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        v_right.setContentsMargins(10, 10, 10, 10)
        v_right.setSpacing(10)

        # پنل اطلاعات به‌صورت کارت‌های بصری: عنوان، مقدار و بخش‌بندی روشن.
        card_qss = """
        QGroupBox {
            background: #171022;
        ...
        QListWidget {
            background: #100a18; border: 1px solid #352544; border-radius: 9px;
            padding: 5px; color: #ddd6fe;
        }
        QListWidget::item { padding: 6px; border-radius: 6px; }
        QListWidget::item:selected { background: #4c1d95; color: white; }
        """
        right_content.setStyleSheet("background: #0f0917;" + card_qss)
        ...
        lbl_style = (
            ...
            "background: #21162e; border: 1px solid #352544; "
...
class ModernLabWindow(QMainWindow):
    ...
    def _create_left_panel(self):
        ...
        outer = QFrame()
        outer.setObjectName("leftControlPanel")
        outer.setStyleSheet("QFrame#leftControlPanel { background:#0e0a16; border-right:1px solid #2e2040; }")
        DimensionController.lock(outer, min_w=350)
        outer.setMinimumWidth(350)
        ...
        panel = QWidget()
        panel.setStyleSheet("QGroupBox { background:#120c1c; border:1px solid #342448; border-radius:11px; margin-top:10px; padding:9px 8px 8px 8px; } QGroupBox::title { subcontrol-origin:margin; right:11px; padding:3px 8px; color:#efe7ff; background:#241536; border:1px solid #5b3a78; border-radius:7px; font-size:12px; font-weight:700; } QLabel { background:transparent; color:#cbd0e6; } QDoubleSpinBox, QComboBox, QLineEdit, QSlider { min-height:30px; }")
        vbox = QVBoxLayout(panel)
        vbox.setSpacing(7)
        vbox.setContentsMargins(9, 9, 9, 9)

        panel_title = QLabel("🧪 کنترل آزمایشگاه")
        panel_title.setAlignment(Qt.AlignCenter)
        panel_title.setStyleSheet("font-size:16px;font-weight:800;color:#c4b5fd;background:#160f22;border:1px solid #3d2a55;border-radius:10px;padding:9px;")
        vbox.addWidget(panel_title)
        ...
        self.lbl_chem_count = QLabel(f"مواد قابل انتخاب: {self.combo_chem.count():,}")
        self.lbl_chem_count.setStyleSheet("color:#8191a8;font-size:10px;padding:1px 3px;background:transparent;")
        self.lbl_chem_count.setAlignment(Qt.AlignRight)
        frm.addRow(self.lbl_chem_count)
        ...
        mol_menu.addAction("🧪 همه مواد فعال ظرف", self.show_mixture_molecule_3d)
    ...
    def create_contents_tab(self):
        ...
        btn_mix3d.setToolTip("نمایش سه‌بعدی همه مواد فعال ظرف")
    ...
    def _refresh_graph(self):
        """بازرسم نمودار زنده؛ در صورت نصب pyqtgraph از مسیر سریع Qt استفاده می‌شود."""
        if HAS_PYQTGRAPH and hasattr(self, "pg_ph_curve"):
            if not self.data_time:
                self.pg_ph_plot.setXRange(0, 10, padding=0)
                self.pg_temp_plot.setXRange(0, 10, padding=0)
                self.pg_ph_curve.setData([], [])
                self.pg_temp_curve.setData([], [])
                return
            x = np.asarray(self.data_time, dtype=np.float64)
            ph = np.asarray(self.data_ph, dtype=np.float64)
            temp = np.asarray(self.data_temp, dtype=np.float64)
            hi = max(float(x[-1]), float(x[0]) + 1.0)
            xpad = max(0.5, (hi - float(x[0])) * 0.025)
            for plot in (self.pg_ph_plot, self.pg_temp_plot):
                plot.setXRange(float(x[0]), hi + xpad, padding=0)
            self.pg_ph_plot.setYRange(0, 14, padding=0)
            if temp.size:
                tlo = float(np.nanmin(temp)) - 3.0
                thi = float(np.nanmax(temp)) + 3.0
                if thi - tlo < 10.0:
                    mid = (thi + tlo) / 2.0
                    tlo, thi = mid - 5.0, mid + 5.0
                self.pg_temp_plot.setYRange(tlo, thi, padding=0)
            self.pg_ph_curve.setData(x, ph)
            self.pg_temp_curve.setData(x, temp)
            self.pg_eq_line.setVisible(bool(self.btn_titrate.isChecked()))
            return
        ...
        try:
            ...
            self.ax1.set_ylim(0, 14)
            temps = np.asarray(self.data_temp, dtype=float)
            if temps.size:
                tlo = float(np.nanmin(temps)) - 3.0
                thi = float(np.nanmax(temps)) + 3.0
                if thi - tlo < 10.0:
                    mid = (thi + tlo) / 2.0
                    tlo, thi = mid - 5.0, mid + 5.0
    ...
    def create_graph_tab(self):
        ...
        header = QFrame()
        header.setStyleSheet("QFrame { background:#100a18; border:1px solid #2e2040; border-radius:10px; } QLabel { border:none; }")
        ...
        if HAS_PYQTGRAPH:
            ...
            host_l.setSpacing(6)
            self.pg_ph_plot = pg.PlotWidget(background="#120e1c")
            self.pg_temp_plot = pg.PlotWidget(background="#120e1c")
            for plot in (self.pg_ph_plot, self.pg_temp_plot):
                plot.showGrid(x=True, y=True, alpha=0.14)
                plot.setMenuEnabled(False)
                plot.setMouseEnabled(x=True, y=True)
                plot.getPlotItem().setClipToView(True)
            self.pg_ph_plot.setLabel("left", "pH")
            self.pg_ph_plot.setLabel("bottom", "زمان (s)")
            self.pg_temp_plot.setLabel("left", "دما (°C)")
            self.pg_temp_plot.setLabel("bottom", "زمان (s)")
            self.pg_ph_curve = self.pg_ph_plot.plot(pen=pg.mkPen("#c4b5fd", width=1))
            self.pg_temp_curve = self.pg_temp_plot.plot(pen=pg.mkPen("#f38ba8", width=1))
            self.pg_eq_line = pg.InfiniteLine(pos=7.0, angle=0, pen=pg.mkPen("#f9e2af", width=1, style=Qt.DashLine))
            self.pg_ph_plot.addItem(self.pg_eq_line)
            self.pg_eq_line.setVisible(False)
            # subsample keeps live plotting cheap even if the history grows later.
            self.pg_ph_curve.setDownsampling(auto=True)
            self.pg_temp_curve.setDownsampling(auto=True)
            host_l.addWidget(self.pg_ph_plot, 1)
            host_l.addWidget(self.pg_temp_plot, 1)
            self.graph_canvas = host
            self.canvas = host
            l.addWidget(host, 1)
        else:
            ...
            self.line_ph, = self.ax1.plot([], [], linewidth=1.0, antialiased=False, label="pH")
            self.line_temp, = self.ax2.plot([], [], linewidth=1.0, antialiased=False, label="دما")
            ...
            for name, rxn in sorted_rxns:
                ...
                if products:
                    ...
                    for p in products:
                        ...
                        p3d = QPushButton("🧬 ۳بعدی")
                        p3d.setToolTip("نمایش ساختار سه‌بعدی این ماده")
                        p3d.setStyleSheet("background:#89b4fa; color:#160f22; border-radius:6px; padding:5px 9px; font-weight:bold;")
                        p3d.clicked.connect(lambda checked, pk=p: self._show_wiki_product_3d(pk))
                        prod_layout.addWidget(p3d)
    ...
    def _show_wiki_product_3d(self, product_key):
        """ساختار سه‌بعدی محصول کشف‌شده در دانشنامه."""
        data = CHEMILAB_DB.get(product_key, {})
        formula = data.get("formula") or product_key
        name = data.get("name") or product_key
    ...
    def show_mixture_molecule_3d(self):
        """نمایش ایمن و همزمان تمام مواد فعال ظرف؛ خطای یک ساختار نباید کل نما را از کار بیندازد."""
        try:
            contents = getattr(self.engine, "contents", None) or {}
            items = MolecularStructureDB.from_engine_contents_all(contents, CHEMILAB_DB)
            if not items:
                QMessageBox.information(self, "۳بعدی مخلوط", "ظرف خالی است یا ساختار ۳بعدی مواد موجود نیست.")
                return
            if len(items) == 1:
                form, _st, name, amount = items[0]
                self.open_molecule_3d(form, f"{name} — {amount:.3g} mol")
                return

            combined = MolecularStructureDB.combine_structures(items)
            if not combined or not combined.get("atoms"):
                raise ValueError("ساختار مشترک مولکول‌ها خالی است")
            title = "مخلوط: " + " + ".join(name for _f, _s, name, _a in items[:5])
            if len(items) > 5:
                title += f" + {len(items)-5} مادهٔ دیگر"
            dlg = MplMolecule3DDialog(structure=combined, title=title, parent=self)
            dlg.exec()
        except Exception as e:
            tlog(f"mixture 3D failed: {e}", "ERROR")
            QMessageBox.warning(
                self, "خطا در مدل ۳بعدی مخلوط",
                f"نمایش سه‌بعدی مخلوط انجام نشد، اما برنامه متوقف نمی‌شود.\n\n{e}"
            )
    ...
    def filter_chemicals(self, text):
        ...
        if hasattr(self, "lbl_chem_count"):
            self.lbl_chem_count.setText(f"{self.combo_chem.count():,} ماده قابل انتخاب")
            ...
            if self.engine.speed_multiplier > 0:
                ...
                if not HAS_PYQTGRAPH:
                    ...
                    try:
                        ...
                        pass
                if graph_tab_active and fi % 4 == 0:
                    try:
                        self._refresh_graph()
                        if not HAS_PYQTGRAPH:
