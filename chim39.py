...
import shutil
from datetime import datetime  # اضافه شده برای گزارش‌گیری
...
class AnimatedContainer(QWidget):
    def __init__(self, parent=None):
        ...
        self.bubbles = []
        self.particles = []  # سیستم ذرات جدید برای واکنش ها
        self.plate_state = "off"
        self.plate_glow_alpha = 0
        self.plate_glow_dir = 5

        # متغیرهای همزن مغناطیسی
    ...
    def process_animations(self):
        ...
        if total_amount > 0:
            ...
            # در صورت روشن بودن همزن تعداد حباب ها بیشتر میشود
            spawn_chance = 0.6 if is_heating else (0.3 if self.stirrer_on else 0.05)
            ...
            for b in self.bubbles:
                ...
                # اثر همزن بر حباب ها
                if self.stirrer_on:
                    b['x'] += math.sin(self.stirrer_angle * math.pi / 180.0) * 2
                else:
    ...
    def trigger_reaction_animation(self):
        ...
        # تولید ذرات جرقه مانند
        margin_x, margin_y = 60, 30
        h = self.height()
        base_y = h - margin_y - 30
        total_amount = sum(l['amount'] for l in self._layers)
        if total_amount > 0:
            scale = (h - 2 * margin_y - 30) / self.max_capacity
        ...
        for _ in range(50):
            self.particles.append({
                ...
                'color': random.choice([QColor(255, 200, 50), QColor(255, 100, 50), QColor(200, 255, 255)])
            })
    ...
    def paintEvent(self, event):
        ...
        painter.drawEllipse(QRectF(plate_rect.left() + 15, plate_rect.top() + 5, 15, 15))
        painter.drawEllipse(QRectF(plate_rect.right() - 30, plate_rect.top() + 5, 15, 15))
...
class LabEngine:
    ...
    def filter_solids(self):
        # حذف تمامی مواد جامد، فلزات و رسوبات از ظرف
        removed = []
        new_layers = []
    ...
    def add_chemical(self, key, amount):
        ...
        if self.total_volume > 0:
            ...
            q_joules = added_moles * (-heat) * 1000
            mass_approx = self.total_volume
            if mass_approx > 0:
                dt = q_joules / (mass_approx * 4.18)
                self.temp_c += dt
...
class ModernLabWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("آزمایشگاه شیمی پیشرفته - شیمی‌لَب (نسخه 39)")
    ...
    def setup_ui(self):
        ...
        btn_toggle_tabs.setStyleSheet("background-color: #585b70; color: #ffffff; font-weight: bold; margin-top: 5px;")
    ...
    def action_toggle_stirrer(self):
        ...
        state_text = "روشن" if is_on else "خاموش"
        self.txt_log.append(f"🌪️ همزن مغناطیسی {state_text} شد.")
    ...
    def create_notes_tab(self):
        ...
        """)
        self.txt_notes.setPlaceholderText("یادداشت‌ها خود را بنویسید...")
        ...
        if filename:
            try:
                with open(filename, 'w', encoding='utf-8') as f:
                    f.write("=== گزارش آزمایشگاه شیمی‌لَب ===\n")
    ...
    def create_about_tab(self):
        ...
        شیمی‌لَب: شبیه‌ساز پیشرفته آزمایشگاه شیمی (نسخه ۳۹)

        امکانات اضافه شده در این نسخه:
        - همزن مغناطیسی (Magnetic Stirrer)
        - سیستم فیلتراسیون هوشمند جامدات
        - خروجی‌گیری گزارشات آزمایشگاه به فایل متنی
        - سیستم ذرات بصری (Particle System) برای واکنش‌های شیمیایی
