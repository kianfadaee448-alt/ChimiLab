...
class ModernLabWindow(QMainWindow):
    ...
    def update_report_card(self):
        ...
        # امتیاز واکنش‌ها (ساده‌تر و منصفانه‌تر)
        ...
        # جریمه‌ی شکستگی (تعدیل شده)
        breaks = s['flask_breaks']
        if breaks == 0:
            score += 10  # پاداش کم‌تر
        elif breaks == 1:
            score -= 10  # جریمه‌ی سبک
        elif breaks == 2:
            score -= 35
        else:
            score -= 70  # بیش از دو بار، سقوط آزاد

        final_score = max(0, min(100, score))

        # نگاشت به حروف (با آستانه‌های پایین‌تر برای D و C)
        if final_score >= 90:
            grade = "A++ (نابغه)"
        elif final_score >= 75:
            grade = "A+ (عالی)"
    ...
    def create_datasheet_tab(self):
        ...
        search_label.setStyleSheet("color: #cdd6f4; font-weight: bold;")

        self.search_input = QLineEdit()
        self.search_input.setPlaceholderText("نام ماده یا فرمول شیمیایی...")
        ...
        # ذخیره‌ی تمام داده‌ها برای فیلتر کردن
    ...
    def populate_datasheet(self, items):
        """پر کردن جدول با آیتم‌های داده شده"""
        ...
        for i, (key, data) in enumerate(items):
            ...
            # ستون ۱: فرمول (با زیرنویس)
            formula = data.get('formula', '')
            table.setItem(i, 1, QTableWidgetItem(ChemicalCalculator.to_subscript(formula)))

            # ستون ۲: نوع (فارسی)
            table.setItem(i, 2, QTableWidgetItem(get_persian_type(data.get('type', ''))))

            # ستون ۳: رنگ (سلول رنگی)
    ...
    def filter_datasheet(self):
        """فیلتر کردن جدول بر اساس متن جستجو (نام یا فرمول)"""
        search_text = self.search_input.text().strip().lower()

        if not search_text:
            filtered_items = self.all_datasheet_items
        else:
            filtered_items = []
            for key, data in self.all_datasheet_items:
                name = data.get('name', '').lower()
                formula = data.get('formula', '').lower()
                # جستجو در نام یا فرمول
                if search_text in name or search_text in formula:
                    filtered_items.append((key, data))

        self.populate_datasheet(filtered_items)
