...
# تلاش برای وارد کردن OpenGL (اگر نباشد برنامه فقط ۲بعدی کار می‌کند)
HAS_OPENGL = False
OPENGL_ERROR = ""
try:
    ...
    HAS_OPENGL = False
    # کلاس ساختگی تا ارجاع‌ها نشکنند
...
QGroupBox {
    border: 1px solid #6d28d9; border-radius: 10px; margin-top: 10px;
...
QGroupBox::title {
    ...
    padding: 3px 12px; background-color: #3b0764; color: #e9d5ff;
    border-radius: 8px; border: 1px solid #a78bfa;
}
QPushButton {
...
def get_db_path():
    ...
    # artifacts / attachments (محیط توسعه)
    ...
    if not existing:
        # اگر هیچ‌کدام نبود، مسیر محلی پیش‌فرض
    ...
    chosen = existing[0]
    local_db = os.path.join(base_dir, "db.db")
    # اگر نسخهٔ جدیدتر خارج از کنار برنامه است، همگام‌سازی اختیاری
    try:
...
def load_databases():
    ...
    try:
        # پاک‌سازی کامل تا داده‌های قدیمی/پیش‌فرض باقی نمانند
        CHEMILAB_DB.clear()
        ...
        for i in cursor.fetchall():
            ...
            # state=9, hazard=10 (اگر ستون‌ها موجود باشند)
            state_val = "جامد"
...
# ============================================================
# احراز هویت مدیر و محافظت از دیتابیس
# ============================================================
import hashlib
import hmac
import secrets
...
def _backup_db_file(db_path):
    ...
    try:
        ...
        # پاک‌سازی بکاپ‌های قدیمی (فقط ۳ تای آخر)
        ...
        for old in baks[3:]:
            try:
                ...
                pass
        # سازگاری: یک .bak ساده هم به‌روز شود
        simple = db_path + ".bak"
        try:
...
def get_save_path(player_name=None):
    ...
    # سازگاری عقب‌رو: فایل سراسری قدیمی
...
def load_active_session():
    ...
    try:
        ...
        if isinstance(d, dict) and d.get("player_name"):
            # is_admin از session برای auto-login پذیرفته نمی‌شود
            d["is_admin"] = False
            return d
    except Exception:
        pass
...
def wipe_all_user_progress():
    ...
    for fname in ("lab_save.json", "lab_save.json.tmp"):
        ...
        try:
            ...
            pass
    # پاک کردن پیشرفت‌های جداگانهٔ کاربران
    try:
        root = get_users_root()
...
def is_solid_chemical_type(ctype):
    ...
    if not ctype:
        return False
    ctype = str(ctype)
    # اگر صریحاً مایع یا گاز است و کلمه جامد ندارد → جامد نیست
    ...
    if has_lg and not has_solid:
        return False
    # کلمات قوی جامد اولویت دارند حتی اگر «محلول» در type باشد
...
class ChemicalCalculator:
    ...
    # جدول کامل جرم اتمی استاندارد (IUPAC) برای هر ۱۱۸ عنصر + دوتریوم/تریتیوم.
    # این جدول به‌عنوان پشتیبان (fallback) وقتی کتابخانهٔ molmass در دسترس نباشد
    # یا نتواند فرمول را تجزیه کند استفاده می‌شود؛ تا پیش از این، عناصر بعد از
    # اورانیوم (لانتانیدها، آکتینیدها، Nb/Tc/Ru/.../Tl/Po/Rn/Fr/Ra و…) اصلاً
    # در جدول نبودند و همین باعث می‌شد درصد جرمی/فرمول تجربی برای آن‌ها صفر
    # یا نادرست محاسبه شود.
    ATOMIC_MASS = {
    ...
    def reduce_slots(atom_amounts):
        ...
        # جریمه اگر target خیلی کوچک بوده
    ...
    def fill_atom_jars(contents_moles, db, merge_isotopes=True):
        ...
        for key, moles in contents_moles.items():
            ...
            form = ""
            name = key
            # تطبیق کلید بدون حساسیت به حروف
            db_entry = None
            if db:
                if key in db:
                    db_entry = db[key]
    ...
    def find_limiting_reagent(reactants_needed, contents_moles, db, coeffs=None):
        ...
        # نادیده گرفتن extent صفر در argmin با مقدار بزرگ
        ...
        # درصد تبدیل نسبت به میانگین بقیه
...
class LabEngine:
    ...
    def load_data(self):
        # مسیر مخصوص کاربر؛ در صورت نبود، مهاجرت از lab_save.json قدیمی فقط اگر نام یکسان باشد
        ...
        try:
            ...
            if not isinstance(d, dict):
                return
            # محدودسازی مقادیر cheat-prone از فایل
            ...
            # نام از engine اولویت دارد مگر فایل متعلق به همین کاربر باشد
            ...
            # مهاجرت به مسیر کاربر اگر از legacy خوانده شد
    ...
    def filter_solids(self):
        ...
        if self.total_volume < 0:
            self.total_volume = 0
        # همگام‌سازی مول اسید/باز بعد از حذف مایعات
        try:
    ...
    def add_chemical(self, key, amount, custom_molarity=None):
        ...
        if any(x in chem_type for x in ["Solid", "Metal", "Salt", "Powder", "Precipitate"]):
            # جرم (g) → مول با جرم مولی واقعی
            mm = ChemicalCalculator.molar_mass(formula)
            if mm > 1e-9:
                added_moles = float(amount) / mm
            else:
                # fallback آموزشی اگر فرمول قابل تجزیه نبود
    ...
    def update_physics(self):
        ...
        # نقطه جوش تقریبی مخلوط (میانگین bp مایعات در صورت موجود بودن)
        boil_pts = []
        for l in self.visual_layers:
    ...
    def _apply_reaction_stoich(self, rxn, extent):
        ...
        # مصرف واکنش‌دهنده‌ها — لایه‌ها را با ساخت لیست جدید به‌روز کن (امن‌تر از remove در حلقه)
        for r in rxn.get("reactants", []):
            ...
            if pk in CHEMILAB_DB:
                ...
                for k, v in CHEMILAB_DB.items():
                    if normalize_key(v.get("formula", "")) == pk or normalize_key(k) == pk:
                        ...
                        break
            if not db_key:
                # محصول ناشناخته — کلید نرمال‌شده تا با contents/normalize سازگار بماند
                db_key = pk
        ...
        # بازمحاسبه حجم از لایه‌ها — H/OH فقط با rebuild
    ...
    def check_reactions(self):
        ...
        # یک واکنش در هر فریم: اول کشف جدید، وگرنه یک واکنش قدیمی با cooldown
    ...
    def get_ph(self):
        if self.total_volume <= 0 or self.is_broken:
            return 7.0
        # state و derived-state همگام: همیشه از محتویات بازسازی کن
        try:
            self.rebuild_acid_base_moles()
        except Exception:
            pass
...
class ModernLabWindow(QMainWindow):
    def __init__(self, player_name="دانشجو", is_admin=False):
        ...
        # بارگذاری lab_save.json و سپس تنظیم نام نشست
        if player_name:
        ...
        if hasattr(self, "combo_chem") and self.combo_chem is not None:
            try:
                ...
                pass
        # فوکوس پنجره اصلی تا میانبرها حتی با فوکوس روی ویجت‌های فرزند کار کنند
        ...
        if self.is_admin:
            # مدیر: بدون حلقه فیزیک آزمایشگاه
            pass
        else:
    ...
    def closeEvent(self, event):
        try:
            if self._logout_requested:
                # خروج آگاهانه: پیشرفت قبلاً پاک شده
                pass
            else:
                self.engine.save_data()
                if not self.is_admin:
                    ...
                    # ادمین: نشست را نگه ندار تا هر بار رمز بخواهد (امنیت)
                    clear_active_session()
        except Exception:
            pass
        event.accept()
    ...
    def _setup_admin_ui(self):
        ...
        # اشیای ساختگی تا game_loop / متدهای مشترک کرش نکنند
    ...
    def eventFilter(self, obj, event):
        # کشیدن از روی کمبو حذف شد — با باز شدن لیست فیلتر تداخل داشت.
        # برای کشیدن فقط از دکمه «کشیدن» (start_chem_drag) استفاده شود.
    ...
    def _create_left_panel(self):
        ...
        # ویجت‌های پروفایل (مخفی) تا update_player_stats کار کند
    ...
    def _create_all_tabs(self):
        ...
        if self.is_admin:
            # پنل مدیر فقط مدیریت دیتابیس — بدون مدل بور و ابزار آزمایشگاه
    ...
    def create_datasheet_tab(self):
        ...
        # قفسه کاربر: موادی که پنهان شده‌اند
    ...
    def datasheet_hide_selected(self):
        ...
        if not hasattr(self, "_datasheet_hidden"):
            self._datasheet_hidden = set()
        # map row -> key via name+formula match
    ...
    def update_wiki_tab(self):
        ...
        try:
            # پیدا کردن ایندکس تب دانشنامه با عنوان
            index = -1
        ...
        try:
            if mods & Qt.ControlModifier:
                ...
                if key == Qt.Key_S:
                    ...
                    return
            # برای حروف تکی فقط وقتی فوکوس روی LineEdit/TextEdit نیست
            focus = self.focusWidget()
    ...
    def game_loop(self):
        try:
            ...
            # تشخیص نقطه هم‌ارزی با هیسترزیس پایدارتر (سرعت بالا / پرش pH)
            ...
            if self.btn_titrate.isChecked():
                ...
                # نیاز به چند فریم متوالی نزدیک ۷ یا پرش واضح
            ...
            if fi % 12 == 0:
                ...
                if achv:
                    ...
                    # پیام غیرمسدودکننده — جلوگیری از لگ و فریز
...
if __name__ == '__main__':
    ...
    while True:
        name = None
        is_admin = False
        # ورود خودکار اگر نشست فعال دانش‌آموز وجود داشته باشد
