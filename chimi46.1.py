...
class AppLogger:
    ...
    def log(self, level, msg, **extra):
        ...
        if len(self.memory) > self.max_mem:
            self.memory.pop(0)
        # لاگ‌های پرتکرار UI/LAYOUT/CLICK فقط در حافظه — نه دیسک/ترمینال (علت لگ I/O)
        ...
        def paintEvent(self, event):
            ...
            if self.engine and not self.engine.is_broken:
                ...
                for layer in (self.engine.visual_layers or [])[-5:]:
                    ...
                    painter.setPen(QColor(layer.get('color', '#a6e3a1')))
...
class BohrModelWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        if HAS_OPENGL:
            ...
            self.canvas = self.canvas_2d
            self.lbl_bohr_mode = QLabel("⚠️ برای مدل سه‌بعدی PySide6.QtOpenGLWidgets و PyOpenGL را نصب کنید.")
            self.lbl_bohr_mode.setStyleSheet("color: #f38ba8; font-size: 11px; border:none;")
