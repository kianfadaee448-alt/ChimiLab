...
class LoginDialog(QDialog):
    def __init__(self, parent=None):
        ...
        title = QLabel("🧪 آزمایشگاه شیمی‌لَب V41")
...
class ModernLabWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("آزمایشگاه شیمی پیشرفته - شیمی‌لَب (نسخه 41 + اتم بور)")
    ...
    def create_about_tab(self):
        ...
        با شیمی‌لَب، دیوار بی‌آزمایشگاهی در مدارس کشور فرو می‌ریزد و هر دانش‌آموز، فرصت درک عملی شیمی را پیدا می‌کند.
    ...
    def filter_chemicals(self, text):
        ...
        for k, v in sorted(CHEMILAB_DB.items(), key=lambda x: x[1]['name']):
            match_text = t in v['name'].lower() or t in v['formula'].lower() or t in k
    ...
    def action_wash(self):
        if self.engine.is_broken:
            self.container.repair_flask_animation()
        self.engine.reset()
    ...
    def game_loop(self):
        try:
            ...
            if self.btn_titrate.isChecked() and (self.last_ph < 6 and ph > 8 or self.last_ph > 8 and ph < 6):
                self.txt_log.append("✅ نقطه هم‌ارزی تیتراسیون فرا رسید!")
            self.last_ph = ph
