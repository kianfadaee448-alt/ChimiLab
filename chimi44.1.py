...
try:
    ...
    from PyQt5.QtCore import Qt, QTimer, QRectF, QPropertyAnimation, pyqtProperty, QEasingCurve, QPointF, QSize
...
class LabEngine:
    ...
    def filter_solids(self):
        ...
        for layer in self.visual_layers:
            ...
            if is_solid_chemical_type(combined_type):
...
class ModernLabWindow(QMainWindow):
  ...
  def _create_left_panel(self):
    """پنل کنترل سمت چپ (پروفایل، افزودن ماده، ابزارها، جزئیات، لاگ)"""
    panel = QFrame()
  ...
  def create_about_tab(self):
    ...
    content_layout.setContentsMargins(15, 15, 15, 15)  # فاصله از لبه‌های اسکرول
  ...
  def update_report_card(self):
    ...
    # سیستم نمره‌دهی سخت‌گیرانه (با اصلاحات)
    score = 0
  ...
  def update_wiki_table(self):
    ...
    for i, (n, d) in enumerate(disc.items()):
      ...
      reactants = d['reactants'] if isinstance(d['reactants'], list) else []
      products = d['products'] if isinstance(d['products'], list) else []
      lbl_r = QLabel(" + ".join([self.get_state_color_text(r) for r in reactants]))
      lbl_r.setWordWrap(True);
      lbl_r.setTextFormat(Qt.RichText)
      self.table_wiki.setCellWidget(i, 1, lbl_r)
      lbl_p = QLabel(" + ".join([self.get_state_color_text(p) for p in products]))
  ...
  def action_add(self):
    try:
      ...
      self._log(msg)
      for w in warnings: self._log(f"<span style='color:#f38ba8;'>{w}</span>")
  ...
  def action_wash(self):
    # باگ قبلی (ثبت خطای اضافی) حذف شد
    self.engine.reset()
