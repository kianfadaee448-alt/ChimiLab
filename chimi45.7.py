...
class ChemicalCalculator:
    ...
    def parse_formula(formula):
        """تجزیه فرمول با molmass — تعداد اتم هر عنصر"""
        ...
        try:
            f = Formula(formula)
            composition = Counter()
            # molmass: composition.items() → (symbol یا Element, count)
            for el, count in f.composition.items():
                symbol = getattr(el, "symbol", None) or str(el)
                # گاهی کلید به صورت 'H' و مقدار تعداد اتم است
                composition[str(symbol)] = int(round(float(count)))
            if composition:
                return composition
        except Exception:
            pass
    ...
    def molar_mass(formula):
        """جرم مولی با molmass (g/mol)"""
        try:
            f = Formula(normalize_chem_formula(formula))
            return float(f.mass)
        except Exception:
            return 0.0
    ...
    def calculate_empirical_from_moles(atom_moles_counter):
        """فرمول تجربی از تعداد مول اتم‌ها — با کمک molmass"""
        if not atom_moles_counter:
            return "ماده‌ای وجود ندارد"
        filtered_atoms = {str(k): float(v) for k, v in atom_moles_counter.items() if v > 1e-9}
        if not filtered_atoms:
            return "-"
        # ساخت فرمول خام نسبی برای molmass
        ...
        for m in range(1, 40):
            err = sum(abs(r * m - round(r * m)) for r in ratios.values())
            if err < best_error:
                best_error = err
                best_multiplier = m
            if err < 0.04:
                break
        # ترتیب Hill: C, H, سپس الفبا
        ...
        raw = ""
        for el in ordered:
            n = int(round(ratios[el] * best_multiplier))
            if n <= 0:
                continue
            raw += el if n == 1 else f"{el}{n}"
        if not raw:
            return "-"
        # تأیید و ساده‌سازی با molmass
        try:
            f = Formula(raw)
            emp = getattr(f, "empirical_formula", None) or getattr(f, "formula", None)
            if emp:
                return ChemicalCalculator.to_subscript(str(emp).replace(" ", ""))
            # از composition molmass دوباره بساز
            parts = []
            comp_items = list(f.composition.items())
            el_order = []
            rest = []
            for el, cnt in comp_items:
                sym = getattr(el, "symbol", None) or str(el)
                if sym == "C":
                    el_order.insert(0, (sym, cnt))
                elif sym == "H":
                    el_order.append((sym, cnt))
                else:
                    rest.append((sym, cnt))
            rest.sort(key=lambda x: x[0])
            # ensure C then H then rest
            final = []
            c_part = [x for x in el_order if x[0] == "C"]
            h_part = [x for x in el_order if x[0] == "H"]
            final.extend(c_part)
            final.extend(h_part)
            final.extend(rest)
            for sym, cnt in final:
                n = int(round(float(cnt)))
                if n <= 0:
                    continue
                parts.append(sym if n == 1 else f"{sym}{n}")
            return ChemicalCalculator.to_subscript("".join(parts) or raw)
        except Exception:
            return ChemicalCalculator.to_subscript(raw)
...
if HAS_OPENGL:
    class GLBeakerCanvas(QOpenGLWidget):
        ...
        def _tick_stirrer(self):
            # فقط وقتی ویجت دیده می‌شود رندر کن (کاهش کندی)
            if not self.isVisible():
                return
            need = self._needs_redraw or self.stirrer_on
            if self.stirrer_on:
        ...
        def paintGL(self):
            ...
            if hasattr(self, 'bubbles_3d') and self.bubbles_3d:
                glDisable(GL_LIGHTING)
                for b in self.bubbles_3d:
                    a = max(0.25, min(0.85, b['life'] / 60.0))
                    glColor4f(0.95, 0.98, 1.0, a)
                    glPushMatrix()
                    glTranslatef(b['x'], b['y'], b['z'])
                    if self.quadric:
                        gluSphere(self.quadric, b['r'], 10, 10)
                    else:
                        disk(0, b['r'], (0.95, 0.98, 1.0, a))
                    glPopMatrix()
...
class ModernLabWindow(QMainWindow):
    ...
    def eventFilter(self, obj, event):
        from PyQt5.QtCore import QEvent, QMimeData, QPoint
