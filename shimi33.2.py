...
class ChemicalCalculator:
    ...
    def parse_formula(formula):
        ...
        # Updated: Handle parentheses logic inside the requested structure
    ...
    def format_gross_formula(atom_counts, simplify=True, empirical=False):
        ...
        # Filter noise (Updated part for float precision)
        atom_counts = {k: v for k, v in atom_counts.items() if v > 1e-9}
        ...
        if empirical:
            vals = [atom_counts[el] for el in sorted_elements]
            if not vals: return ""
            min_val = min(vals)

            # Normalize to the smallest value
            for el in sorted_elements:
                atom_counts[el] /= min_val

            # Rounding (Updated logic to handle near-integers)
            for el in sorted_elements:
                atom_counts[el] = round(atom_counts[el])
...
class LabEngine:
    ...
    def get_mixture_empirical_formula(self):
        ...
        return ChemicalCalculator.format_gross_formula(total_atoms, empirical=True)
