...
try:
    from PySide6.QtWidgets import (
        ...
        QScrollBar, QToolBar, QSlider, QCheckBox
    )
    from PySide6.QtCore import Qt, QTimer, QRectF, QPropertyAnimation, Property, QEasingCurve, QPointF, QSize, QRect, QEvent, QMimeData, QPoint, QUrl
    from PySide6.QtGui import QFont, QColor, QPainter, QPen, QLinearGradient, QPainterPath, QBrush, QRadialGradient, \
        QPixmap, QPdfWriter, QPageSize, QImage, QAction, QIcon, QDrag, QShortcut, QKeySequence
...
try:
    ...
    OPENGL_ERROR = str(e)
    print(f"[WARN] OpenGL/PyOpenGL در دسترس نیست — حالت ۲بعدی فعال می‌شود: {e}", flush=True)
    HAS_OPENGL = False
...
try:
    ...
    print(f"[WARN] QtWebEngine در دسترس نیست — تب وب ۳بعدی از OpenGL داخلی استفاده می‌کند: {e}", flush=True)
    HAS_WEBENGINE = False
...
QWidget { background-color: #0a0612; color: #e8e0f5; font-family: 'Tahoma', sans-serif; font-size: 13px; }
/* متن بدون جعبه و حاشیهٔ اضافه */
QLabel {
...
QListWidget {
    background-color: #100a18; border: 1px solid #2e2040; border-radius: 12px;
    color: #e8e0f5; font-size: 14px; padding: 4px;
}
...
QTableWidget {
    background-color: #100a18; gridline-color: #2e2040; color: #e8e0f5;
    border: 1px solid #2e2040; border-radius: 12px;
}
QHeaderView::section {
...
QTabBar::tab {
    background: #1a1228; color: #a89bb8; padding: 6px 10px; margin-right: 2px;
    border-top-left-radius: 8px; border-top-right-radius: 8px;
    border: 1px solid #2e2040; border-bottom: none;
    font-size: 11px; min-height: 22px; max-height: 28px;
}
QTabBar::tab:selected {
    background: qlineargradient(x1:0,y1:0,x2:1,y2:0, stop:0 #7c3aed, stop:1 #e879f9);
    color: #0c0914; font-weight: bold;
}
QTabBar::tab:hover:!selected {
    background: #2e1065; color: #e9d5ff;
}
QTabBar::tab:hover:!selected { background: #2a1e3d; color: #e8e0f5; }
QProgressBar {
...
QScrollArea { border: none; background-color: #0a0612; }
QScrollArea > QWidget > QWidget { background-color: #0a0612; }
QScrollBar:vertical {
...
# ============================================================
LAB3D_STATIONS = [
    {"id": "bench", "pos": (0.15, 1.05, 0.05), "radius": 1.15, "title": "میز کار", "action": "bench"},
    {"id": "hotplate", "pos": (-0.95, 1.05, 0.1), "radius": 0.55, "title": "هات‌پلیت", "action": "heat"},
    {"id": "bunsen", "pos": (-1.55, 1.05, -0.15), "radius": 0.5, "title": "مشعل بنزن", "action": "heat"},
    {"id": "burette", "pos": (1.15, 1.2, 0.05), "radius": 0.55, "title": "بورت", "action": "titrate"},
    {"id": "filter", "pos": (1.7, 1.05, 0.2), "radius": 0.45, "title": "فیلتر", "action": "filter"},
    {"id": "icebath", "pos": (0.55, 1.05, 0.35), "radius": 0.4, "title": "حمام یخ", "action": "cool"},
    {"id": "cabinet", "pos": (-6.15, 1.2, -0.4), "radius": 1.3, "title": "کابینت مواد", "action": "cabinet"},
    {"id": "fridge", "pos": (-6.2, 1.2, 3.15), "radius": 0.9, "title": "یخچال", "action": "cool"},
    {"id": "fumehood", "pos": (-4.6, 1.3, -4.55), "radius": 1.1, "title": "هود بخار", "action": "fume"},
    {"id": "sink", "pos": (3.8, 1.0, -4.55), "radius": 0.85, "title": "سینک", "action": "wash"},
    {"id": "balance", "pos": (5.55, 1.05, -4.5), "radius": 0.6, "title": "ترازو", "action": "info"},
    {"id": "phmeter", "pos": (5.1, 1.05, 0.35), "radius": 0.45, "title": "pH متر", "action": "info"},
    {"id": "spec", "pos": (5.7, 1.1, 1.6), "radius": 0.7, "title": "اسپکتروفتومتر", "action": "info"},
    {"id": "computer", "pos": (5.6, 1.1, 3.55), "radius": 0.9, "title": "کامپیوتر", "action": "info"},
    {"id": "whiteboard", "pos": (6.7, 1.7, 0.2), "radius": 1.1, "title": "وایت‌برد", "action": "info"},
    {"id": "periodic", "pos": (1.6, 1.8, -5.25), "radius": 1.4, "title": "جدول تناوبی", "action": "info"},
    {"id": "shower", "pos": (-6.2, 1.4, 4.85), "radius": 0.7, "title": "دوش ایمنی", "action": "info"},
    {"id": "extinguisher", "pos": (6.45, 1.1, -4.9), "radius": 0.45, "title": "کپسول آتش", "action": "info"},
]
LAB3D_ROOM = {"xMin": -6.72, "xMax": 6.72, "zMin": -5.22, "zMax": 5.22}
LAB3D_COLLIDERS = [
    ((-1.9, -0.7), (1.95, 0.7)),
    ((-6.95, -5.35), (-5.35, 1.6)),
    ((-6.95, 2.35), (-5.45, 4.0)),
...
def safe_float(val, default=0.0):
    """تبدیل امن به float؛ در صورت خطا مقدار پیش‌فرض برمی‌گردد."""
    try:
        if val is None:
            return default
        return float(val)
    ...
    try:
        ...
        cur.execute(
            """CREATE TABLE IF NOT EXISTS chemilab (
                ...
                density REAL DEFAULT 1.0, mp REAL, bp REAL
            )"""
        )
        for col, default in (
            ("state", "جامد"),
        ...
        for key, d in CHEMILAB_DB.items():
            cur.execute(
                "INSERT INTO chemilab (name, fa_name, type, pH, molarity, heat, color, formula, state, hazard, density, mp, bp) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)",
                (
                    str(d.get("name", "")),
                    str(key),
                    str(d.get("type", "")),
...
def _user_id_from_name(name: str) -> str:
    """شناسه پایدار کاربر از نام — برای مسیر ذخیرهٔ جداگانه."""
...
def load_active_session():
    ...
    try:
        with open(p, "r", encoding="utf-8") as f:
            d = json.load(f)
        if isinstance(d, dict) and d.get("player_name"):
            d["is_admin"] = False
            return d
    except Exception:
        pass
...
def wipe_all_user_progress():
    """پاک‌سازی کامل پیشرفت کاربر — بدون امکان بازگشت"""
...
def get_lab3d_html_path():
    """مسیر فایل وب سه‌بعدی (lab3d.html) — جستجوی گسترده کنار exe/cwd/attachments."""
    base = get_app_base_dir()
    cwd = os.path.abspath(".")
    candidates = [
        os.path.join(base, "lab3d.html"),
        os.path.join(base, "artifacts", "lab3d.html"),
        os.path.join(base, "attachments", "lab3d.html"),
        os.path.join(cwd, "lab3d.html"),
        os.path.join(cwd, "artifacts", "lab3d.html"),
        os.path.join(cwd, "attachments", "lab3d.html"),
        os.path.join(os.path.dirname(base), "lab3d.html"),
        os.path.join(os.path.dirname(base), "artifacts", "lab3d.html"),
        os.path.join(os.path.dirname(base), "attachments", "lab3d.html"),
    ]
    if hasattr(sys, "_MEIPASS"):
        candidates.insert(0, os.path.join(sys._MEIPASS, "lab3d.html"))
    existing = []
    seen = set()
    for p in candidates:
        try:
        ...
        if ap in seen:
            continue
        seen.add(ap)
        if os.path.isfile(ap) and os.path.getsize(ap) > 500:
            existing.append(ap)
    if existing:
        existing.sort(key=lambda p: os.path.getmtime(p), reverse=True)
        return existing[0]
...
class ChemicalCalculator:
    ...
    def _parse_formula_cached(s, merge_isotopes):
        ...
        for el, num in re.findall(r'([A-Z][a-z]*)(\d*)', s):
            if merge_isotopes:
                el = ChemicalCalculator.ISOTOPE_MAP.get(el, el)
            flat.append((el, int(num) if num else 1))
        return tuple(flat)
    ...
    def electron_configuration(z):
        """آرایش الکترونی ساده تا آرگون (Z≤18) و تقریب برای بقیه"""
        if z <= 0 or z > 118:
            return ""
        order = [
    ...
    def find_limiting_reagent(reactants_needed, contents_moles, db, coeffs=None):
        ...
        others = extent_arr[np.arange(len(extent_arr)) != i]
        others = others[others > 1e-15]
        if others.size:
            conv = float(np.clip((lim_extent / others.mean()) * 100.0, 0.0, 100.0))
        else:
            conv = 100.0
...
class LabEngine:
    ...
    def load_data(self):
        ...
        try:
            ...
            self.discovered = set(d.get("discovered", []) or [])
            saved_name = str(d.get("player_name", "") or "").strip()
            if saved_name:
                self.player_name = saved_name
            self.notes = str(d.get("notes", "") or "")
            self.badges = set(d.get("badges", []) or [])
            self.completed_missions = set(d.get("completed_missions", []) or [])
    ...
    def from_dict(self, d):
        ...
        self.score = safe_int(d.get("score", 0), 0)
        self.level = safe_int(d.get("level", 1), 1)
        self.player_name = str(d.get("player_name", "دانشجو") or "دانشجو")
        self.discovered = set(d.get("discovered", []) or [])
        self.badges = set(d.get("badges", []) or [])
        self.completed_missions = set(d.get("completed_missions", []) or [])
        self.notes = str(d.get("notes", "") or "")
        self.total_volume = safe_float(d.get("total_volume", 0.0), 0.0)
        self.moles_h = safe_float(d.get("moles_h", 0.0), 0.0)
        self.moles_oh = safe_float(d.get("moles_oh", 0.0), 0.0)
        self.temp_c = safe_float(d.get("temp_c", 25.0), 25.0)
        self.contents = d.get("contents", {}) if isinstance(d.get("contents"), dict) else {}
        self.contents = {str(k): safe_float(v, 0.0) for k, v in self.contents.items()}
        layers = d.get("visual_layers", [])
        self.visual_layers = copy.deepcopy(layers) if isinstance(layers, list) else []
        self.is_broken = bool(d.get("is_broken", False))
        self.titration_volume = safe_float(d.get("titration_volume", 0.0), 0.0)
        self.flask_label = str(d.get("flask_label", "بشر شماره ۱") or "بشر شماره ۱")
    ...
    def check_reactions(self):
        ...
        for name, rxn in CUSTOM_REACTIONS.items():
            ...
            applied_extent = min(extent, max(0.05, extent * 0.85)) if extent > 0 else 0.0
            thermo = {
                "dH": dH,
        ...
        if can_apply and applied_extent > 1e-12:
            self.push_undo()
            run_extent = applied_extent if is_new else min(applied_extent, max(0.02, applied_extent * 0.35))
...
if HAS_OPENGL:
    class GLBeakerCanvas(QOpenGLWidget):
        ...
        def paintEvent(self, event):
            ...
            painter = QPainter(self)
            painter.setRenderHint(QPainter.Antialiasing)
            painter.setBrush(QColor(20, 22, 32, 200))
            painter.setPen(QPen(QColor(100, 140, 200, 180), 1))
            painter.drawRoundedRect(8, 8, 210, 28 + 16 * min(5, len(getattr(self.engine, 'visual_layers', []) or [])), 8, 8)
            font = painter.font()
            font.setPointSize(11)
            font.setBold(True)
            painter.setFont(font)
            painter.setPen(QColor(200, 230, 255))
            if self.engine and not self.engine.is_broken:
                vol = float(getattr(self.engine, 'total_volume', 0) or 0)
                painter.drawText(16, 28, f"{vol:.0f} mL | pH {self.engine.get_ph():.1f} | {self.engine.temp_c:.0f}°C")
                font.setPointSize(10)
                font.setBold(False)
                painter.setFont(font)
                y = 46
                for layer in (self.engine.visual_layers or [])[-5:]:
                    name = layer.get('name', '?')
                    amt = layer.get('amount', 0)
                    painter.setPen(QColor(layer.get('color', '#c4b5fd')))
                    painter.drawText(16, y, f"● {name}  {amt:.0f}")
                    y += 16
            else:
                painter.drawText(16, 28, "ظرف خالی — ماده اضافه کنید")
            if self.stirrer_on:
                painter.setPen(QColor(255, 210, 60))
                font.setBold(True)
                painter.setFont(font)
                painter.drawText(self.width() - 100, 28, "🌪️ همزن")
            painter.end()
...
if HAS_OPENGL:
    class GLLabWorld(QOpenGLWidget):
        """اتاق آزمایشگاه سه‌بعدی — معادل LabRoom + Equipment + Player نسخه وب"""
        EYE = 1.62
        WALK = 3.35
        SPRINT = 5.4
        RADIUS = 0.32
        ...
        def __init__(self, engine=None, parent=None):
            ...
            self.px, self.py, self.pz = 0.0, self.EYE, 4.0
            ...
            self.timer.start(16)
            self.on_station = None  # callback(station_dict)
            self.fov_scale = 1.0  # wheel zoom
        ...
        def _move(self, dt):
            speed = self.SPRINT if ('Shift' in self.keys or Qt.Key_Shift in self.keys) else self.WALK
            fx = math.sin(self.yaw)
            fz = -math.cos(self.yaw)
            rx, rz = math.cos(self.yaw), math.sin(self.yaw)
            dx = dz = 0.0
            if Qt.Key_W in self.keys or 'W' in self.keys:
                dx += fx; dz += fz
            if Qt.Key_S in self.keys or 'S' in self.keys:
                dx -= fx; dz -= fz
            if Qt.Key_A in self.keys or 'A' in self.keys:
                dx -= rx; dz -= rz
            if Qt.Key_D in self.keys or 'D' in self.keys:
                dx += rx; dz += rz
            ln = math.hypot(dx, dz)
            if ln > 1e-6:
                dx, dz = dx / ln * speed * dt, dz / ln * speed * dt
                self.px, self.pz = self._collide(self.px + dx, self.pz + dz)

        def _collide(self, x, z):
            r = self.RADIUS
            x = max(LAB3D_ROOM["xMin"] + r, min(LAB3D_ROOM["xMax"] - r, x))
            z = max(LAB3D_ROOM["zMin"] + r, min(LAB3D_ROOM["zMax"] - r, z))
            for (mn, mx) in LAB3D_COLLIDERS:
                minX, minZ = mn[0] - r, mn[1] - r
                maxX, maxZ = mx[0] + r, mx[1] + r
        ...
        def _update_hover(self):
            self.hovered = None
            best = 1e9
            for st in LAB3D_STATIONS:
                sx, sy, sz = st["pos"]
                d = math.hypot(self.px - sx, self.pz - sz)
                if d < st["radius"] and d < best:
                    best = d
                    self.hovered = st
        ...
        def resizeGL(self, w, h):
            ...
            glMatrixMode(GL_PROJECTION)
            glLoadIdentity()
            fov = 72.0 / max(0.5, getattr(self, "fov_scale", 1.0))
            gluPerspective(fov, max(0.1, w / max(1, h)), 0.08, 60.0)
            glMatrixMode(GL_MODELVIEW)
        ...
        def _cylinder(self, x, y, z, radius, height, r, g, b, slices=16):
            glColor3f(r, g, b)
            glPushMatrix()
            glTranslatef(x, y, z)
            quad = gluNewQuadric()
        ...
        def paintGL(self):
            ...
            glLoadIdentity()
            # camera look
            lx = self.px + math.sin(self.yaw) * math.cos(self.pitch)
            ly = self.py + math.sin(self.pitch)
            lz = self.pz - math.cos(self.yaw) * math.cos(self.pitch)
            gluLookAt(self.px, self.py, self.pz, lx, ly, lz, 0, 1, 0)

            # lights (simple)
            glEnable(GL_LIGHTING)
            glEnable(GL_LIGHT0)
            glLightfv(GL_LIGHT0, GL_POSITION, (2.0, 6.5, -8.0, 1.0))
            glLightfv(GL_LIGHT0, GL_DIFFUSE, (1.0, 0.95, 0.85, 1.0))
            glLightfv(GL_LIGHT0, GL_AMBIENT, (0.25, 0.25, 0.28, 1.0))
            glEnable(GL_COLOR_MATERIAL)

            # floor / ceiling
            self._box(0, -0.05, 0, 14.2, 0.1, 11.2, 0.35, 0.34, 0.32)
            self._box(0, 3.18, 0, 14.2, 0.08, 11.2, 0.55, 0.55, 0.52)
            # walls
            self._box(0, 1.55, -5.55, 14.4, 3.15, 0.28, 0.85, 0.82, 0.75)
            self._box(0, 1.55, 5.55, 14.4, 3.15, 0.28, 0.85, 0.82, 0.75)
            self._box(-7.1, 1.55, 0, 0.28, 3.15, 11.4, 0.82, 0.80, 0.74)
            self._box(7.1, 1.55, 0, 0.28, 3.15, 11.4, 0.82, 0.80, 0.74)

            # central island bench
            self._box(0, 0.5, 0, 3.6, 1.0, 1.2, 0.45, 0.42, 0.40)
            self._box(0, 1.02, 0, 3.7, 0.06, 1.3, 0.55, 0.52, 0.48)

            # side benches (north)
            self._box(0, 0.5, -4.85, 10.0, 1.0, 0.7, 0.42, 0.40, 0.38)
            self._box(0, 1.02, -4.85, 10.1, 0.06, 0.75, 0.52, 0.50, 0.46)

            # cabinets west
            self._box(-6.15, 1.0, -1.5, 1.2, 2.0, 4.5, 0.28, 0.32, 0.38)
            # fridge
            self._box(-6.2, 1.1, 3.15, 1.0, 2.2, 1.2, 0.75, 0.78, 0.82)
            # fume hood
            self._box(-4.6, 1.4, -4.55, 2.2, 2.4, 1.0, 0.55, 0.58, 0.62)
            # sink
            self._box(3.8, 0.85, -4.55, 1.2, 0.9, 0.8, 0.6, 0.65, 0.7)
            # computer desk
            self._box(5.6, 0.7, 3.55, 1.4, 1.0, 1.2, 0.35, 0.33, 0.30)

            # beaker on bench (simple)
            fill = 0.0
            col = (0.4, 0.6, 0.9)
            if self.engine:
                try:
                    fill = min(1.0, max(0.0, float(getattr(self.engine, 'total_volume', 0)) / 100.0))
                    layers = getattr(self.engine, 'visual_layers', []) or []
                    if layers:
                        hx = layers[-1].get('color', '#6699cc')
                        hx = str(hx).lstrip('#')
                        if len(hx) >= 6:
                            col = (int(hx[0:2], 16)/255, int(hx[2:4], 16)/255, int(hx[4:6], 16)/255)
                except Exception:
                    pass
            self._cylinder(0.05, 1.08, 0.02, 0.12, 0.28, 0.75, 0.85, 0.95)
            if fill > 0.02:
                self._cylinder(0.05, 1.08, 0.02, 0.10, 0.25 * fill, *col)

            # hotplate / bunsen markers
            self._box(-0.95, 1.0, 0.12, 0.35, 0.08, 0.35, 0.2, 0.2, 0.22)
            self._cylinder(-1.55, 0.98, -0.12, 0.06, 0.25, 0.3, 0.3, 0.35)

            # extra equipment detail
            # whiteboard
            self._box(6.85, 1.7, 0.2, 0.06, 1.6, 2.2, 0.94, 0.95, 0.97)
            # periodic board
            self._box(1.6, 1.85, -5.38, 2.8, 1.4, 0.06, 0.12, 0.16, 0.28)
            # safety shower head
            self._cylinder(-6.2, 2.4, 4.85, 0.08, 0.5, 0.8, 0.85, 0.9)
            self._box(-6.2, 1.0, 4.85, 0.5, 0.15, 0.5, 0.8, 0.2, 0.2)
            # extinguisher
            self._cylinder(6.45, 0.7, -4.9, 0.12, 0.55, 0.8, 0.15, 0.15)
            # balance
            self._box(5.55, 1.05, -4.5, 0.45, 0.08, 0.35, 0.25, 0.25, 0.3)
            # pH meter
            self._box(5.1, 1.05, 0.35, 0.25, 0.2, 0.18, 0.2, 0.25, 0.3)
            self._box(5.1, 1.2, 0.35, 0.18, 0.1, 0.02, 0.25, 0.75, 0.45)
            # spectro
            self._box(5.7, 1.15, 1.6, 0.7, 0.35, 0.45, 0.28, 0.28, 0.35)
            # burette tube
            self._cylinder(1.15, 1.15, 0.05, 0.025, 0.55, 0.7, 0.85, 0.92)
            # filter funnel
            self._cylinder(1.7, 1.1, 0.2, 0.1, 0.08, 0.8, 0.85, 0.9)
            # ice bath
            self._cylinder(0.55, 1.05, 0.35, 0.16, 0.14, 0.6, 0.8, 0.9)
            # fridge handle hint
            self._box(-5.65, 1.15, 3.15, 0.04, 1.8, 0.9, 0.65, 0.7, 0.75)

            # station highlights
            for st in LAB3D_STATIONS:
                sx, sy, sz = st["pos"]
                is_h = self.hovered and self.hovered["id"] == st["id"]
                g = 0.85 if is_h else 0.35
                self._box(sx, 0.08, sz, 0.35, 0.04, 0.35, 0.2, g, 0.45)

            # simple humanoid at camera? skip - FPS view
            glDisable(GL_LIGHTING)

            # HUD overlay via 2D ortho
            glMatrixMode(GL_PROJECTION)
            glPushMatrix()
            glLoadIdentity()
        ...
        def keyPressEvent(self, e):
            self.keys.add(e.key())
            if e.key() == Qt.Key_E and self.hovered:
                if callable(self.on_station):
                    self.on_station(self.hovered)
        ...
        def mousePressEvent(self, e):
            if e.button() == Qt.LeftButton:
                ...
                self._last_mouse = e.position() if hasattr(e, 'position') else e.localPos()
        ...
        def mouseMoveEvent(self, e):
            ...
            if self._last_mouse is not None:
                dx = float(pos.x() - self._last_mouse.x())
                dy = float(pos.y() - self._last_mouse.y())
                self.yaw += dx * 0.004
                self.pitch -= dy * 0.004
        ...
        def wheelEvent(self, e):
            try:
                ...
                self.fov_scale = max(0.55, min(2.4, getattr(self, "fov_scale", 1.0) + delta * 0.0012))
        ...
        def get_hint(self):
            if self.hovered:
                return f"E — تعامل با «{self.hovered['title']}» · زوم {getattr(self,'fov_scale',1):.1f}×"
            return "WASD حرکت | ماوس نگاه | چرخ‌ماوس زوم | Shift دویدن | E تعامل"

else:
    class GLLabWorld(QWidget):
        def __init__(self, engine=None, parent=None):
            super().__init__(parent)
            self.engine = engine
            self.on_station = None
            lay = QVBoxLayout(self)
            lay.addWidget(QLabel("⚠️ OpenGL در دسترس نیست — نمای اتاق سه‌بعدی غیرفعال است."))
        def set_engine(self, eng):
            self.engine = eng
        def get_hint(self):
            return "OpenGL لازم است"
...
class MolecularStructureDB:
    ...
    def get(cls, formula):
        ...
        for k, v in cls._DATA.items():
            if k.upper() == key:
                return v
        # fallback تقریبی از فرمول ساده (دو/سه اتم)
    ...
    def load_from_sqlite(cls, db_path=None):
        """بارگذاری ساختارهای ۳بعدی از جدول molecule_structures (آفلاین)."""
        try:
...
class MplMolecule3DWidget(QWidget):
    ...
    def show_structure(self, structure, title=None):
        ...
        # میله‌ها (bonds) — خطوط ضخیم بین اتم‌ها
        for bi, bj, order in bonds:
        ...
        # گلوله‌ها (atoms)
        size_map = {"H": 120, "C": 280, "N": 260, "O": 250, "S": 320, "Cl": 340,
                    "Na": 360, "K": 400, "Ca": 380, "Fe": 300, "Cu": 300, "Ag": 320, "F": 220, "P": 300}
        for el, x, y, z in atoms:
            col = ELEMENT_COLORS_3D.get(el, "#b0b0b0")
            s = size_map.get(el, 240)
            self.ax.scatter(
...
class MplMolecule3DDialog(QDialog):
    ...
    def __init__(self, structure=None, formula=None, title="", parent=None):
        ...
        tip = QLabel("مدل میله و گلوله · با درگ ماوس بچرخانید · آفلاین")
        tip.setStyleSheet("color:#6b5b80; font-size:11px; border:none;")
...
class BohrModelWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        left_panel = QFrame()
        left_panel.setStyleSheet("background-color: transparent; border: 2px solid #3d2a55; border-radius: 10px;")
        v_left = QVBoxLayout(left_panel)

        title_lbl = QLabel("مدل اتمی بور و آرایش اوربیتالی (۱۱۸ عنصر)")
        title_lbl.setAlignment(Qt.AlignCenter)
        title_lbl.setStyleSheet("color: #818cf8; font-size: 18px; font-weight: bold; margin: 5px; border:none;")
        ...
        if HAS_OPENGL:
            try:
                ...
                self.lbl_bohr_mode.setStyleSheet("color: #c4b5fd; font-size: 11px; border:none;")
            except Exception as e:
                ...
                self.lbl_bohr_mode.setStyleSheet("color: #fb7185; font-size: 11px; border:none;")
        else:
            self.canvas = self.canvas_2d
            self.lbl_bohr_mode = QLabel("⚠️ مدل سه‌بعدی فقط با PyOpenGL در دسترس است — حالت ۲بعدی فعال است.")
            self.lbl_bohr_mode.setStyleSheet("color: #fb7185; font-size: 11px; border:none;")
        ...
        self.btn_toggle_bohr_dim.setStyleSheet(
            "background-color: #c4b5fd; color: #160f22; border:none; padding: 6px; font-weight: bold;")
        ...
        btn_add.setStyleSheet("background-color: #c4b5fd; color: #160f22; border:none; padding: 6px;")
        btn_add.clicked.connect(self.add_electron)

        btn_remove = QPushButton("➖ حذف آخرین")
        btn_remove.setStyleSheet("background-color: #fb7185; color: #160f22; border:none; padding: 6px;")
        btn_remove.clicked.connect(self.remove_electron)

        btn_reset = QPushButton("🌀 تخلیه مدارها")
        btn_reset.setStyleSheet("background-color: #22d3ee; color: #160f22; border:none; padding: 6px;")
        ...
        btn_brochure.setStyleSheet("background-color: #fbbf24; color: #160f22; border:none; padding: 6px;")
        btn_brochure.clicked.connect(self.download_brochure)

        self.lbl_e_total = QLabel("📀 الکترون‌ها: 0")
        self.lbl_e_total.setStyleSheet("color: #fbbf24; font-size: 16px; font-weight:bold; border:none; padding: 5px;")
        ...
        lbl_style = "color: #e8e0f5; font-size: 15px; font-weight: bold;"
        ...
        self.list_compounds.setStyleSheet(
            "background-color: #160f22; border: 1px solid #3d2a55; border-radius: 6px; padding: 3px;")
    ...
    def toggle_bohr_dimension(self):
        ...
        if self._use_3d_bohr:
            ...
            self.lbl_bohr_mode.setStyleSheet("color: #818cf8; font-size: 11px; border:none;")
        self.update_info()
...
class AnimatedContainer(QWidget):
    ...
    def show_context_menu(self, pos):
        menu = QMenu(self)
        menu.setStyleSheet("background-color: #160f22; color: #e8e0f5; border: 1px solid #3d2a55;")
...
class LoginDialog(QDialog):
    ...
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle(f"{APP_NAME} {APP_VERSION} — ورود")
        ...
        sub = QLabel(f"Universe ChimiLab {APP_VERSION}  •  خراسان رضوی")
        ...
        self.err_lbl = QLabel("")
        self.err_lbl.setStyleSheet("color: #fb7185; font-size: 12px;")
...
class ModernLabWindow(QMainWindow):
    def __init__(self, player_name="دانشجو", is_admin=False):
        ...
        if player_name:
            self.engine.player_name = player_name
            self.engine.save_data()
        else:
    ...
    def request_logout(self):
        """خروج از حساب — تمام اطلاعات کاربر پاک می‌شود."""
        global DB_WRITE_UNLOCKED
        msg = (
            "آیا از حساب خارج می‌شوید؟\n\n"
            "تمام امتیاز، کشف‌ها، مأموریت‌ها و یادداشت‌ها پاک می‌شوند.\n"
            "راهی برای بازگردانی وجود ندارد."
        )
    ...
    def setup_ui(self):
        ...
        top_bar.setStyleSheet(
            "background: qlineargradient(x1:0,y1:0,x2:1,y2:0, stop:0 #12121c, stop:1 #1a1a2e);"
        ...
        btn_settings.setStyleSheet(
            "background-color: #818cf8; color: #160f22; font-weight: bold; padding: 8px 16px; "
            "border-radius: 8px; font-size: 14px;")
        btn_settings.clicked.connect(self.open_settings_panel)
        top_layout.addWidget(btn_settings)
        self.btn_profile = QPushButton("👤 پروفایل شیمیدان")
        self.btn_profile.setStyleSheet(
            "background-color: #c4b5fd; color: #160f22; font-weight: bold; padding: 8px 14px; "
            "border-radius: 8px; font-size: 14px;")
        ...
        self.btn_undo = QPushButton("↩ بازگشت")
        self.btn_undo.setStyleSheet(
            "background-color: #fab387; color: #160f22; font-weight: bold; padding: 8px 14px; "
            "border-radius: 8px; font-size: 14px;")
        self.btn_undo.clicked.connect(self.action_undo)
        top_layout.addWidget(self.btn_undo)
        self.btn_redo = QPushButton("↪ جلو")
        self.btn_redo.setStyleSheet(
            "background-color: #c4b5fd; color: #160f22; font-weight: bold; padding: 8px 14px; "
            "border-radius: 8px; font-size: 14px;")
        self.btn_redo.clicked.connect(self.action_redo)
        top_layout.addWidget(self.btn_redo)
        self.btn_logout = QPushButton("🚪 خروج از حساب")
        self.btn_logout.setCursor(Qt.PointingHandCursor)
        self.btn_logout.setStyleSheet(
            "QPushButton { background: qlineargradient(x1:0,y1:0,x2:1,y2:0, stop:0 #7c3aed, stop:1 #c026d3);"
            "color: #fff; font-weight: bold; padding: 8px 16px; border-radius: 10px; font-size: 13px; border: 1px solid #a78bfa; }"
            "QPushButton:hover { background: qlineargradient(x1:0,y1:0,x2:1,y2:0, stop:0 #8b5cf6, stop:1 #e879f9); }"
            "QPushButton:pressed { background: #5b21b6; }")
        self.btn_logout.clicked.connect(self.request_logout)
        top_layout.addWidget(self.btn_logout)
        self.lbl_safety_status = QLabel("✅ وضعیت ایمنی: ایمن")
        self.lbl_safety_status.setStyleSheet(
            "background-color: #1e2e1a; color: #c4b5fd; font-weight: bold; padding: 4px 10px; "
            "border-radius: 8px; border: 1px solid #c4b5fd; font-size: 12px;")
        top_layout.addWidget(self.lbl_safety_status)
        top_layout.addStretch()
        self.lbl_top_status = QLabel(f"{APP_NAME} {APP_VERSION}")
        self.lbl_top_status.setStyleSheet("color: #22d3ee; font-weight: bold; font-size: 14px;")
        ...
        main_splitter.setSizes([320, 520, 480])
        ...
        main_splitter.setStyleSheet(
            "QSplitter::handle { background: #3d2a55; width: 8px; }"
    ...
    def _update_undo_redo_labels(self):
        if hasattr(self, 'btn_undo'):
            n = self.engine.undo_count()
            self.btn_undo.setText(f"↩ بازگشت ({n})" if n else "↩ بازگشت")
        if hasattr(self, 'btn_redo'):
            n = self.engine.redo_count()
            self.btn_redo.setText(f"↪ جلو ({n})" if n else "↪ جلو")
    ...
    def open_settings_panel(self):
        ...
        btn_log.setStyleSheet("background-color: #818cf8; color: #160f22; font-weight: bold; padding: 10px; border-radius: 8px;")
        def open_log():
            import subprocess
            p = get_logger().path
            try:
    ...
    def _create_left_panel(self):
        ...
        outer = QFrame()
        DimensionController.lock(outer, min_w=280, max_w=420)
        ...
        frm = QFormLayout()
        frm.setSpacing(8)
        frm.setContentsMargins(8, 12, 8, 8)
        ...
        btn_drag.setMinimumHeight(36)
        btn_drag.setToolTip("بزنید و روی ظرف رها کنید")
        btn_drag.setStyleSheet("background-color: #fbbf24; color: #160f22; font-weight: bold; border-radius: 8px; font-size: 13px;")
        btn_drag.clicked.connect(self.start_chem_drag)
        self.btn_titrate = QPushButton("💧 بورت")
        self.btn_titrate.setMinimumHeight(36)
        self.btn_titrate.setCheckable(True)
        self.btn_titrate.clicked.connect(self.action_toggle_titration)
        self.btn_titrate.setStyleSheet("background-color: #818cf8; color: #160f22; font-weight: bold; border-radius: 8px; font-size: 13px;")
        h_btn.addWidget(btn_add)
        h_btn.addWidget(btn_drag)
        h_btn.addWidget(self.btn_titrate)
        DimensionController.lock_row_buttons([btn_add, btn_drag, self.btn_titrate], h=36, min_w=88)

        h_btn2 = QHBoxLayout()
        h_btn2.setSpacing(6)
        self.btn_mol3d = QPushButton("🧬 نمایش ۳بعدی مولکول")
        self.btn_mol3d.setMinimumHeight(38)
        self.btn_mol3d.setToolTip("ساختار سه‌بعدی مادهٔ انتخاب‌شده — برای همه مواد و ترکیبات")
        self.btn_mol3d.setStyleSheet(
            "background: qlineargradient(x1:0,y1:0,x2:1,y2:0, stop:0 #10b981, stop:1 #34d399);"
            "color: #0c0914; font-weight: bold; border-radius: 9px; font-size: 13px; border: 1px solid #6ee7b7;")
        self.btn_mol3d.clicked.connect(self.show_selected_molecule_3d)
        self.btn_mix3d = QPushButton("🧪 ۳بعدی مخلوط")
        self.btn_mix3d.setMinimumHeight(38)
        self.btn_mix3d.setToolTip("نمایش ۳بعدی اولین لایهٔ داخل ظرف")
        self.btn_mix3d.setStyleSheet(
            "background: qlineargradient(x1:0,y1:0,x2:1,y2:0, stop:0 #6366f1, stop:1 #a78bfa);"
            "color: #0c0914; font-weight: bold; border-radius: 9px; font-size: 13px; border: 1px solid #c4b5fd;")
        self.btn_mix3d.clicked.connect(self.show_mixture_molecule_3d)
        h_btn2.addWidget(self.btn_mol3d, 2)
        h_btn2.addWidget(self.btn_mix3d, 1)
        DimensionController.lock_row_buttons([self.btn_mol3d, self.btn_mix3d], h=38, min_w=100)
        ...
        btn_cool.setStyleSheet("background-color: #818cf8; color: #160f22; font-weight: bold; border-radius: 8px;")
        btn_heat.setStyleSheet("background-color: #fb7185; color: #160f22; font-weight: bold; border-radius: 8px;")
        ...
        self.lbl_titration_status.setStyleSheet("color: #818cf8; font-weight: bold; font-size: 12px;")
        ...
        self.lbl_active_mission.setStyleSheet(
            ...
            "padding: 6px; color: #fbbf24; font-size: 11px;")
    ...
    def _create_bottom_toolbar(self, parent_layout=None):
        """نوار پایین — همیشه دیده می‌شود، با لاگ ابعاد"""
        tlog("ساخت نوار پایین شروع شد", "LAYOUT")
        toolbar = QFrame()
        toolbar.setObjectName("bottomToolbar")
        toolbar.setFixedHeight(58)
        toolbar.setMinimumHeight(58)
        toolbar.setMaximumHeight(58)
        toolbar.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)
        toolbar.setStyleSheet(
            "#bottomToolbar { background-color: #1a1a28; border-top: 2px solid #818cf8; }")
        lay = QHBoxLayout(toolbar)
        lay.setContentsMargins(14, 10, 14, 10)
        lay.setSpacing(12)

        def mk(text, slot, bg, name=""):
            b = QPushButton(text)
            b.setFixedHeight(38)
            b.setMinimumWidth(110)
            b.setSizePolicy(QSizePolicy.Preferred, QSizePolicy.Fixed)
            b.setStyleSheet(
                f"QPushButton {{ background-color: {bg}; color: #160f22; font-weight: bold; "
                f"border-radius: 8px; font-size: 12px; padding: 4px 10px; }}"
                f"QPushButton:hover {{ border: 2px solid #fff; }}")
            def _wrapped(*args, _t=text, _s=slot):
                tlog(f"کلیک نوار پایین: {_t}", "CLICK")
                try:
                    _s()
                except Exception as e:
                    tlog(f"خطا در {_t}: {e}", "ERROR")
            b.clicked.connect(_wrapped)
            return b

        lay.addWidget(mk("⚗️ فیلتر جامدات", self.action_filter, "#fbbf24"))
        self.btn_stirrer = QPushButton("🌪️ همزن مغناطیسی")
        self.btn_stirrer.setCheckable(True)
        self.btn_stirrer.setFixedHeight(38)
        self.btn_stirrer.setMinimumWidth(130)
        self.btn_stirrer.setStyleSheet(
            "background-color: #c4b5fd; color: #160f22; font-weight: bold; border-radius: 8px;")
        def _stir():
            tlog(f"کلیک همزن checked={self.btn_stirrer.isChecked()}", "CLICK")
            self.action_toggle_stirrer()
        self.btn_stirrer.clicked.connect(_stir)
        lay.addWidget(self.btn_stirrer)
        lay.addWidget(mk("📸 عکس", self.action_screenshot, "#2a1e3d"))
        lay.addWidget(mk("🚿 تعویض ظرف", self.action_wash, "#22d3ee"))
        lay.addStretch(1)
        if HAS_OPENGL:
            lay.addWidget(mk("۳بعدی", self.toggle_beaker_view, "#c4b5fd"))

        if parent_layout is not None:
            parent_layout.addWidget(toolbar, 0)  # stretch factor 0 = جمع نشود
            tlog("نوار پایین به layout اضافه شد", "LAYOUT",
                 stretch=0, height=toolbar.height(), fixedH=toolbar.minimumHeight())
        else:
            tlog("parent_layout=None — نوار اضافه نشد!", "ERROR")
        self._bottom_toolbar_ref = toolbar
        toolbar.show()
        tlog("نوار پایین show() شد", "LAYOUT", visible=toolbar.isVisible())
    ...
    def show_3d_beaker(self):
        ...
        if not HAS_OPENGL:
            QMessageBox.information(
                self, "حالت دوبعدی",
                "نمایش سه‌بعدی در این اجرا فعال نیست.\n\n"
                "برای EXE باید PyOpenGL همراه برنامه بسته‌بندی شود:\n"
                "  pyinstaller --collect-all PyOpenGL --hidden-import OpenGL.GL ...\n\n"
                "برنامه با بشر دوبعدی کامل کار می‌کند."
                + (f"\n\nجزئیات: {OPENGL_ERROR}" if OPENGL_ERROR else "")
            )
            return
        dlg = QDialog(self)
        ...
        try:
            ...
            QMessageBox.warning(dlg, "خطا", f"ساخت نمای سه‌بعدی ممکن نشد:\n{e}")
            dlg.reject()
            return
        lbl = QLabel("💡 با ماوس بچرخانید")
        lbl.setAlignment(Qt.AlignCenter)
        lbl.setStyleSheet("color: #c4b5fd; font-size: 14px;")
    ...
    def _create_center_panel(self):
        panel = QFrame()
        panel.setStyleSheet("background-color: #0e0e16; border-radius: 18px; border: 1px solid #2e2e48;")
        ...
        title.setAlignment(Qt.AlignCenter)
        title.setStyleSheet(
            "font-size: 16px; color: #22d3ee; font-weight: bold; padding: 6px;")
        ...
        if HAS_OPENGL:
            try:
                ...
                self.gl_beaker.setAcceptDrops(True)
                self.gl_beaker.dragEnterEvent = self.container.dragEnterEvent
                self.gl_beaker.dropEvent = self.container.dropEvent
                self.beaker_stack.addWidget(self.gl_beaker)
            except Exception as e:
                print(f"[WARN] ساخت بشر سه‌بعدی ناموفق — فقط ۲بعدی: {e}", flush=True)
                self.gl_beaker = None
        ...
        self.lbl_temp_display.setStyleSheet("font-size: 16px; color: #fb7185; font-weight: bold;")
    ...
    def toggle_beaker_view(self):
        ...
        if not HAS_OPENGL or self.gl_beaker is None:
            tlog("OpenGL نیست یا gl_beaker=None", "WARN")
            return
    ...
    def _create_all_tabs(self):
        ...
        self.tabs.setStyleSheet(
            "QTabWidget::pane { background: #140f20; border: 1px solid #2e2040; }"
            "QTabBar::tab { background: #1a1228; color: #a89bb8; }"
        )
        if self.is_admin:
            ...
            self.tabs.addTab(self.create_lab3d_tab(), "🏢 آزمایشگاه ۳بعدی")
        return self.tabs
    ...
    def create_admin_db_tab(self):
        ...
        self.admin_chem_table.setColumnCount(9)
        self.admin_chem_table.setHorizontalHeaderLabels(
            ["کلید", "نام", "فرمول", "نوع", "حالت", "درجه خطر", "pH", "مولاریته", "رنگ"])
    ...
    def _refresh_admin_chem_table(self):
        ...
        for i, (k, d) in enumerate(items):
            vals = [k, d.get("name", ""), d.get("formula", ""), d.get("type", ""),
                    d.get("state", "جامد"), d.get("hazard", "کم‌خطر"),
    ...
    def admin_chem_upsert(self):
        ...
        if not color.startswith('#'):
            color = '#' + color
        CHEMILAB_DB[key] = {
            "name": self.adm_c_name.text().strip() or key,
            "type": self.adm_c_type.text().strip() or "Solid",
            "pH": ph,
            "molarity": mol,
            "heat": heat,
            "color": color,
            "formula": self.adm_c_form.text().strip() or key,
            "state": self.adm_c_state.currentText() or "جامد",
            "hazard": (self.adm_c_hazard.text().strip() if hasattr(self, "adm_c_hazard") else "") or "کم‌خطر — احتیاط معمولی",
        }
        self._refresh_admin_chem_table()
    ...
    def admin_rxn_upsert(self):
        ...
        CUSTOM_REACTIONS[name] = {
            "reactants": reactants,
            "products": products,
            "desc": self.adm_r_desc.text().strip(),
            "xp": self.adm_r_xp.value(),
            "temp_min": self.adm_r_temp.value(),
        }
        self._refresh_admin_rxn_table()
    ...
    def admin_save_all_db(self):
        ...
        if ok1 and ok2:
            ...
            QMessageBox.critical(self, "خطا", f"{m1}\n{m2}")
    ...
    def create_about_tab(self):
        ...
        btn_mail.setStyleSheet(
            "background: #818cf8; color: #160f22; font-weight: bold; padding: 10px 18px; border-radius: 10px;"
        )
        def open_mail():
            try:
                from PySide6.QtGui import QDesktopServices
    ...
    def create_lab3d_tab(self):
        """تب آزمایشگاه سه‌بعدی — اولویت: lab3d.html (Three.js) سپس OpenGL داخلی."""
        w = QWidget()
        lay = QVBoxLayout(w)
        lay.setContentsMargins(4, 4, 4, 4)
        lay.setSpacing(4)
        ...
        top = QHBoxLayout()
        self.lab3d_hint = QLabel("در حال بارگذاری نمای سه‌بعدی…")
        self.lab3d_hint.setStyleSheet("color:#c4b5fd; font-weight:bold; padding:4px;")
        top.addWidget(self.lab3d_hint, 1)
        self.btn_lab3d_fs = QPushButton("⛶ تمام‌صفحه")
        self.btn_lab3d_fs.setFixedHeight(32)
        self.btn_lab3d_fs.setStyleSheet(
            "background:#3b0764; color:#f5d0fe; border:1px solid #a78bfa; border-radius:8px; font-weight:bold; padding:4px 10px;")
        self.btn_lab3d_fs.setToolTip("نمای سه‌بعدی را تمام‌صفحه کنید (Esc برای خروج)")
        self.btn_lab3d_fs.clicked.connect(self._lab3d_toggle_fullscreen)
        top.addWidget(self.btn_lab3d_fs)
        self.btn_lab3d_reload = QPushButton("↻ بارگذاری مجدد")
        self.btn_lab3d_reload.setFixedHeight(32)
        self.btn_lab3d_reload.setStyleSheet(
            "background:#1e1530; color:#c4b5fd; border:1px solid #6d28d9; border-radius:8px; padding:4px 10px;")
        self.btn_lab3d_reload.clicked.connect(self._lab3d_reload)
        top.addWidget(self.btn_lab3d_reload)
        self.btn_lab3d_enter = QPushButton("▶ ورود به آزمایشگاه")
        self.btn_lab3d_enter.setFixedHeight(34)
        self.btn_lab3d_enter.setStyleSheet(
            "background:qlineargradient(x1:0,y1:0,x2:1,y2:0, stop:0 #a78bfa, stop:1 #e879f9);"
            "color:#0c0914; font-weight:bold; border:none; border-radius:10px; padding:4px 14px; font-size:13px;")
        self.btn_lab3d_enter.setToolTip("فعال‌سازی کنترل حرکت و دوربین — اگر کلیک روی صحنه کار نکرد این را بزنید")
        self.btn_lab3d_enter.clicked.connect(self._lab3d_enter_scene)
        top.addWidget(self.btn_lab3d_enter)
        lay.addLayout(top)

        self.lab3d_world = None
        self.lab3d_web = None
        self._lab3d_mode = None
        self._lab3d_container = w

        html_path = get_lab3d_html_path()
        has_html = os.path.isfile(html_path)
        if HAS_WEBENGINE and has_html:
            try:
                self.lab3d_web = QWebEngineView(w)
                try:
                    sett = self.lab3d_web.settings()
                    sett.setAttribute(QWebEngineSettings.WebAttribute.LocalContentCanAccessRemoteUrls, True)
                    sett.setAttribute(QWebEngineSettings.WebAttribute.JavascriptEnabled, True)
                    sett.setAttribute(QWebEngineSettings.WebAttribute.LocalContentCanAccessFileUrls, True)
                except Exception:
                    pass
                url = QUrl.fromLocalFile(html_path)
                self.lab3d_web.load(url)
                self.lab3d_web.loadFinished.connect(self._lab3d_on_loaded)
                lay.addWidget(self.lab3d_web, 1)
                self._lab3d_mode = "web"
                self.lab3d_hint.setText(f"وب سه‌بعدی آماده · {os.path.basename(html_path)} — کلیک روی صحنه برای ورود")
                self._lab3d_sync_timer = QTimer(self)
                self._lab3d_sync_timer.timeout.connect(self._lab3d_push_state)
                self._lab3d_sync_timer.start(500)
                self._lab3d_poll_timer = QTimer(self)
                self._lab3d_poll_timer.timeout.connect(self._lab3d_poll_events)
                self._lab3d_poll_timer.start(300)
            except Exception as e:
                tlog(f"lab3d web failed: {e}", "WARN")
                self.lab3d_web = None

        if self.lab3d_web is None:
            try:
                if not HAS_OPENGL:
                    raise RuntimeError("OpenGL در دسترس نیست")
                self.lab3d_world = GLLabWorld(self.engine, w)
                self.lab3d_world.on_station = self._on_lab3d_station
                lay.addWidget(self.lab3d_world, 1)
                self._lab3d_mode = "opengl"
                self.lab3d_hint.setText("OpenGL داخلی — WASD | ماوس | چرخ‌ماوس زوم | E | دکمه تمام‌صفحه")
                self._lab3d_hint_timer = QTimer(self)
                self._lab3d_hint_timer.timeout.connect(self._refresh_lab3d_hint)
                self._lab3d_hint_timer.start(200)
            except Exception as e:
                msg = QLabel(
                    "نمای سه‌بعدی در دسترس نیست.\n"
                    "① فایل lab3d.html را کنار chimi52.py بگذارید\n"
                    "② pip install PySide6-WebEngine\n"
                    "③ یا درایور OpenGL سیستم را به‌روز کنید\n"
                    f"جزئیات: {e}"
                )
                msg.setWordWrap(True)
                msg.setStyleSheet("color:#fb7185; padding:12px; background:#1a1228; border-radius:10px;")
                lay.addWidget(msg)
                self._lab3d_mode = None

        help_l = QLabel(
            "فایل‌های لازم: db.db · lab_save.json · chimi52.py · lab3d.html  |  "
            "ایستگاه‌ها: هات‌پلیت/بنزن=گرما · یخ/یخچال=سرما · بورت=تیتراسیون · فیلتر · سینک=شستشو · کابینت=مواد"
        )
        help_l.setWordWrap(True)
        help_l.setStyleSheet("color:#a89bb8; font-size:11px;")
        lay.addWidget(help_l)
        return w

    def _lab3d_toggle_fullscreen(self):
        """تمام‌صفحه کردن ویجت آزمایشگاه ۳بعدی."""
        try:
            target = self.lab3d_web if getattr(self, "lab3d_web", None) else getattr(self, "lab3d_world", None)
            if target is None:
            ...
            if target.isFullScreen():
                target.showNormal()
                self.btn_lab3d_fs.setText("⛶ تمام‌صفحه")
            else:
                target.showFullScreen()
                self.btn_lab3d_fs.setText("⛶ خروج تمام‌صفحه")
        except Exception as e:
            tlog(f"lab3d fullscreen: {e}", "WARN")
            try:
                if self.isFullScreen():
                    self.showNormal()
                else:
    ...
    def _lab3d_reload(self):
        """بارگذاری مجدد صحنه وب یا ریست دوربین OpenGL."""
        try:
            if getattr(self, "lab3d_web", None) is not None:
                html_path = get_lab3d_html_path()
                if os.path.isfile(html_path):
                    self.lab3d_web.load(QUrl.fromLocalFile(html_path))
                    self.lab3d_hint.setText("در حال بارگذاری مجدد…")
            elif getattr(self, "lab3d_world", None) is not None:
                w = self.lab3d_world
                w.px, w.py, w.pz = 0.0, w.EYE, 4.0
                w.yaw, w.pitch = 0.0, -0.12
                if hasattr(w, "fov_scale"):
                    w.fov_scale = 1.0
                w.update()
                self.lab3d_hint.setText("دوربین بازنشانی شد")
        except Exception as e:
            tlog(f"lab3d reload: {e}", "WARN")

    def _lab3d_enter_scene(self):
        """ورود صریح به کنترل صحنه — بدون وابستگی به pointer-lock مرورگر."""
        try:
            if getattr(self, "lab3d_web", None) is not None:
                # فراخوانی API جاوااسکریپت
                js = "try{ if(window.chimiEnter) window.chimiEnter(); else { var b=document.getElementById('btn-enter'); if(b) b.click(); } }catch(e){}"
                self.lab3d_web.page().runJavaScript(js)
                self.lab3d_hint.setText("وارد شدید — WASD حرکت · نگه داشتن کلیک چپ = نگاه · Esc خروج")
                self.lab3d_web.setFocus()
            elif getattr(self, "lab3d_world", None) is not None:
                self.lab3d_world.setFocus()
                self.lab3d_world._mouse_captured = True
                self.lab3d_world.setCursor(Qt.BlankCursor)
                self.lab3d_hint.setText("OpenGL: کلیک روی صحنه + حرکت ماوس · WASD · Esc آزاد")
        except Exception as e:
            tlog(f"lab3d enter: {e}", "WARN")
            QMessageBox.information(
                self, "ورود",
                "اگر صحنه وب لود نشده، lab3d.html را کنار برنامه بگذارید و "
                "PySide6-WebEngine را نصب کنید.\n"
                f"{e}"
            )

    def _lab3d_on_loaded(self, ok):
        if not ok:
            self.lab3d_hint.setText("بارگذاری lab3d.html ناموفق بود")
            return
        self.lab3d_hint.setText("آماده — کلیک روی صحنه برای قفل ماوس")
        # hook console via JS queue
        js = r"""
        (function(){
          if (window.__chimiHooked) return;
          window.__chimiHooked = true;
          window.__chimiQueue = window.__chimiQueue || [];
          const _log = console.log.bind(console);
          console.log = function(){
            try {
              const s = Array.from(arguments).map(String).join(' ');
              if (s.indexOf('CHIMI:') === 0) window.__chimiQueue.push(s.slice(6));
            } catch(e) {}
        ...
        try:
            ...
            pass
        self._lab3d_push_state()

    def _lab3d_push_state(self):
        if not self.lab3d_web or not self.engine:
            return
        try:
            eng = self.engine
            vol = float(getattr(eng, "total_volume", 0) or 0)
            fill = max(0.0, min(1.0, vol / 100.0))
            color = "#6699cc"
            layers = getattr(eng, "visual_layers", None) or []
            if layers:
                color = str(layers[-1].get("color", color) or color)
            payload = {
                "ph": float(eng.get_ph()) if hasattr(eng, "get_ph") else 7.0,
                "temp": float(getattr(eng, "temp_c", 25) or 25),
                "volume": vol,
                "fill": fill,
                "color": color,
                "broken": bool(getattr(eng, "is_broken", False)),
                "stirrer": bool(getattr(getattr(self, "container", None), "stirrer_on", False)) if hasattr(self, "container") else False,
                "formula": str(getattr(eng, "get_mixture_empirical_formula", lambda: "")() or ""),
                "score": int(getattr(eng, "score", 0) or 0),
                "level": int(getattr(eng, "level", 1) or 1),
            }
            import json as _json
            js = "window.chimiBridge && window.chimiBridge.setState(" + _json.dumps(payload, ensure_ascii=False) + ");"
            self.lab3d_web.page().runJavaScript(js)
        except Exception as e:
            pass

    def _lab3d_poll_events(self):
        if not self.lab3d_web:
            return
        js = r"""
        (function(){
          const q = window.__chimiQueue || [];
          window.__chimiQueue = [];
          return JSON.stringify(q);
        })();
        """
        try:
            self.lab3d_web.page().runJavaScript(js, self._lab3d_on_queue)
        except Exception:
            pass

    def _lab3d_on_queue(self, result):
        if not result:
            return
        try:
            import json as _json
            items = _json.loads(result) if isinstance(result, str) else result
            ...
            for raw in items:
                ...
                if not isinstance(msg, dict):
                    continue
                if msg.get("type") == "station":
                    self._on_lab3d_station(msg)
                elif msg.get("type") == "ready":
                    self.lab3d_hint.setText("صحنه آماده است")
        except Exception:
            pass

    def _refresh_lab3d_hint(self):
        if getattr(self, 'lab3d_world', None) and hasattr(self, 'lab3d_hint'):
            try:
                self.lab3d_hint.setText(self.lab3d_world.get_hint())
            except Exception:
                pass

    def _on_lab3d_station(self, st):
        """نقشه ایستگاه سه‌بعدی → اکشن‌های موتور/UI موجود"""
        if not st:
            return
        act = st.get('action')
        title = st.get('title', st.get('id'))
        try:
            if act == 'heat':
                self.action_heat()
            elif act == 'cool':
                ...
                if hasattr(self, 'btn_titrate'):
                    self.btn_titrate.setChecked(not self.btn_titrate.isChecked())
                    self.action_toggle_titration()
            elif act == 'cabinet':
                if hasattr(self, 'combo_chem'):
                    self.combo_chem.setFocus()
                self._log("🗄️ نزدیک کابینت مواد — از پنل چپ ماده انتخاب و اضافه کنید.")
            elif act == 'bench':
                self._log("🧪 روی میز کار هستید — بشر اصلی اینجاست.")
            else:
                self._log(f"📍 ایستگاه «{title}» — ابزار از پنل‌های کناری در دسترس است.")
            self._log(f"🕹️ تعامل ۳بعدی: {title}")
            self._lab3d_push_state()
        except Exception as e:
            tlog(f"lab3d station: {e}", "ERROR")
    ...
    def create_report_card_tab(self):
        ...
        lbl_title.setStyleSheet("font-size: 20px; font-weight: bold; color: #818cf8; margin: 10px;")
    ...
    def create_missions_badges_tab(self):
        ...
        title.setStyleSheet("font-size: 18px; font-weight: bold; color: #818cf8; border: none; background: transparent;")
        ...
        self.btn_show_missions.setStyleSheet(
            "QPushButton { background-color: #818cf8; color: #160f22; font-weight: bold; padding: 12px; border-radius: 10px; font-size: 14px; }"
            "QPushButton:checked { background-color: #818cf8; }"
        ...
        self.btn_show_badges.setStyleSheet(
            ...
            "QPushButton:checked { background-color: #fbbf24; color: #160f22; }"
            "QPushButton:!checked { background-color: #2a2a3a; color: #e8e0f5; }")
        ...
        self.mission_overall_bar.setStyleSheet(
            ...
            "QProgressBar::chunk { background: qlineargradient(x1:0,y1:0,x2:1,y2:0, stop:0 #818cf8, stop:1 #c4b5fd); border-radius: 7px; }")
        ...
        blbl.setStyleSheet("font-size: 14px; font-weight: bold; color: #fbbf24;")
        pb.addWidget(blbl)
    ...
    def create_notes_tab(self):
        ...
        self.txt_notes.setStyleSheet(
            "QTextEdit { font-size: 16px; background-color: #160f22; color: #e8e0f5; padding: 15px; border-radius: 8px; font-weight: 500; line-height: 1.8; border: 2px solid #818cf8; }")
    ...
    def create_contents_tab(self):
        ...
        btn_mix3d.setStyleSheet("background-color: #34d399; color: #0c0914; font-weight: bold; border-radius: 7px; padding: 6px 10px;")
        ...
        self.lbl_total_mass.setStyleSheet("font-size: 14px; color: #fbbf24; font-weight: bold;")
        row2.addWidget(self.lbl_total_mass)
        btn_mass = QPushButton("⚖️ محاسبه جرم کل مخلوط")
        btn_mass.setStyleSheet("background-color: #818cf8; color: #160f22; font-weight: bold; padding: 6px 12px;")
    ...
    def create_graph_tab(self):
        w = QWidget()
        l = QVBoxLayout(w)
        self.figure = Figure(figsize=(5, 6), facecolor='#100a18')
        self.canvas = FigureCanvas(self.figure)
        self.ax1 = self.figure.add_subplot(211)
        self.ax1.set_facecolor('#160f22')
        self.ax1.set_ylabel('pH', color='white')
        self.ax1.tick_params(colors='white')
        self.ax2 = self.figure.add_subplot(212)
        self.ax2.set_facecolor('#160f22')
        self.ax2.set_ylabel('Temp (°C)', color='white')
        self.ax2.tick_params(colors='white')
        self.line_ph, = self.ax1.plot([], [], color='#c4b5fd', linewidth=2, label='pH')
        self.line_temp, = self.ax2.plot([], [], color='#fb7185', linewidth=2)
        self.eq_line = self.ax1.axhline(y=7.0, color='#fbbf24', linestyle='--', linewidth=1.5,
                                        alpha=0.0, label='نقطه هم‌ارزی ≈۷')
        try:
            self.ax1.legend(loc='upper right', fontsize=8, facecolor='#160f22', edgecolor='#3d2a55', labelcolor='white')
        except Exception:
            pass
        ...
        if not CUSTOM_REACTIONS:
            ...
            for name, rxn in sorted_rxns:
                ...
                if is_disc:
                    ...
                    title_lbl.setStyleSheet("font-size: 16px; font-weight: bold; color: #fbbf24;")
                else:
    ...
    def show_product_detail(self, name, color):
        ...
        dlg.setStyleSheet("background-color: #160f22; border: 2px solid #818cf8; border-radius: 10px;")
        ...
        lbl_name.setStyleSheet("font-size: 22px; font-weight: bold; color: #fbbf24;")
    ...
    def create_datasheet_tab(self):
        ...
        self.search_input.setPlaceholderText("🔍 جستجو (نام، فرمول، حالت، خطر)...")
        ...
        btn_hide.setStyleSheet("background:#fb7185; color:#160f22; font-weight:bold; padding:6px 10px; border-radius:8px;")
        ...
        btn_restore_one.setStyleSheet("background:#818cf8; color:#160f22; font-weight:bold; padding:6px 10px; border-radius:8px;")
        ...
        self.datasheet_table.setColumnCount(9)
        self.datasheet_table.setHorizontalHeaderLabels(
            ["نام", "فرمول", "نوع", "حالت", "درجه خطر", "رنگ", "مولاریته (M)", "pH", "ΔH"])
        ...
        self.datasheet_table.setSelectionBehavior(QTableWidget.SelectRows)
        ...
        self.datasheet_table.setStyleSheet(
            ...
            "border: 1px solid #3d2a55; border-radius: 6px; } "
            "QHeaderView::section { background-color: #160f22; padding: 8px; border: 1px solid #3d2a55; "
            ...
            for m in self.engine.missions:
                ...
                if completed:
                    ...
                    card.setStyleSheet(
                        ...
                        "QFrame:hover { border: 1px solid #818cf8; }")
                cl = QHBoxLayout(card)
                icon = icons.get(m['id'], "🎯")
                ...
                xp = QLabel(f"+{m['xp']} XP")
                xp.setStyleSheet(
                    "font-size: 13px; font-weight: bold; color: #fbbf24; background: #3d2a55; "
    ...
    def update_report_card(self):
        ...
        text = f"""
            ...
            <b>نمره ارزیابی کلی سیستم: <span style='color:#fb7185; font-size: 24px;'>{grade}</span></b>
            """
    ...
    def update_discoveries_table(self):
        ...
        self.table_disc.setStyleSheet(
            ...
            "QHeaderView::section { background-color: #160f22; color: #fbbf24; font-weight: bold; padding: 8px; }"
        ...
        for i, (n, d) in enumerate(items):
            ...
            if n in self.engine.discovered:
                ...
                xp_item.setForeground(QColor("#fbbf24"))
    ...
    def populate_datasheet(self, items):
        ...
        for i, (key, data) in enumerate(items):
            ...
            table.setItem(i, 3, QTableWidgetItem(str(data.get('state', 'جامد'))))
            table.setItem(i, 4, QTableWidgetItem(str(data.get('hazard', 'کم‌خطر'))))
            ...
            table.setItem(i, 5, color_cell)
            table.setItem(i, 6, QTableWidgetItem(str(data.get('molarity', '-'))))
            table.setItem(i, 7, QTableWidgetItem(str(data.get('pH', '-'))))
            table.setItem(i, 8, QTableWidgetItem(str(data.get('heat', '-'))))
    ...
    def action_add(self):
        try:
            ...
            self._log(msg)
            for w in warnings:
                self._log(f"<span style='color:#fb7185;'>{w}</span>")
            if overflow:
    ...
    def action_toggle_stirrer(self):
        ...
        if hasattr(self, 'lbl_stirrer_status'):
            ...
            self.lbl_stirrer_status.setStyleSheet(
                "color: #c4b5fd; font-weight: bold; font-size: 12px;" if on
                else "color: #c4b5fd; font-weight: bold; font-size: 12px;")
        if hasattr(self, 'gl_beaker') and self.gl_beaker is not None:
    ...
    def action_toggle_titration(self):
        if self.btn_titrate.isChecked():
            ...
            self.btn_titrate.setStyleSheet("background-color: #fb7185; color: #160f22;")
            ...
            self.btn_titrate.setStyleSheet("background-color: #818cf8; color: #160f22;")
            if hasattr(self, 'lbl_titration_status'):
    ...
    def _enforce_splitter_layout(self):
        ...
        if sp is None:
            return
        try:
            total = max(sp.width(), self.width() - 24, 1000)
            left = 320
            # حداقل ۳۸۰ برای بشر، ۳۲۰ برای تب‌ها
            mid = max(380, int(total * 0.42))
            right = max(320, total - left - mid)
            if left + mid + right > total:
                mid = max(380, total - left - right)
            sp.setSizes([left, mid, right])
            tlog("enforce splitter", "LAYOUT", sizes=sp.sizes(), total=total)
        except Exception as e:
        ...
        try:
            ...
            if not st:
                ...
                try:
                    ...
                    return
            dlg = MplMolecule3DDialog(structure=st, title=f"{name} — {formula}", parent=self)
            dlg.exec()
        except Exception as e:
    ...
    def game_loop(self):
        try:
            ...
            if crossed:
                ...
                analyte_vol_ml = max(1.0, self.engine.total_volume - vol)
                approx_conc = (titrant_M * (vol / 1000.0)) / (analyte_vol_ml / 1000.0) if analyte_vol_ml > 0 else 0
            ...
            try:
                if hasattr(self, 'tabs') and self.tabs.currentIndex() >= 0:
                    graph_tab_active = True
            except Exception:
                pass

            if self.engine.speed_multiplier > 0 and fi % 4 == 0:
                self.data_time.append(t)
                self.data_ph.append(ph)
                self.data_temp.append(temp)
                if len(self.data_time) > 60:
                ...
                try:
                    ...
                    pass
                if self.data_time:
                    t0, t1 = self.data_time[0], self.data_time[-1]
                    if t1 - t0 < 1:
                        t1 = t0 + 1
                    self.ax1.set_xlim(t0, t1 + 0.5)
                    self.ax1.set_ylim(0, 14)
                    self.ax2.set_xlim(t0, t1 + 0.5)
                    lo = min(self.data_temp) - 5
                    hi = max(self.data_temp) + 5
                    if hi - lo < 10:
                        hi = lo + 10
                    self.ax2.set_ylim(lo, hi)
                if graph_tab_active and fi % 15 == 0:
                    try:
                        self.canvas.draw_idle()
                    except Exception:
                        pass
