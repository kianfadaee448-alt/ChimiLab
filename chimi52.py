...
def get_lab3d_html_path():
    """مسیر فایل وب سه‌بعدی (lab3d.html) کنار برنامه یا artifacts."""
    base = get_app_base_dir()
    candidates = [
        os.path.join(base, "lab3d.html"),
        os.path.join(base, "artifacts", "lab3d.html"),
        os.path.join(os.path.abspath("."), "lab3d.html"),
        os.path.join(os.path.abspath("."), "artifacts", "lab3d.html"),
    ]
    for p in candidates:
        if os.path.isfile(p):
...
if HAS_OPENGL:
    class GLLabWorld(QOpenGLWidget):
        ...
        def resizeGL(self, w, h):
            ...
            glMatrixMode(GL_PROJECTION)
            glLoadIdentity()
            gluPerspective(72.0, max(0.1, w / max(1, h)), 0.08, 60.0)
            glMatrixMode(GL_MODELVIEW)
        ...
        def get_hint(self):
            if self.hovered:
                return f"E — تعامل با «{self.hovered['title']}»"
            return "WASD حرکت | ماوس نگاه | Shift دویدن | E تعامل"
...
class ModernLabWindow(QMainWindow):
    ...
    def _create_left_panel(self):
        ...
        outer = QFrame()
        DimensionController.lock(outer, w=380, min_w=320, max_w=420)
        ...
        btn_add.setStyleSheet("background-color: #c4b5fd; color: #160f22; font-weight: bold; border-radius: 7px;")
        btn_drag = QPushButton("🖐️ کشیدن")
        btn_drag.setMinimumHeight(34)
        btn_drag.setToolTip("بزنید و روی ظرف رها کنید")
        btn_drag.setStyleSheet("background-color: #fbbf24; color: #160f22; font-weight: bold; border-radius: 7px;")
        btn_drag.clicked.connect(self.start_chem_drag)
        self.btn_titrate = QPushButton("💧 بورت")
        self.btn_titrate.setMinimumHeight(34)
        self.btn_titrate.setCheckable(True)
        self.btn_titrate.clicked.connect(self.action_toggle_titration)
        self.btn_titrate.setStyleSheet("background-color: #818cf8; color: #160f22; font-weight: bold; border-radius: 7px;")
        h_btn.addWidget(btn_add)
        h_btn.addWidget(btn_drag)
        h_btn.addWidget(self.btn_titrate)
        self.btn_mol3d = QPushButton("🧬 ۳بعدی ماده")
        self.btn_mol3d.setMinimumHeight(34)
        self.btn_mol3d.setToolTip("نمایش سه‌بعدی مولکول مادهٔ انتخاب‌شده (Matplotlib آفلاین)")
        self.btn_mol3d.setStyleSheet("background-color: #34d399; color: #0c0914; font-weight: bold; border-radius: 7px;")
        self.btn_mol3d.clicked.connect(self.show_selected_molecule_3d)
        h_btn.addWidget(self.btn_mol3d)
    ...
    def create_lab3d_tab(self):
        ...
        lay.addWidget(self.lab3d_hint)
        self.lab3d_world = None
        self.lab3d_web = None
        self._lab3d_mode = None

        html_path = get_lab3d_html_path()
        if HAS_WEBENGINE and os.path.isfile(html_path):
            try:
                ...
                try:
                    ...
                    pass
                page = self.lab3d_web.page()
                try:
                    page.javaScriptConsoleMessage = self._lab3d_console  # may not stick; use signal if available
                except Exception:
                    pass
                try:
                    # Qt6: javaScriptConsoleMessage is a method to override — connect via profile is hard;
                    # use periodic poll + inject hook instead
                    pass
                except Exception:
                    pass
                ...
                self.lab3d_hint.setText(f"وب سه‌بعدی: {os.path.basename(html_path)} — کلیک برای ورود")
                # inject bridge poll timer
        ...
        if self.lab3d_web is None:
            try:
                ...
                self.lab3d_hint.setText("OpenGL داخلی — WASD | ماوس | E")
                ...
                lay.addWidget(QLabel(
                    f"نمای سه‌بعدی در دسترس نیست.\n"
                    f"فایل lab3d.html را کنار برنامه بگذارید و PySide6-WebEngine نصب کنید.\n{e}"
                ))
                self._lab3d_mode = None

        help_l = QLabel(
            "چهار فایل پروژه: ① db.db  ② JSON ذخیره  ③ chimi52.py  ④ lab3d.html  |  "
            "ایستگاه‌ها: هات‌پلیت/بنزن=گرما · یخ=سرما · بورت=تیتراسیون · فیلتر · سینک=شستشو"
        )
    ...
    def resizeEvent(self, event):
        ...
        try:
            ...
            if hasattr(self, 'main_splitter'):
                ...
                if len(sizes) >= 3 and (sizes[0] < 200 or sizes[1] < 200 or sizes[2] < 150):
                    total = max(sum(sizes), self.width() - 20, 900)
                    fixed_left = min(400, max(360, sizes[0] if sizes[0] >= 200 else 400))
                    rest = total - fixed_left
                    mid = max(300, int(rest * 0.45))
                    right = max(250, rest - mid)
                    self.main_splitter.setSizes([fixed_left, mid, right])
                    tlog("اسپلیتر اصلاح شد (پنل صفر)", "LAYOUT", sizes=self.main_splitter.sizes())
    ...
    def open_molecule_3d(self, formula, name=None):
        """نمایش ۳بعدی برای هر ماده/ترکیب — ساختار دقیق یا تقریبی خودکار (از نسخه ۵۴)."""
        try:
    ...
    def show_selected_molecule_3d(self):
        """مادهٔ انتخاب‌شده در کمبو."""
        k = self.combo_chem.currentData() if hasattr(self, "combo_chem") else None
        if not k or k not in CHEMILAB_DB:
            QMessageBox.information(self, "۳بعدی", "ابتدا یک ماده انتخاب کنید.")
            return
        d = CHEMILAB_DB[k]
        self.open_molecule_3d(d.get("formula") or k, d.get("name"))
