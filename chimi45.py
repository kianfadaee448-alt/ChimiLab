...
try:
    ...
    from PyQt5.QtGui import QFont, QColor, QPainter, QPen, QLinearGradient, QPainterPath, QBrush, QRadialGradient, \
        QPixmap, QPdfWriter, QPageSize, QImage, QPainter as QPainterGui
...
def is_solid_chemical_type(ctype):
    if not ctype:
        return False
    ctype = str(ctype)
    # اگر مایع یا گاز باشد و جامد نباشد، false برمی‌گرداند
...
class LabEngine:
    ...
    def filter_solids(self):
        ...
        for layer in self.visual_layers:
            ...
            # تشخیص بهتر جامد بودن
            is_solid = is_solid_chemical_type(combined_type)
            # همچنین اگر type خود لایه شامل کلمات جامد باشد
    ...
    def get_mixture_empirical_formula(self):
        ...
        # حذف اتم‌هایی که مقدار بسیار کمی دارند
...
class AnimatedContainer(QWidget):
    ...
    def set_stirrer(self, state):
        ...
        if not state:
            self.stirrer_angle = 0.0  # تنظیم زاویه به حالت افقی
        ...
        if not self.engine.is_broken:
            ...
            if total_amount > 0:
                for layer in sorted(self.engine.visual_layers, key=layer_density, reverse=True):
                    ...
                    # نمایش نام ماده و فرمول روی لایه
            ...
            if total_amount > 0 and self.engine.visual_layers:
                ...
                # اگر خاموش باشد، زاویه صفر است (افقی)
            ...
            if self.engine.temp_c < 0:
                frost_intensity = min(1.0, abs(self.engine.temp_c) / 20.0)
                frost_alpha = int(80 + 120 * frost_intensity)
                painter.setPen(QPen(QColor(180, 220, 255, frost_alpha), 1))
                # الگوی کریستال یخ
                for i in range(int(20 + 40 * frost_intensity)):
                    x = container_rect.left() + random.uniform(10, container_rect.width() - 10)
                    y = container_rect.top() + random.uniform(10, container_rect.height() - 10)
                    size = random.uniform(3, 8 + 10 * frost_intensity)
                    # رسم ستاره‌های یخ
                    painter.save()
                    painter.translate(x, y)
                    for j in range(6):
                        angle = j * 60 + random.uniform(-5, 5)
                        painter.drawLine(0, 0, size * math.cos(math.radians(angle)),
                                         size * math.sin(math.radians(angle)))
                    painter.restore()
                # لبه‌های یخ زده روی ظرف
                painter.setPen(QPen(QColor(200, 240, 255, int(60 + 100 * frost_intensity)), 3))
...
class ModernLabWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        right_tabs = self._create_all_tabs()

        splitter = QSplitter(Qt.Horizontal)
        splitter.addWidget(left_panel)
        splitter.addWidget(center_panel)
        splitter.addWidget(right_tabs)
        splitter.setSizes([400, 450, 700])
        layout.addWidget(splitter)
    ...
    def _create_left_panel(self):
        ...
        btn_theme.setStyleSheet("background-color: #45475a; color: #f9e2af; font-weight: bold; border-radius: 8px;")
    ...
    def create_missions_badges_tab(self):
        ...
        self.badge_widgets = []
        # پر کردن مدال‌ها بعداً در update انجام می‌شود
    ...
    def create_wiki_tab(self):
        """تب دانشنامه با نمایش تصاویر محصولات به جای جدول"""
        w = QWidget()
        layout = QVBoxLayout(w)
        ...
        # نمایش واکنش‌های کشف شده به صورت کارت
        self.wiki_cards = []
        for name, rxn in CUSTOM_REACTIONS.items():
            card = QFrame()
            card.setStyleSheet("""
            ...
            if name in self.engine.discovered:
                ...
                status_lbl = QLabel("❓ کشف نشده")
                status_lbl.setStyleSheet("color: #f38ba8; font-weight: bold;")
    ...
    def show_product_detail(self, name, color):
        """نمایش جزئیات محصول در یک دیالوگ"""
        dlg = QDialog(self)
    ...
    def update_wiki_tab(self):
        """به‌روزرسانی تب دانشنامه"""
        # برای سادگی، تب را دوباره می‌سازیم
        index = self.tabs.indexOf(self.tabs.widget(7))  # دانشنامه در ایندکس 7
        if index >= 0:
    ...
    def action_screenshot(self):
        ...
        if path:
            ...
            # نام مواد روی لایه‌ها قبلاً در paintEvent رسم می‌شود
            painter.end()
            pixmap.save(path)
    ...
    def action_export_pdf(self):
        ...
        try:
            ...
            y = 800
            line_height = 150
            info = [f"شیمیدان: {self.engine.player_name} (سطح {self.engine.level})",
            ...
            painter.drawText(QRectF(100, y, w - 200, h - y - 100), Qt.AlignRight | Qt.TextWordWrap, notes)

            # مدال‌ها و مأموریت‌ها در انتهای PDF
            y = h - 300
            painter.drawText(QRectF(100, y, w - 200, line_height), Qt.AlignRight, "🏅 مدال‌ها:")
            y += line_height
            badges_text = ", ".join([f"{BADGE_CATALOG.get(b, ('', ''))[0]} {b}" for b in self.engine.badges]) or "—"
            painter.drawText(QRectF(200, y, w - 300, line_height * 2), Qt.AlignRight | Qt.TextWordWrap, badges_text)
            y += line_height * 2

            painter.drawText(QRectF(100, y, w - 200, line_height), Qt.AlignRight, "🎯 مأموریت‌های تکمیل شده:")
            y += line_height
            missions_text = ", ".join([m['title'] for m in self.engine.missions if m['id'] in self.engine.completed_missions]) or "—"
            painter.drawText(QRectF(200, y, w - 300, line_height), Qt.AlignRight, missions_text)

            painter.end()
            QMessageBox.information(self, "موفق", "گزارش PDF با موفقیت ذخیره شد.")
        except Exception as e:
