...
class AdminAuth:
    ...
    def change_password(old_password: str, new_password: str) -> tuple:
        ...
        if len((new_password or "").strip()) < 4:
            return False, "رمز جدید باید حداقل ۴ کاراکتر باشد."
...
def save_chemilab_to_db():
    ...
    try:
        ...
        for col, default in (("state", "جامد"), ("hazard", "کم‌خطر — احتیاط معمولی")):
            try:
        ...
        for key, d in CHEMILAB_DB.items():
            cur.execute(
                "INSERT INTO chemilab (name, fa_name, type, pH, molarity, heat, color, formula, state, hazard) VALUES (?,?,?,?,?,?,?,?,?,?)",
                (
                    str(d.get("name", "")),
                    str(key),
                    str(d.get("type", "")),
...
def save_reactions_to_db():
    ...
    try:
        ...
        for name, rxn in CUSTOM_REACTIONS.items():
            cur.execute(
                "INSERT INTO custom_reactions (fa_name, reactants, products, desc, xp, temp_min) VALUES (?,?,?,?,?,?)",
                (
                    str(name),
...
class AppLogger:
    ...
    def __init__(self, filename="chimi48_lab.log"):
...
class ChemicalCalculator:
    ...
    def reduce_slots(atom_amounts):
        """تقسیم بر کوچک‌ترین مقدار و گرد کردن به نسبت صحیح ساده"""
        ...
        ratios = {el: val / min_v for el, val in filtered.items()}
        best, best_err = None, 1e9
        for mult in range(1, 13):
            rounded = {el: max(1, int(round(r * mult))) for el, r in ratios.items()}
            err = sum(abs(ratios[el] * mult - rounded[el]) for el in ratios)
            if err < best_err:
                best_err = err
                best = rounded
        if not best:
            return {}
        g = _reduce(gcd, list(best.values()))
        if g > 1:
            best = {el: n // g for el, n in best.items()}
        return best
...
class LabEngine:
    ...
    def load_data(self):
        ...
        try:
            ...
            self.level = max(1, safe_int(d.get("level", 1), 1))
            ...
            # بازگردانی وضعیت ظرف (اگر ذخیره شده بود)
            if "temp_c" in d:
                self.temp_c = safe_float(d.get("temp_c"), 25.0)
            if isinstance(d.get("contents"), dict):
                self.contents = {str(k): safe_float(v, 0.0) for k, v in d["contents"].items()}
            if "total_volume" in d:
                self.total_volume = safe_float(d.get("total_volume"), 0.0)
            if "moles_h" in d:
                self.moles_h = safe_float(d.get("moles_h"), 0.0)
            if "moles_oh" in d:
                self.moles_oh = safe_float(d.get("moles_oh"), 0.0)
    ...
    def save_data(self):
        ...
        try:
            ...
            # نوشتن اتمی: اول فایل موقت سپس rename — جلوگیری از خراب شدن فایل
            tmp = save_path + ".tmp"
            ...
            print(f"[SAVE][ERROR] ذخیره lab_save.json ناموفق ({save_path}): {e}", flush=True)
            try:
...
class LoginDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        sub = QLabel("Universe ChimiLab V48  •  خراسان رضوی")
...
class ModernLabWindow(QMainWindow):
    def __init__(self, player_name="دانشجو", is_admin=False):
        ...
        self.setWindowTitle(f"شیمی‌لَب V48 — {role}: {player_name}")
    ...
    def start_simulation(self):
        self.timer.start(80)  # 12.5 fps — سبک‌تر و پایدارتر
    ...
    def _setup_admin_ui(self):
        ...
        top = QHBoxLayout()
        title = QLabel("🛡️ شیمی‌لَب V48 — پنل مدیریت دیتابیس")
        ...
        self.lbl_admin_lock.setStyleSheet("color: #a6e3a1; font-weight: bold; font-size: 13px;")
    ...
    def setup_ui(self):
        ...
        self.lbl_top_status = QLabel("شیمی‌لَب V48")
    ...
    def show_tutorial(self):
        dlg = QDialog(self)
        dlg.setWindowTitle("راهنما — شیمی‌لَب V48")
        ...
        def open_mail():
            from PySide6.QtGui import QDesktopServices
            from PySide6.QtCore import QUrl
            QDesktopServices.openUrl(QUrl("mailto:kianfadaee448@gmail.com?subject=ChimiLab%20V48"))
    ...
    def _create_left_panel(self):
        ...
        self.txt_log.setStyleSheet(
            "QTextEdit { background-color: #11111b; color: #a6e3a1; font-size: 12px; "
            "border: 1px solid #313244; border-radius: 8px; padding: 4px; }")
    ...
    def open_profile_dialog(self):
        ...
        self._dlg_lbl_score.setStyleSheet("color: #a6e3a1; font-size: 14px; font-weight: bold;")
    ...
    def _admin_rxn_fill_form(self):
        ...
        self.adm_r_temp.setValue(float(rxn.get("temp_min", 0) or 0))
    ...
    def admin_chem_upsert(self):
        ...
        CHEMILAB_DB[key] = {
            ...
            "pH": self.adm_c_ph.value(),
            "molarity": self.adm_c_mol.value(),
            "heat": self.adm_c_heat.value(),
    ...
    def create_about_tab(self):
        ...
        btn_mail.setStyleSheet(
            "background: #89b4fa; color: #1e1e2e; font-weight: bold; padding: 10px 18px; border-radius: 10px;"
        )
        def open_mail():
            try:
                from PySide6.QtGui import QDesktopServices
    ...
    def create_datasheet_tab(self):
        ...
        btn_hide.setStyleSheet("background:#f38ba8; color:#1e1e2e; font-weight:bold; padding:6px 10px; border-radius:8px;")
        ...
        btn_restore.setStyleSheet("background:#a6e3a1; color:#1e1e2e; font-weight:bold; padding:6px 10px; border-radius:8px;")
        ...
        btn_restore_one.setStyleSheet("background:#89b4fa; color:#1e1e2e; font-weight:bold; padding:6px 10px; border-radius:8px;")
        ...
        self.datasheet_table.setStyleSheet(
            ...
            "QHeaderView::section { background-color: #1e1e2e; padding: 8px; border: 1px solid #313244; "
    ...
    def datasheet_show_hidden_dialog(self):
        ...
        btn.setStyleSheet("background:#a6e3a1; color:#1e1e2e; font-weight:bold; padding:8px; border-radius:8px;")
        def restore_sel():
    ...
    def action_export_pdf(self):
        ...
        try:
            ...
            painter.drawText(QRectF(margin, y + 160, w - 2 * margin, 70), Qt.AlignHCenter, "شیمی‌لَب  •  Universe ChimiLab V48")
            ...
            painter.drawText(QRectF(margin, h - 70, w - 2 * margin, 40), Qt.AlignHCenter,
                             f"شیمی‌لَب (Universe ChimiLab V48)  |  صفحه {page_num[0]}  |  تولید خودکار گزارش")
