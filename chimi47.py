...
class LoginDialog(QDialog):
    """پنجره ورود — فقط نام. اگر نام admin باشد وارد پنل مدیر می‌شود."""
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("شیمی‌لَب — ورود")
        ...
        title = QLabel("🧪 شیمی‌لَب")
        title.setAlignment(Qt.AlignCenter)
        title.setStyleSheet("font-size: 26px; font-weight: bold; color: #c4b5fd; margin-bottom: 4px;")
        layout.addWidget(title)

        sub = QLabel("Universe ChimiLab")
        sub.setAlignment(Qt.AlignCenter)
        sub.setStyleSheet("font-size: 13px; color: #a78bfa; font-weight: normal;")
        layout.addWidget(sub)

        layout.addSpacing(8)
        lbl = QLabel("نام کاربری:")
        ...
        hint = QLabel("برای شروع فقط نام خود را بنویسید")
    ...
    def check_input(self):
        ...
        if not name:
            ...
            self.name_input.setStyleSheet(
                "border: 2px solid #f38ba8; background: #1e1530; color: #c4b5fd;"
                "padding: 12px 16px; border-radius: 12px; font-size: 15px;")
            return
        self._is_admin = (name.lower() == "admin")
        self.accept()
...
class ModernLabWindow(QMainWindow):
    def __init__(self, player_name="دانشجو", is_admin=False):
        ...
        self.setWindowTitle(f"شیمی‌لَب V47 — {role}: {player_name}")
        ...
        if not self.is_admin and (self.engine.score == 0):
    ...
    def request_logout(self):
        """خروج از حساب با تأیید"""
        if self.is_admin:
            msg = ("آیا از حساب مدیر خارج می‌شوید؟\n"
                   "تغییرات دیتابیس/واکنش‌ها ماندگار می‌مانند.\n"
                   "نشست فعلی بسته می‌شود.")
        else:
            msg = ("آیا از حساب کاربری خارج می‌شوید؟\n"
                   "تمام پیشرفت، امتیاز و یادداشت این کاربر پاک خواهد شد.")
        ...
        try:
            ...
            pass
        if self.is_admin:
            # مدیر: فقط فایل نشست نام را پاک کن؛ داده ساختاری حفظ
            try:
                self.engine.player_name = "admin"
                self.engine.save_data()
            except Exception:
                pass
        else:
            # دانش‌آموز: پاکسازی کامل
            try:
                sp = get_save_path()
            ...
            try:
                self.engine.score = 0
                self.engine.level = 1
                self.engine.discovered = set()
                self.engine.badges = set()
                self.engine.completed_missions = set()
                self.engine.notes = ""
                self.engine.player_name = "دانشجو"
                self.engine.reset()
                self.engine.stats = {
    ...
    def closeEvent(self, event):
        try:
            if not self._logout_requested and not self.is_admin:
                self.engine.save_data()
            elif self.is_admin:
                self.engine.save_data()
        except Exception:
    ...
    def setup_ui(self):
        ...
        self.lbl_top_status = QLabel("شیمی‌لَب V47")
    ...
    def _create_left_panel(self):
        ...
        h_top = QHBoxLayout()
        role_txt = "🛡️ مدیر" if self.is_admin else "👤 شیمیدان"
        self.lbl_welcome = QLabel(f"{role_txt}: {self.engine.player_name}")
        self.lbl_welcome.setStyleSheet("color: #c4b5fd; font-size: 15px; font-weight: bold;")
    ...
    def _create_all_tabs(self):
        ...
        if self.is_admin:
            self.tabs.addTab(self.create_admin_dashboard_tab(), "🛡️ پنل مدیر")
            ...
            self.tabs.addTab(self.create_notes_tab(), "📝 لاگ سیستم")
    ...
    def create_admin_dashboard_tab(self):
        w = QWidget()
        lay = QVBoxLayout(w)
        title = QLabel("🛡️ داشبورد مدیر سیستم")
        title.setStyleSheet("font-size: 20px; font-weight: bold; color: #c4b5fd; margin: 12px;")
        lay.addWidget(title)
        info = QLabel(
            f"مواد در دیتابیس: {len(CHEMILAB_DB)}\n"
            f"واکنش‌های تعریف‌شده: {len(CUSTOM_REACTIONS)}\n"
            f"مسیر دیتابیس: {get_db_path()}\n"
            f"مسیر ذخیره: {get_save_path()}\n\n"
            "از تب‌های «مدیریت مواد» و «مدیریت واکنش‌ها» برای ویرایش استفاده کنید.\n"
            "دانش‌آموزان به این بخش دسترسی ندارند."
        )
        info.setStyleSheet(
            "font-size: 14px; line-height: 1.8; color: #e8e0f5; background: #1a1228;"
            "border: 1px solid #5b3d7a; border-radius: 12px; padding: 16px;")
        info.setWordWrap(True)
        lay.addWidget(info)
        btn_reload = QPushButton("🔄 بارگذاری مجدد دیتابیس")
    ...
    def create_admin_db_tab(self):
        w = QWidget()
        lay = QVBoxLayout(w)
        lay.addWidget(QLabel("🗄️ فهرست مواد (فقط مشاهده امن — ویرایش مستقیم از SQLite)"))
        table = QTableWidget()
        table.setColumnCount(5)
        table.setHorizontalHeaderLabels(["کلید", "نام", "فرمول", "نوع", "pH"])
        ...
        for i, (k, d) in enumerate(items):
            ...
            table.setItem(i, 1, QTableWidgetItem(str(d.get("name", ""))))
            table.setItem(i, 2, QTableWidgetItem(str(d.get("formula", ""))))
            table.setItem(i, 3, QTableWidgetItem(str(d.get("type", ""))))
            table.setItem(i, 4, QTableWidgetItem(str(d.get("pH", ""))))
        lay.addWidget(table)
        note = QLabel("⚠️ برای امنیت، دانش‌آموزان نمی‌توانند دیتابیس را تغییر دهند. فایل db.db فقط از طریق مدیر سیستم قابل مدیریت است.")
    ...
    def create_admin_reactions_tab(self):
        w = QWidget()
        lay = QVBoxLayout(w)
        lay.addWidget(QLabel("⚗️ واکنش‌های تعریف‌شده در سیستم"))
        table = QTableWidget()
        table.setColumnCount(4)
        table.setHorizontalHeaderLabels(["نام", "واکنش‌دهنده‌ها", "محصولات", "XP"])
        table.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
        ...
        for i, (name, rxn) in enumerate(items):
            ...
            table.setItem(i, 1, QTableWidgetItem(", ".join(rxn.get("reactants", []))))
            table.setItem(i, 2, QTableWidgetItem(", ".join(rxn.get("products", []))))
            table.setItem(i, 3, QTableWidgetItem(str(rxn.get("xp", 0))))
        lay.addWidget(table)
        return w
    ...
    def admin_reload_db(self):
        try:
            ...
            load_databases()
            QMessageBox.information(self, "مدیر", f"بارگذاری شد: {len(CHEMILAB_DB)} ماده، {len(CUSTOM_REACTIONS)} واکنش")
        except Exception as e:
