...
#مول = molarity × (حجم به لیتر) = 0.1 × (50 / 1000) = 0.005 مول
...
class ChemicalCalculator:
    ...
    def calculate_empirical_from_moles(atom_moles_counter):
        """محاسبه فرمول تجربی دقیق با تبدیل نسبت‌های اعشاری به صحیح"""
        if not atom_moles_counter:
            return "ماده‌ای وجود ندارد"

        # حذف مقادیر خطای محاسباتی
        ...
        # تلاش برای یافتن ضریب صحیح (از 1 تا 10) برای رفع اعشار
        # مثلا اگر نسبت 1 به 1.5 باشد، ضریب 2 باعث می‌شود بشود 2 به 3
        best_multiplier = 1
        best_error = 1.0  # خطای مجاز اولیه
        ...
        for m in range(1, 11):
            ...
            for r in ratios.values():
                val = r * m
                # فاصله تا نزدیک‌ترین عدد صحیح
                diff = abs(val - round(val))
                if diff > 0.1:  # اگر خیلی دور بود، این ضریب مناسب نیست
                    is_valid = False
                    break
                current_error += diff

            if is_valid:
                # اگر ضریب مناسب پیدا شد، استفاده می‌کنیم و حلقه را می‌شکنیم
                best_multiplier = m
                break

        # مرتب‌سازی عناصر (کربن، هیدروژن، سپس بقیه)
        sorted_elements = []
