...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout,
                             ...
                             QTextEdit, QGroupBox, QFormLayout, QMessageBox, QDoubleSpinBox)
...
class DBManagerApp(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("مدیریت پایگاه داده شیمی‌لَب")
        self.resize(750, 750)

        # اعمال استایل تاریک درخواستی
        self.setStyleSheet(APP_STYLE)
        self.setLayoutDirection(Qt.RightToLeft)

        self.db_path = os.path.join(os.path.abspath("."), "db.db")
        self.init_ui()
    ...
    def init_ui(self):
        ...
        # -- بخش اول: مسیر دیتابیس --
        path_layout = QHBoxLayout()
        self.txt_path = QLineEdit(self.db_path)
        btn_update_path = QPushButton("تنظیم مسیر")
        btn_update_path.clicked.connect(self.update_path)
        path_layout.addWidget(QLabel("مسیر فایل db.db:"))
        path_layout.addWidget(self.txt_path)
        path_layout.addWidget(btn_update_path)
        layout.addLayout(path_layout)

        # -- بخش دوم: ساخت دیتابیس از روی دیکشنری --
        gb_auto = QGroupBox("عملیات خودکار (Import و Reset)")
        v_auto = QVBoxLayout(gb_auto)
        lbl_auto = QLabel(
            "با کلیک روی دکمه زیر، تمام داده‌های قبلی دیتابیس پاک شده (Reset) و اطلاعات دیکشنری‌های داخل این کد جایگزین می‌شوند.")
        lbl_auto.setWordWrap(True)
        btn_build = QPushButton("📥 ریست و بازسازی دیتابیس از روی دیکشنری‌ها")
        btn_build.setStyleSheet("background-color: #a6d189; color: #232634;")  # کمی تغییر رنگ برای تناسب با تم دارک
        btn_build.clicked.connect(self.build_db_from_dicts)
        v_auto.addWidget(lbl_auto)
        v_auto.addWidget(btn_build)
        layout.addWidget(gb_auto)

        # -- بخش سوم: فرم افزودن ماده جدید --
        gb_chem = QGroupBox("افزودن ماده شیمیایی جدید به دیتابیس فعلی")
        f_chem = QFormLayout(gb_chem)

        self.c_name = QLineEdit()
        self.c_fa_name = QLineEdit()
        self.c_fa_name.setPlaceholderText("مثلاً: h2so4 (با حروف کوچک انگلیسی)")
        self.c_type = QLineEdit()
        self.c_ph = QDoubleSpinBox()
        self.c_ph.setRange(0, 14)
        self.c_ph.setValue(7.0)
        self.c_molarity = QDoubleSpinBox()
        self.c_molarity.setRange(0, 100)
        self.c_molarity.setValue(0.1)
        self.c_heat = QDoubleSpinBox()
        self.c_heat.setRange(-2000, 2000)
        self.c_heat.setValue(0.0)
        self.c_color = QLineEdit("#FFFFFF")
        self.c_formula = QLineEdit()

        f_chem.addRow("نام فارسی:", self.c_name)
        f_chem.addRow("کلید (نام انگلیسی بدون فاصله):", self.c_fa_name)
        f_chem.addRow("فرمول شیمیایی:", self.c_formula)
        f_chem.addRow("نوع ماده (انگلیسی):", self.c_type)
        f_chem.addRow("مقدار pH:", self.c_ph)
        f_chem.addRow("مولاریته:", self.c_molarity)
        f_chem.addRow("آنتالپی / گرما:", self.c_heat)
        f_chem.addRow("کد رنگ (HEX):", self.c_color)

        btn_add_chem = QPushButton("➕ افزودن ماده")
        btn_add_chem.clicked.connect(self.add_single_chemical)
        f_chem.addRow(btn_add_chem)
        layout.addWidget(gb_chem)
    ...
    def update_path(self):
        self.db_path = self.txt_path.text()
        self.log.append(f"مسیر دیتابیس تنظیم شد: {self.db_path}")

    def init_tables(self, cursor):
        # ساخت مجدد جداول با ساختار کامل (شامل temp_min)
        cursor.execute('''CREATE TABLE IF NOT EXISTS chemilab (
        ...
        cursor.execute('''CREATE TABLE IF NOT EXISTS custom_reactions (
                            id INTEGER PRIMARY KEY AUTOINCREMENT,
                            fa_name TEXT UNIQUE,
                            reactants TEXT,
                            products TEXT,
    ...
    def build_db_from_dicts(self):
        # درخواست تایید قبل از پاک کردن اطلاعات
        reply = QMessageBox.question(self, 'هشدار ریست دیتابیس',
                                     'آیا مطمئن هستید؟ این کار جداول قبلی را کاملا پاک کرده و داده‌های پیش‌فرض داخل کد را از نو می‌سازد.',
                                     QMessageBox.Yes | QMessageBox.No, QMessageBox.No)

        if reply == QMessageBox.No:
            self.log.append("عملیات بازسازی توسط کاربر لغو شد.")
            return
        ...
        try:
            ...
            # --- بخش جدید: پاک کردن جداول قدیمی (Reset) ---
            cursor.execute("DROP TABLE IF EXISTS chemilab")
            cursor.execute("DROP TABLE IF EXISTS custom_reactions")
            self.log.append("جداول قدیمی حذف شدند (Reset).")
            # ---------------------------------------------

            # ساخت مجدد جداول (این بار چون جداول پاک شده اند، ستون temp_min قطعا ساخته میشود)
            self.init_tables(cursor)

            c_count = 0
            for key, data in CHEMILAB_DICT.items():
                try:
                    cursor.execute('''INSERT INTO chemilab 
                        (name, fa_name, type, pH, molarity, heat, color, formula) 
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?)''',
                                   (data['name'], key, data['type'], data.get('pH', 7.0),
                                    data.get('molarity', 0.1), data.get('heat', 0.0),
                                    data.get('color', '#fff'), data['formula']))
                    c_count += 1
                except Exception as e:
                    self.log.append(f"خطا در افزودن ماده {key}: {e}")

            r_count = 0
            for name, data in REACTIONS_DICT.items():
                try:
                    r_str = json.dumps(data['reactants'], ensure_ascii=False)
                    p_str = json.dumps(data['products'], ensure_ascii=False)
                    cursor.execute('''INSERT INTO custom_reactions 
                        (fa_name, reactants, products, desc, xp, temp_min) 
                        VALUES (?, ?, ?, ?, ?, ?)''',
                                   (name, r_str, p_str, data.get('desc', ''),
                                    data.get('xp', 50), data.get('temp_min', 0.0)))
                    r_count += 1
                except Exception as e:
                    self.log.append(f"خطا در افزودن واکنش {name}: {e}")

            conn.commit()
            conn.close()
            self.log.append(f"✅ موفقیت: {c_count} ماده و {r_count} واکنش به دیتابیس افزوده شد.")
            QMessageBox.information(self, "موفق", "دیتابیس با موفقیت ریست و بازسازی شد!")
        except Exception as e:
            self.log.append(f"❌ خطای کلی دیتابیس: {e}")

    def add_single_chemical(self):
        fa_name = self.c_fa_name.text().strip().lower()
        name = self.c_name.text().strip()
        if not fa_name or not name:
            QMessageBox.warning(self, "خطا", "نام و کلید انگلیسی الزامی است.")
            return

        try:
            conn = sqlite3.connect(self.db_path)
            cursor = conn.cursor()
            # اگر دیتابیس وجود نداشت اول بسازد
            self.init_tables(cursor)

            cursor.execute('''INSERT OR REPLACE INTO chemilab 
                (name, fa_name, type, pH, molarity, heat, color, formula) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)''',
                           (name, fa_name, self.c_type.text(), self.c_ph.value(),
                            self.c_molarity.value(), self.c_heat.value(),
                            self.c_color.text(), self.c_formula.text()))

            conn.commit()
            conn.close()
            self.log.append(f"✅ ماده جدید '{name}' با موفقیت به دیتابیس افزوده شد.")
            QMessageBox.information(self, "موفق", "ماده اضافه شد!")

            # پاک کردن فرم
            self.c_name.clear()
            self.c_fa_name.clear()
            self.c_formula.clear()
        except Exception as e:
            self.log.append(f"❌ خطا در ذخیره: {e}")
