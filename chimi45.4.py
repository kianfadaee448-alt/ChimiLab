...
if HAS_OPENGL:
    class GLBeakerCanvas(QOpenGLWidget):
        ...
        def paintGL(self):
            ...
            # --- مایع داخل بشر بر اساس visual_layers موتور ---
            if self.engine and not self.engine.is_broken and self.engine.total_volume > 0:
                max_cap = max(1.0, float(self.engine.max_capacity))
                fill_ratio = min(1.0, float(self.engine.total_volume) / max_cap)
                liquid_height = (y_top - y_bottom) * fill_ratio
                ...
                layers = sorted(self.engine.visual_layers, key=layer_density, reverse=True)
                current_y = y_bottom
                glDisable(GL_LIGHTING)
                for layer in layers:
                    amt = float(layer.get('amount', 0))
                    if amt <= 0:
                        continue
                    lh = (amt / max_cap) * (y_top - y_bottom)
                    if current_y + lh > y_bottom + liquid_height:
                        lh = max(0.01, y_bottom + liquid_height - current_y)
                    if lh <= 0:
                        continue
                    r, g, b = self._hex_to_rgb(layer.get('color', '#88ccff'))
                    # استوانه مایع (نوار)
                    steps = 24
                    ...
                    for i in range(steps + 1):
                        ...
                        # شعاع متناسب با ارتفاع در بشر مخروطی
                        t0 = (current_y - y_bottom) / max(0.01, y_top - y_bottom)
                        t1 = (current_y + lh - y_bottom) / max(0.01, y_top - y_bottom)
                        rad0 = r_bottom + (r_top - r_bottom) * t0
                        rad1 = r_bottom + (r_top - r_bottom) * t1
                        x0 = math.cos(ang) * rad0 * 0.92
                        z0 = math.sin(ang) * rad0 * 0.92
                        x1 = math.cos(ang) * rad1 * 0.92
                        z1 = math.sin(ang) * rad1 * 0.92
                        glVertex3f(x0, current_y, z0)
                        glVertex3f(x1, current_y + lh, z1)
                    glEnd()
                    # سطح بالای لایه
                    glColor4f(min(1.0, r + 0.15), min(1.0, g + 0.15), min(1.0, b + 0.15), 0.85)
                    ...
                    for i in range(steps + 1):
                        ...
                        t1 = (current_y + lh - y_bottom) / max(0.01, y_top - y_bottom)
                        rad1 = r_bottom + (r_top - r_bottom) * t1
                        glVertex3f(math.cos(ang) * rad1 * 0.92, current_y + lh, math.sin(ang) * rad1 * 0.92)
                    glEnd()
                    current_y += lh
                glEnable(GL_LIGHTING)
            ...
            # افکت سرما: هاله آبی روی بشر
            if self.engine and self.engine.temp_c < 5.0:
                intensity = min(1.0, max(0.1, (5.0 - self.engine.temp_c) / 25.0))
                glDisable(GL_LIGHTING)
                glColor4f(0.6, 0.85, 1.0, 0.12 + 0.25 * intensity)
                glBegin(GL_QUAD_STRIP)
                for i in range(37):
                    ang = math.radians(i * 10)
                    x = math.cos(ang) * (r_bottom + 0.08)
                    z = math.sin(ang) * (r_bottom + 0.08)
                    glVertex3f(x, y_bottom, z)
                    glVertex3f(x * (r_top / r_bottom), y_top, z * (r_top / r_bottom))
                glEnd()
                glEnable(GL_LIGHTING)
...
class AnimatedContainer(QWidget):
    ...
    def process_animations(self):
        ...
        if dt_mult > 0:
            ...
            # ذرات یخ/سرما هنگام دمای پایین
            if self.engine.temp_c < 15.0 and total_amount > 0 and not self.engine.is_broken:
                if random.random() < 0.35 * dt_mult:
                    self.steam_particles.append({
                        'x': random.uniform(margin_x + 10, self.width() - margin_x - 10),
                        'y': h - margin_y - 30 - (total_amount * (container_h / self.engine.max_capacity)) * random.uniform(0.2, 0.9),
                        'vy': random.uniform(-0.5, 0.8), 'life': 80, 'size': random.uniform(4, 12),
                        'ice': True
                    })
    ...
    def paintEvent(self, event):
        ...
        if not self.engine.is_broken:
            ...
            # افکت یخ‌زدگی — دما یا حالت خنک‌کننده
            is_cold = self.engine.temp_c < 15.0 or self.plate_state == "cool"
            if is_cold and not self.engine.is_broken:
                try:
                    if self.plate_state == "cool":
                        frost_intensity = max(0.4, min(1.0, (15.0 - float(self.engine.temp_c)) / 20.0))
                    else:
                        frost_intensity = min(1.0, max(0.25, (15.0 - float(self.engine.temp_c)) / 30.0))
                    # لایه نیمه‌شفاف آبی روی مایع
                    frost_fill = QColor(160, 210, 255, int(30 + 70 * frost_intensity))
                    painter.setPen(Qt.NoPen)
                    painter.setBrush(frost_fill)
                    painter.drawRect(container_rect)
                    # بلورهای یخ روی دیواره
                    rng = random.Random(self.frost_seed + int(self.engine.temp_c * 3))
                    painter.setPen(QPen(QColor(200, 240, 255, int(120 + 100 * frost_intensity)), 1.5))
                    n_crystals = max(12, int(25 + 50 * frost_intensity))
                    for i in range(n_crystals):
                        side = rng.randint(0, 3)
                        if side == 0:  # left
                            x = container_rect.left() + rng.uniform(2, 18)
                            y = container_rect.top() + rng.uniform(10, container_rect.height() - 10)
                        elif side == 1:  # right
                            x = container_rect.right() - rng.uniform(2, 18)
                            y = container_rect.top() + rng.uniform(10, container_rect.height() - 10)
                        else:
                            x = container_rect.left() + rng.uniform(10, container_rect.width() - 10)
                            y = container_rect.top() + rng.uniform(5, 40)
                        size = rng.uniform(4, 10 + 8 * frost_intensity)
                        painter.save()
                        painter.translate(x, y)
                        for j in range(6):
                            angle = j * 60 + rng.uniform(-8, 8)
                            painter.drawLine(0, 0,
                                             size * math.cos(math.radians(angle)),
                                             size * math.sin(math.radians(angle)))
                        painter.restore()
                    # حاشیه یخ روی لبه ظرف
                    painter.setPen(QPen(QColor(180, 230, 255, int(100 + 120 * frost_intensity)), 3))
                    painter.setBrush(Qt.NoBrush)
                    painter.drawRoundedRect(container_rect.adjusted(2, 2, -2, -2), 4, 4)
                    # برچسب سرما
                    painter.setPen(QColor(150, 220, 255, 220))
                    font_c = painter.font()
                    font_c.setPointSize(10)
                    font_c.setBold(True)
                    painter.setFont(font_c)
                    painter.drawText(container_rect.adjusted(8, 8, -8, -8),
                                     Qt.AlignTop | Qt.AlignLeft, "❄️ سرما")
                except Exception:
                    pass
        ...
        for sp in self.steam_particles:
            if sp.get('ice'):
                alpha = max(0, min(255, int(180 * (sp['life'] / 80.0))))
...
class ModernLabWindow(QMainWindow):
    ...
    def create_missions_badges_tab(self):
        ...
        title.setStyleSheet("font-size: 20px; font-weight: bold; color: #89b4fa; border: none; background: transparent;")
        ...
        # لیست مأموریت‌ها به‌صورت کارت
        missions_lbl = QLabel("چالش‌های فعال")
        missions_lbl.setStyleSheet("font-size: 15px; font-weight: bold; color: #f9e2af;")
        outer.addWidget(missions_lbl)
        ...
        outer.addWidget(scroll_m, 2)

        # سازگاری با کد قدیمی که list_missions دارد
        self.list_missions = QListWidget()
        self.list_missions.hide()

        # مدال‌ها
        badges_lbl = QLabel("🏅 اتاق مدال‌ها — موس روی هر مدال برای جزئیات")
        badges_lbl.setStyleSheet("font-size: 15px; font-weight: bold; color: #f9e2af;")
        outer.addWidget(badges_lbl)

        scroll_badges = QScrollArea()
        scroll_badges.setWidgetResizable(True)
        scroll_badges.setMinimumHeight(150)
        ...
        outer.addWidget(scroll_badges, 1)
