...
QTableWidget {
    ...
    gridline-color: #2a1e3d;
    color: #e8e0f5;
    border: 1px solid #3d2a55;
    border-radius: 12px;
    selection-background-color: #3b2a55;
    selection-color: #f5f3ff;
    alternate-background-color: #140f20;
    outline: 0;
}
QTableWidget::item {
    padding: 8px 10px;
...
class ChemicalCalculator:
    ...
    def electron_configuration(z):
        ...
        if z in EXCEPTIONS:
            # تبدیل به superscript unicode ساده
            s = EXCEPTIONS[z]
...
if HAS_OPENGL:
    class GLBohrCanvas(QOpenGLWidget):
        ...
        def initializeGL(self):
            ...
            if getattr(self, "quadric", None) is None:
                try:
                    self.quadric = gluNewQuadric()
                except Exception as e:
                    print(f"[GL] quadric: {e}", flush=True)
                    self.quadric = None
    ...
    class GLLabWorld(QOpenGLWidget):
        ...
        def initializeGL(self):
            ...
            try:
                ...
                print(f"[GL] Lab3D quadric init: {e}", flush=True)
        ...
        def resizeGL(self, w, h):
            ...
            gluPerspective(fov, max(0.1, w / float(h)), 0.08, 60.0)
            glMatrixMode(GL_MODELVIEW)
...
class LoginDialog(QDialog):
    ...
    def _on_name_changed(self, text):
        ...
        if need_set:
            ...
            self.pass_input.setPlaceholderText("حداقل {MIN_ADMIN_PASSWORD_LEN} کاراکتر")
        else:
    ...
    def check_input(self):
        ...
        if not AdminAuth.is_password_set():
            ...
            if len(pwd.strip()) < MIN_ADMIN_PASSWORD_LEN:
                self.err_lbl.setText(f"⚠️ رمز باید حداقل {MIN_ADMIN_PASSWORD_LEN} کاراکتر باشد")
                return
            if pwd != pwd2:
...
class ModernLabWindow(QMainWindow):
    ...
    def _build_menubar(self):
        ...
        if not HAS_OPENGL:
            act_3d.setEnabled(False)
        m_view.addAction(act_3d)
        act_mol = QAction("🧬 مولکول ۳بعدی (ماده انتخاب‌شده)", self)
    ...
    def setup_ui(self):
        ...
        self.lbl_safety_status.setStyleSheet(
            "background-color: #1e2e1a; color: #a7f3d0; font-weight: bold; padding: 4px 10px; "
            "border-radius: 8px; border: 1px solid #34d399; font-size: 12px;")
        top_layout.addWidget(self.lbl_safety_status)

        top_layout.addStretch()

        self.lbl_top_status = QLabel(f"{APP_NAME} {APP_VERSION}")
        self.lbl_top_status.setStyleSheet("color: #22d3ee; font-weight: bold; font-size: 13px;")
    ...
    def _create_left_panel(self):
        ...
        self.btn_mol3d.setMinimumHeight(36)
        ...
        self.btn_mol3d.setStyleSheet(
            "QToolButton { background: qlineargradient(x1:0,y1:0,x2:1,y2:0, stop:0 #10b981, stop:1 #34d399);"
            "color: #0c0914; font-weight: bold; border-radius: 9px; font-size: 12px; border: 1px solid #6ee7b7; padding: 6px 10px; }"
            "QToolButton:hover { border: 2px solid #a7f3d0; }"
            "QToolButton::menu-indicator { image: none; width: 0; }")
        mol_menu = QMenu(self.btn_mol3d)
        mol_menu.addAction("🧬 مولکول مادهٔ انتخاب‌شده", self.show_selected_molecule_3d)
    ...
    def _create_bottom_toolbar(self, parent_layout=None):
        """نوار پایین فشرده — فقط میانبرهای پرکاربرد + منوی ابزارها."""
        ...
        toolbar.setStyleSheet(
            "#bottomToolbar { background-color: #120e1c; border-top: 2px solid #7c3aed; }")
        lay = QHBoxLayout(toolbar)
        lay.setContentsMargins(12, 6, 12, 6)
        lay.setSpacing(8)

        def mk_quick(text, slot, bg="#5b21b6", fg="#f5f3ff", checkable=False):
            b = QPushButton(text)
            b.setFixedHeight(36)
            ...
            b.setStyleSheet(
                ...
                f"border-radius: 9px; font-size: 12px; padding: 4px 12px; border: 1px solid #7c3aed; }}"
                f"QPushButton:hover {{ border: 2px solid #c4b5fd; }}"
                f"QPushButton:checked {{ background-color: #e879f9; color: #0c0914; border: 2px solid #f5d0fe; }}")
            b.clicked.connect(slot)
            return b

        # میانبرهای اصلی — همیشه در دسترس
        lay.addWidget(mk_quick("⚗️ فیلتر", self.action_filter, "#fbbf24", "#160f22"))
        self.btn_stirrer = mk_quick("🌪️ همزن", self.action_toggle_stirrer, "#c4b5fd", "#160f22", checkable=True)
        lay.addWidget(self.btn_stirrer)
        lay.addWidget(mk_quick("🚿 شستشو", self.action_wash, "#22d3ee", "#160f22"))
        lay.addWidget(mk_quick("🔥 +۵°", self.action_heat, "#fb7185", "#160f22"))
        lay.addWidget(mk_quick("🧊 −۵°", self.action_cool, "#818cf8", "#160f22"))

        lay.addStretch(1)

        # منوی کشویی «ابزارها» — بقیهٔ دکمه‌های پراکنده
        tools_btn = QToolButton()
        tools_btn.setText("🧰 ابزارها ▾")
        tools_btn.setPopupMode(QToolButton.InstantPopup)
        tools_btn.setFixedHeight(36)
        tools_btn.setCursor(Qt.PointingHandCursor)
        tools_btn.setStyleSheet(
            "QToolButton { background: qlineargradient(x1:0,y1:0,x2:1,y2:0, stop:0 #5b21b6, stop:1 #7c3aed);"
            "color: #fff; font-weight: bold; border-radius: 9px; padding: 6px 14px; border: 1px solid #a78bfa; }"
            "QToolButton:hover { background: #8b5cf6; }"
            "QToolButton::menu-indicator { image: none; width: 0; }")
        tools_menu = QMenu(tools_btn)
        tools_menu.addAction("📸 عکس از ظرف", self.action_screenshot)
        tools_menu.addAction("💾 ذخیره وضعیت", self.action_save_state_file)
        tools_menu.addAction("📂 بارگذاری وضعیت", self.action_load_state_file)
        tools_menu.addSeparator()
        tools_menu.addAction("🧬 مولکول ۳بعدی", self.show_selected_molecule_3d)
        tools_menu.addAction("🧪 ۳بعدی مخلوط", self.show_mixture_molecule_3d)
        if HAS_OPENGL:
            tools_menu.addAction("🔄 نمای ۲بعدی / ۳بعدی", self.toggle_beaker_view)
        tools_menu.addSeparator()
        tools_menu.addAction("📥 خروجی TXT", self.action_export_txt)
        tools_menu.addAction("📑 خروجی PDF", self.action_export_pdf)
        tools_menu.addSeparator()
        tools_menu.addAction("⚙️ تنظیمات", self.open_settings_panel)
        tools_menu.addAction("❓ راهنما", self.show_tutorial)
        tools_btn.setMenu(tools_menu)
        lay.addWidget(tools_btn)

        if parent_layout is not None:
            parent_layout.addWidget(toolbar, 0)
        ...
        if checked:
            if hasattr(self, "btn_titrate") and self.btn_titrate is not None:
                ...
                self.btn_titrate.setStyleSheet(
                    "background-color: #fb7185; color: #160f22; font-weight: bold; border-radius: 8px; font-size: 13px;")
            ...
            if hasattr(self, "btn_titrate") and self.btn_titrate is not None:
                ...
                self.btn_titrate.setStyleSheet(
                    "background-color: #818cf8; color: #160f22; font-weight: bold; border-radius: 8px; font-size: 13px;")
            if hasattr(self, 'lbl_titration_status'):
